...
class attachment_model extends model
{
    ...
    /**
     * 附件更新接口.
     * @param string|array $content 可传入空，html，数组形式url，url地址，传入空时，以cookie方式记录。
     * @param int          $keyid 传入附件关系表中的组装id
     * @param int          $isurl 为本地地址时设为1,以cookie形式管理时设置为2
     * @return bool
     */
            ...
            if ($isurl == 0) {
                ...
                if (is_array($matches) && !empty($matches)) {
                    ...
                    foreach ($att_arr as $_k => $_v) {
                        $att_arrs[$_k] = md5(str_replace([$domain, $dir_name], '', $_v));
                    }
                }
            } elseif ($isurl == 1) {
                if (is_array($content)) {
                    ...
                    $att_arrs[] = md5(str_replace([$domain, $dir_name], '', $content));
                }
            }
            $att_index_db->delete(['keyid' => $keyid]);
            if (is_array($att_arrs) && !empty($att_arrs)) {
                foreach ($att_arrs as $r) {
                    $infos = $this->get_one(['authcode' => $r], 'aid');
                    if ($infos) {
                        $this->update(['status' => 1], ['aid' => $infos['aid']]);
                        $att_index_db->insert(['keyid' => $keyid, 'aid' => $infos['aid']]);
                    }
                }
            }
        }
    ...
    private function api_update_cookie($keyid)
    {
        ...
        foreach ($att as $_v) {
            $this->update(['status' => 1], ['aid' => $_v['aid']]);
            $att_index_db->insert(['keyid' => $keyid, 'aid' => $_v['aid']]);
        }
    }
    ...
    public function api_delete($keyid)
    {
        ...
        $info = $att_index_db->select(['keyid' => $keyid], 'aid');
        if ($info) {
            $att_index_db->delete(['keyid' => $keyid]);
            foreach ($info as $_v) {
                if (!$att_index_db->get_one(['aid' => $_v['aid']])) {
                    $attachment->delete(['aid' => $_v['aid']]);
                }
            }
