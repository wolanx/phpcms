...
use phpcms\modules\content\fields\content_input;
use phpcms\modules\content\fields\content_update;

/**
 * 内容模型数据库操作类
 * @property url                 $url
 * @property array               $model
 * @property content_check_model $content_check_db
 */
class content_model extends model
{
    ...
    public function add_content($data, $isimport = 0)
    {
        ...
        $this->search_db = new search_model();
        ...
        if (isset($_POST['add_introduce']) && $systeminfo['description'] == '' && isset($modelinfo['content'])) {
            ...
            $systeminfo['description'] = str_cut(str_replace(["'", "\r\n", "\t", '[page]', '[/page]', '&ldquo;', '&rdquo;', '&nbsp;'], '', strip_tags($content)),
                $introcude_length);
        ...
        $hitsid = 'c-' . $modelid . '-' . $id;
        $this->hits_db->insert(['hitsid' => $hitsid, 'catid' => $systeminfo['catid'], 'updatetime' => SYS_TIME]);
        if ($data['status'] == 99) {
            //更新到全站搜索
        ...
        if ($id && isset($_POST['othor_catid']) && is_array($_POST['othor_catid'])) {
            ...
            foreach ($_POST['othor_catid'] as $cid => $_v) {
                ...
                $hitsid = 'c-' . $mid . '-' . $newid;
                $this->hits_db->insert(['hitsid' => $hitsid, 'catid' => $cid, 'updatetime' => SYS_TIME]);
            }
        }
        //END 发布到其他栏目
        //更新附件状态
    ...
    public function edit_content($data, $id)
    {
        ...
        if (!defined('IN_ADMIN')) {
            ...
            $us = $this->get_one(['id' => $id, 'username' => $_username]);
            if (!$us) {
                return false;
            }
        }
        ...
        if ($data['islink'] == 1) {
            ...
            $systeminfo['url'] = str_replace(['select ', ')', '\\', '#', "'"], ' ', $systeminfo['url']);
        } else {
            //更新URL地址
        ...
        $this->content_check_db->update($check_data, ['checkid' => 'c-' . $id . '-' . $this->modelid]);
        //生成静态
        $html = new html();
    ...
    public function status($ids = [], $status = 99)
    {
        ...
        if (is_array($ids) && !empty($ids)) {
            foreach ($ids as $id) {
                ...
                if ($status == 0) {
                    ...
                    $this->content_check_db->delete(['checkid' => 'c-' . $id . '-' . $this->modelid]);
                    $del = true;
                }
                if (!$del) {
                    $this->content_check_db->update(['status' => $status], ['checkid' => 'c-' . $id . '-' . $this->modelid]);
                }
            }
        } else {
            $this->update(['status' => $status], ['id' => $ids]);
            $del = false;
            $r = $this->get_one(['id' => $ids]);
            if ($status == 0) {
                //退稿发送短消息、邮件
                $message = L('reject_message_tips') . $r['title'] . "<BR><a href=\'index.php?m=member&c=content&a=edit&catid={$r[catid]}&id={$r[id]}\'><font color=red>" . L('click_edit') . "</font></a><br>";
                if (isset($_POST['reject_c']) && $_POST['reject_c'] != L('reject_msg')) {
                ...
                $this->content_check_db->delete(['checkid' => 'c-' . $ids . '-' . $this->modelid]);
                $del = true;
            }
            if (!$del) {
                $this->content_check_db->update(['status' => $status], ['checkid' => 'c-' . $ids . '-' . $this->modelid]);
            }
        }
    ...
    public function search_api($id = 0, $data = [], $action = 'update')
    {
        ...
        if ($action == 'update') {
            ...
            $this->search_db->update_search($typeid, $id, $fulltextcontent, addslashes($data['system']['title']) . ' ' . addslashes($data['system']['keywords']),
    ...
    private function update_category_items($catid, $action = 'add', $cache = 0)
    {
        ...
        if ($action == 'add') {
            $this->category_db->update(['items' => '+=1'], ['catid' => $catid]);
        } else {
            $this->category_db->update(['items' => '-=1'], ['catid' => $catid]);
        }
        if ($cache) {
    ...
    {
        $datas = $this->category_db->select(['modelid' => $this->modelid], 'catid,type,items', 10000);
        $array = [];
        foreach ($datas as $r) {
            if ($r['type'] == 0) {
