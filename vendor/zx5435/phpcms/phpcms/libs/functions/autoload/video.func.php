...
function get_comments($commentid)
{
	...
	$r = $db->get_one(['commentid' => $commentid]);
	if ($r) {
		echo $r['total'];
	} else {
		echo '0';
 ...
 * @param $isall 是否显示全部
 */
function video_filters($field = '', $modelid, $diyarr = [], $isall = 1)
{
	...
	foreach ($options as $_k) {
		...
		$option[$k]['name'] = $v[0];
		$option[$k]['value'] = $k;
		$option[$k]['url'] = video_filters_url($field, [$field => $k], 2, $modelid);
		$option[$k]['menu'] = $field_value == $k ? '<a href="#" class="ac">' . $v[0] . '</a>' : '<a href="' . $option[$k]['url'] . '">' . $v[0] . '</a>';
	}
	if ($isall) {
		$all['name'] = L('all');
		$all['url'] = video_filters_url($field, [$field => ''], 2, $modelid);
 ...
 * 构造筛选URL
 */
function video_filters_url($fieldname, $array = [], $type = 1, $modelid, $isphp = 0)
{
	if (empty($array)) {
		$array = $_GET;
	} else {
