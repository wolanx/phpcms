 ...
 * @param array  $cfg 邮件配置信息
 * @param string $sitename 邮件站点名称
 * @return bool
 */
function sendmail($toemail, $subject, $message, $from = '', $cfg = [], $sitename = '')
{
    if ($sitename == '') {
        $siteid = get_siteid();
    ...
    //发信者
    $email_from = $from == '' ? '=?' . CHARSET . '?B?' . base64_encode($sitename) . "?= <" . $from . ">" : (preg_match('/^(.+?) \<(.+?)\>$/', $from,
        $mats) ? '=?' . CHARSET . '?B?' . base64_encode($mats[1]) . "?= <$mats[2]>" : $from);
    ...
    $headers .= 'Message-ID: <' . gmdate('YmdHs') . '.' . substr(md5($email_message . microtime()), 0, 6) . rand(100000,
            999999) . '@' . $_SERVER['HTTP_HOST'] . ">{$maildelimiter}";
