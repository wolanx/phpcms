...
use phpcms\libs\classes\cache_factory;
 ...
 */
function array2string($data, $isformdata = true)
{
    if ($data == '' || empty($data)) {
        return '';
    }
...
function sys_auth($string, $operation = 'ENCODE', $key = '', $expiry = 0)
{
    ...
    $string = $operation == 'DECODE' ? base64_decode(strtr(substr($string, $ckey_length), '-_', '+/')) : sprintf('%010d',
 ...
 * @return string        语言字符
 */
function L($language = 'no_language', $pars = [], $modules = '')
{
    static $LANG = [];
    static $LANG_MODULES = [];
    static $lang = '';
    ...
    if (!$LANG) {
        require_once PATH_PHPCMS . 'languages' . DS . $lang . DS . 'system.lang.php';
        if (defined('IN_ADMIN')) {
            require_once PATH_PHPCMS . 'languages' . DS . $lang . DS . 'system_menu.lang.php';
        }
        if (file_exists(PATH_PHPCMS . 'languages' . DS . $lang . DS . ROUTE_M . '.lang.php')) {
            require_once PATH_PHPCMS . 'languages' . DS . $lang . DS . ROUTE_M . '.lang.php';
        }
    }
    if (!empty($modules)) {
        ...
        foreach ($modules AS $m) {
            if (!isset($LANG_MODULES[$m])) {
                require_once PATH_PHPCMS . 'languages' . DS . $lang . DS . $m . '.lang.php';
            }
        }
    }
...
function template($module = 'content', $template = 'index', $style = '')
{
    ...
    $template_cache = new \phpcms\libs\classes\template_cache();
    $compiledtplfile = CACHE_PATH . 'caches_template' . DS . $style . DS . $module . DS . $template . '.php';

    if (file_exists(PATH_PHPCMS . 'templates' . DS . $style . DS . $module . DS . $template . '.html')) {
        if (
            PC_DEBUG ||
            !file_exists($compiledtplfile) ||
            (@filemtime(PATH_PHPCMS . 'templates' . DS . $style . DS . $module . DS . $template . '.html') > @filemtime($compiledtplfile))
        ) {
            $template_cache->template_compile($module, $template, $style);
        }
    } else {
        $compiledtplfile = CACHE_PATH . 'caches_template' . DS . 'default' . DS . $module . DS . $template . '.php';
        if (!file_exists($compiledtplfile) || (file_exists(PATH_PHPCMS . 'templates' . DS . 'default' . DS . $module . DS . $template . '.html') && filemtime(PATH_PHPCMS . 'templates' . DS . 'default' . DS . $module . DS . $template . '.html') > filemtime($compiledtplfile))) {
            $template_cache->template_compile($module, $template, 'default');
        } elseif (!file_exists(PATH_PHPCMS . 'templates' . DS . 'default' . DS . $module . DS . $template . '.html')) {
            showmessage('Template does not exist.' . DS . $style . DS . $module . DS . $template . '.html');
        }
    }
...
function my_error_handler($errno, $errstr, $errfile, $errline)
{
    ...
    if (pc_base::load_config('system', 'errorlog')) {
        error_log(date('m-d H:i:s', SYS_TIME)
            . ' | ' . $errno
            . ' | ' . str_pad($errstr, 30)
            . ' | ' . $errfile
            . ' | ' . $errline
 ...
 * @param array|string $data 条件数组或者字符串
 ...
 * @return string 分页
 */
function pages($num, $curr_page, $perpage = 20, $urlrule = '', $array = [], $setpages = 10)
{
    ...
    if ($num > $perpage) {
        ...
        if ($curr_page < $pages) {
            if ($curr_page < $pages - 5 && $more) {
                $multipage .= ' ..<a href="' . pageurl($urlrule, $pages, $array) . '">' . $pages . '</a> <a href="' . pageurl($urlrule, $curr_page + 1,
                        $array) . '" class="a1">' . L('next') . '</a>';
            } else {
                $multipage .= ' <a href="' . pageurl($urlrule, $pages, $array) . '">' . $pages . '</a> <a href="' . pageurl($urlrule, $curr_page + 1,
                        $array) . '" class="a1">' . L('next') . '</a>';
            }
...
function show_ad($siteid, $id)
{
    ...
    $p = new \phpcms\model\poster_model();
    $r = $p->get_one(['spaceid' => $id, 'siteid' => $siteid], 'disabled, setting', '`id` ASC');
    if ($r['disabled']) {
        return '';
    }
    if ($r['setting']) {
...
function get_memberinfo($userid, $field = '')
{
    if (!is_numeric($userid)) {
        ...
        if (!isset($memberinfo[$userid])) {
            $member_db = pc_base::load_model('member_model');
            $memberinfo[$userid] = $member_db->get_one(['userid' => $userid]);
        }
...
function get_memberinfo_buyusername($username, $field = '')
{
    ...
    if (!isset($memberinfo[$username])) {
        $member_db = pc_base::load_model('member_model');
        $memberinfo[$username] = $member_db->get_one(['username' => $username]);
    }
 ...
 * @param int $is_userid $uid是否为v9 userid，如果为真，执行sql查询此用户的phpssouid
...
function get_memberavatar($uid, $is_userid = '', $size = '30')
{
    if ($is_userid) {
        ...
        $memberinfo = $db->get_one(['userid' => $uid]);
...
function menu_linkage($linkageid = 0, $id = 'linkid', $defaultvalue = 0)
{
    ...
    if ($datas['style'] == '1') {
        ...
        $var_div = $defaultvalue && (ROUTE_A == 'edit' || ROUTE_A == 'account_manage_info' || ROUTE_A == 'info_publish' || ROUTE_A == 'orderinfo') ? menu_linkage_level($defaultvalue,
            $linkageid,
            $infos) : $datas['title'];
...
function menu_linkage_level($linkageid, $keyid, $infos, $result = [])
{
 ...
 * @return string
 */
function menu_level($menuid, $cache_file, $cache_path = 'commons', $key = 'catname', $parentkey = 'parentid', $linkstring = ' > ', $result = [])
{
 ...
 * @param int $type 返回间隔符链接，完整路径名称 3 返回完整路径数组，2返回当前联动菜单名称，4 直接返回ID
 * @param array $result 递归使用字段1
 * @param array $infos 递归使用字段2
 * @return array|string
 */
function get_linkage($linkageid, $keyid, $space = '>', $type = 1, $result = [], $infos = [])
{
...
/**
 * 获取子栏目
 * @param null $parentid 父级id
 * @param null $type 栏目类型
...
{
    $pc_auth_key = md5(PATH_PHPCMS . 'upload' . pc_base::load_config('system', 'auth_key') . $_SERVER['HTTP_USER_AGENT']);
...
{
    if ($prefix == 'login') {
        $pc_auth_key = md5(PATH_PHPCMS . 'login' . pc_base::load_config('system', 'auth_key') . ip());
    } else {
        if ($prefix == 'email') {
            $pc_auth_key = md5(PATH_PHPCMS . 'email' . pc_base::load_config('system', 'auth_key'));
        } else {
            $pc_auth_key = md5(PATH_PHPCMS . 'other' . pc_base::load_config('system', 'auth_key') . $suffix);
        }
    }
...
function runhook($method)
{
    ...
    if (!empty($hook_appid)) {
        foreach ($hook_appid as $appid => $p) {
            $pluginfilepath = PATH_PHPCMS . 'plugin' . DS . $p . DS . 'hook.class.php';
            $getpclass = true;
...
function p_template($plugin = 'content', $template = 'index', $style = 'default')
{
    ...
    $compiledtplfile = PATH_CACHES . 'caches_template' . DS . $style . DS . 'plugin' . DS . $plugin . DS . $template . '.php';

    if (!file_exists($compiledtplfile) || (file_exists(PATH_PHPCMS . 'plugin' . DS . $plugin . DS . 'templates' . DS . $template . '.html') && filemtime(PATH_PHPCMS . 'plugin' . DS . $plugin . DS . 'templates' . DS . $template . '.html') > filemtime($compiledtplfile))) {
        $template_cache->template_compile('plugin/' . $plugin, $template, 'default');
    } elseif (!file_exists(PATH_PHPCMS . 'plugin' . DS . $plugin . DS . 'templates' . DS . $template . '.html')) {
        showmessage('Template does not exist.' . DS . 'plugin' . DS . $plugin . DS . $template . '.html');
    }
...
function get_vid($contentid = 0, $catid = 0, $isspecial = 0)
{
    ...
    if (!$isspecial) {
        ...
        $r = $video_content->get_one(['contentid' => $contentid, 'modelid' => $modelid], 'videoid', '`listorder` ASC');
        $video_store = pc_base::load_model('video_store_model');

        return $video_store->get_one(['videoid' => $r['videoid']]);
    } else {
        $special_content = pc_base::load_model('special_content_model');
        $contentid = intval($contentid);
        $video_store = pc_base::load_model('video_store_model');
        $r = $special_content->get_one(['id' => $contentid], 'videoid');
