...
class attachment
{
    ...
    var $attachments;
    var $field;
    var $imageexts = ['gif', 'jpg', 'jpeg', 'png', 'bmp'];
    var $uploadedfiles = [];
    var $downloadedfiles = [];
    var $error;
    var $upload_root;
     ...
     * @param string $alowexts 允许上传类型
     * @param int    $maxsize 最大上传大小
     * @param int    $overwrite 是否覆盖原有文件
     * @param array  $thumb_setting 缩略图设置
     * @param int    $watermark_enable 是否添加水印
     * @return array
     */
    function upload($field, $alowexts = '', $maxsize = 0, $overwrite = 0, $thumb_setting = [], $watermark_enable = 1)
    {
        ...
        $uploadfiles = [];
        $description = isset($GLOBALS[$field . '_description']) ? $GLOBALS[$field . '_description'] : [];
        ...
        foreach ($uploadfiles as $k => $file) {
            ...
            if (@$upload_func($file['tmp_name'], $savefile)) {
                ...
                $uploadedfile = ['filename' => $file['name'], 'filepath' => $filepath, 'filesize' => $file['size'], 'fileext' => $fileext, 'fn' => $file['fn']];
    ...
    function download($field, $value, $watermark = '0', $ext = 'gif|jpg|jpeg|bmp|png', $absurl = '', $basehref = '')
    {
        ...
        foreach ($remotefileurls as $k => $file) {
            ...
            if (!preg_match("/($ext)/is", $filename) || in_array($filename,
                    ['php', 'phtml', 'php3', 'php4', 'jsp', 'dll', 'asp', 'cer', 'asa', 'shtml', 'shtm', 'aspx', 'asax', 'cgi', 'fcgi', 'pl'])
            ) {
                continue;
            }
            ...
            if ($upload_func($file, $newfile)) {
                ...
                $downloadedfile = ['filename' => $filename, 'filepath' => $filepath, 'filesize' => filesize($newfile), 'fileext' => $fileext];
