...
class cache_file
{
    ...
    public function set($name, $data, $setting = '', $type = 'data', $module = ROUTE_M)
    {
        ...
        if ($module == 'commons' || ($module == 'commons' && substr($name, 0, 16) != 'category_content')) {
            ...
            if ($db->get_one(['filename' => $filename, 'path' => 'caches_' . $module . '/caches_' . $type . '/'], '`filename`')) {
                $db->update(['data' => $datas], ['filename' => $filename, 'path' => 'caches_' . $module . '/caches_' . $type . '/']);
            } else {
                $db->insert(['filename' => $filename, 'path' => 'caches_' . $module . '/caches_' . $type . '/', 'data' => $datas]);
            }
        }
    ...
    public function delete($name, $setting = '', $type = 'data', $module = ROUTE_M)
    {
        ...
        if (file_exists($filepath . $filename)) {
            if ($module == 'commons' && substr($name, 0, 16) != 'category_content') {
                $db = pc_base::load_model('cache_model');
                $db->delete(['filename' => $filename, 'path' => 'caches_' . $module . '/caches_' . $type . '/']);
            }
