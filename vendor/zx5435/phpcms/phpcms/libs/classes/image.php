...
class image
{
	...
	function __construct($thumb_enable = 0, $siteid)
	{
		...
		if ($siteid) {
			...
			$this->set($this->site_setting['watermark_img'], $this->site_setting['watermark_pos'], $this->site_setting['watermark_minwidth'], $this->site_setting['watermark_minheight'],
				$this->site_setting['watermark_quality'], $this->site_setting['watermark_pct']);
		}
	}
	...
	function watermark($source, $target = '', $w_pos = '', $w_img = '', $w_text = 'phpcms', $w_font = 8, $w_color = '#ff0000')
	{
		...
		if (!empty($w_img) && file_exists($w_img)) {
			...
			$ifwaterimage = 0;
			$temp = imagettfbbox(ceil($w_font * 2.5), 0, PATH_PHPCMS . 'libs/data/font/elephant.ttf', $w_text);
			$width = $temp[2] - $temp[6];
