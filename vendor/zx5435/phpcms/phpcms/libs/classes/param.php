...
class param
{
    ...
    public static function set_cookie($var, $value = '', $time = 0)
    {
        ...
        if (is_array($value)) {
            foreach ($value as $k => $v) {
                setcookie($var . '[' . $k . ']', sys_auth($v, 'ENCODE', md5(PATH_PHPCMS . 'cookie' . $var) . pc_base::load_config('system', 'auth_key')), $time,
                    pc_base::load_config('system', 'cookie_path'), pc_base::load_config('system', 'cookie_domain'), $s, $httponly);
            }
        } else {
            setcookie($var, sys_auth($value, 'ENCODE', md5(PATH_PHPCMS . 'cookie' . $var) . pc_base::load_config('system', 'auth_key')), $time, pc_base::load_config('system', 'cookie_path'),
                pc_base::load_config('system', 'cookie_domain'), $s, $httponly);
        }
    }
    ...
    {
        $var = pc_base::load_config('system', 'cookie_pre') . $var;
        $value = isset($_COOKIE[$var]) ? sys_auth($_COOKIE[$var], 'DECODE', md5(PATH_PHPCMS . 'cookie' . $var) . pc_base::load_config('system', 'auth_key')) : $default;
        if (in_array($var, ['_userid', 'userid', 'siteid', '_groupid', '_roleid'])) {
            $value = intval($value);
        } elseif (in_array($var, ['_username', 'username', '_nickname', 'admin_username', 'sys_lang'])) { //  site_model auth
