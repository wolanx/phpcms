...
final class template_cache
{
     ...
     * @param string $style 是否为标签模板
     * @return int
     */
    public function template_compile($module, $template, $style = 'default')
    {
        if (strpos($module, '/') === false) {
            $tplfile = $_tpl = PATH_PHPCMS . 'templates' . DS . $style . DS . $module . DS . $template . '.html';
            ...
            $module = str_replace('/', DS, $module);
            $tplfile = $_tpl = PATH_PHPCMS . 'plugin' . DS . $plugin . DS . 'templates' . DS . $template . '.html';
        }
        if ($style != 'default' && !file_exists($tplfile)) {
            $style = 'default';
            $tplfile = PATH_PHPCMS . 'templates' . DS . 'default' . DS . $module . DS . $template . '.html';
        }
        if (!file_exists($tplfile)) {
            showmessage("templates" . DS . $style . DS . $module . DS . $template . ".html is not exists!");
        }
        $content = @file_get_contents($tplfile);

        $filepath = CACHE_PATH . 'caches_template' . DS . $style . DS . $module . DS;
        if (!is_dir($filepath)) {
    ...
    public function template_parse($str)
    {
        ...
        $str = preg_replace("/\{\\$([a-zA-Z_\x7f-\xff][a-zA-Z0-9_\x7f-\xff:]*\(([^{}]*)\))\}/", "<?php echo \\1;?>", $str);
        $str = preg_replace("/\{(\\$[a-zA-Z_\x7f-\xff][a-zA-Z0-9_\x7f-\xff]*)\}/", "<?php echo \\1;?>", $str);
        $str = preg_replace_callback("/\{(\\$[a-zA-Z0-9_\[\]\'\"\$\x7f-\xff]+)\}/s", [$this, 'addquote'], $str);
        $str = preg_replace("/\{([A-Z_\x7f-\xff][A-Z0-9_\x7f-\xff]*)\}/s", "<?php echo \\1;?>", $str);
        $str = preg_replace_callback("/\{pc:(\w+)\s+([^}]+)\}/i", [$this, 'pc_tag_callback'], $str);
        $str = preg_replace_callback("/\{\/pc\}/i", [$this, 'end_pc_tag'], $str);
    ...
    public static function pc_tag($op, $data, $html)
    {
        ...
        $arr = ['action', 'num', 'cache', 'page', 'pagesize', 'urlrule', 'return', 'start'];
        $tools = ['json', 'xml', 'block', 'get'];
        $datas = [];
        ...
        if (in_array($op, $tools)) {
            ...
            if (module_exists($op) && file_exists(PATH_PHPCMS . DS . 'modules' . DS . $op . DS . 'classes' . DS . $op . '_tag.php')) {
