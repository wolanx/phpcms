...
class form
{
    ...
    public static function editor(
        ...
        if ($allowupload) {
            $str .= "filebrowserUploadUrl : 'index.php?m=attachment&c=attachments&a=upload&module=" . $module . "&catid=" . $catid . "&dosubmit=1',\r\n";
        }
        if ($color) {
     ...
     * @return string
     */
    public static function upfiles($name, $id = '', $value = '', $moudle = '', $catid = '', $size = 50, $class = '', $ext = '', $alowexts = '', $file_setting = [])
    {
        if (!$id) {
            $id = $name;
        }
        ...
        if (is_array($result)) {
            foreach ($result as $r) {
                ...
                if ($is_push == '1' and $r['child'] == '0') {
                    $sql = ['catid' => $r['catid'], 'roleid' => $user_groupid, 'action' => 'add'];
    ...
    public static function select_linkage($keyid = 0, $parentid = 0, $name = 'parentid', $id = '', $alt = '', $linkageid = 0, $property = '')
    {
        ...
        if ($result['data']) {
            foreach ($result['data'] as $area) {
                $categorys[$area['linkageid']] = ['id' => $area['linkageid'], 'parentid' => $area['parentid'], 'name' => $area['name']];
            }
        }
    ...
    public static function select_template($style, $module, $id = '', $str = '', $pre = '')
    {
        ...
        $templatedir = PATH_PHPCMS . $tpl_root . DS . $style . DS . $module . DS;
        $confing_path = PATH_PHPCMS . $tpl_root . DS . $style . DS . 'config.php';
        $localdir = str_replace(['/', '\\'], '', $tpl_root) . '|' . $style . '|' . $module;
