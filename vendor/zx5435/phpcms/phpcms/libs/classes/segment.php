...
class segment
{
    ...
    function __construct($loaddic = true)
    {
        if ($loaddic) {
            ...
            $dicfile = PATH_PHPCMS . 'libs' . DS . 'data' . DS . 'dict' . DS . 'dict.csv';
            $fp = fopen($dicfile, 'r');
    ...
    function split_result($str = '', $try_num_name = true, $try_diff = true)
    {
        ...
        for ($i = $spLen; $i >= 0; $i--) {
            if (ord($spwords[$i][0]) < 33) {
                ...
                if (!isset($spwords[$i][$this->min_len])) {
                    $this->result_string = $spwords[$i] . $spc . $this->result_string;
                } else {
                    if (ord($spwords[$i][0]) < 0x81) {
                        $this->result_string = $spwords[$i] . $spc . $this->result_string;
                    } else {
    ...
    function par_other($word_array)
    {
        ...
        for ($i = $wlen; $i >= 0; $i--) {
            ...
            if (strlen($word_array[$i]) == 4 && isset($this->two_name_dic[$word_array[$i]])) {
                ...
                if (strlen($word_array[$i]) == 2 && isset($this->one_name_dic[$word_array[$i]])) {
                    $rsStr .= $spc . $word_array[$i];
                    if ($i > 0 && strlen($word_array[$i - 1]) == 2) {
    ...
    function split_mm($str, $try_num_name = true, $try_diff = true)
    {
        ...
        if ($try_diff) {
            $rsStr = $this->test_diff(trim($rsStr));
        }
        ...
        for ($i = 0; $i < $wlen; $i++) {
            if ($i >= ($wlen - 1)) {
                ...
                if (strlen($ws[$i]) == 2 && strlen($ws[$i + 1]) < 8 && strlen($ws[$i + 1]) > 2) {
                    ...
                    if (strlen($ws[$i]) > 2 && strlen($ws[$i]) < 8 && strlen($ws[$i + 1]) > 2 && strlen($ws[$i + 1]) < 8) {
        ...
        for ($i = 0; $i < $slen; $i++) {
            if (ord($str[$i]) < 0x81) {
                if (ord($str[$i]) < 33) {
                    ...
                    if (preg_match("/[^0-9a-zA-Z@\.%#:\\/\\&_-]/", $str[$i])) {
                        if ($prechar == 0) {
                            $okstr .= $str[$i];
                            $prechar = 3;
                        } else {
