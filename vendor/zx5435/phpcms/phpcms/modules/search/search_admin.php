...
class search_admin extends admin
{
    ...
    public function setting()
    {
        ...
        if (isset($_POST['dosubmit'])) {
            //合并数据库缓存与新提交缓存
            $r = $this->module_db->get_one(['module' => 'search']);
            ...
            $this->module_db->update(['setting' => $setting], ['module' => 'search']);
            ...
            $big_menu = [
                'javascript:window.top.art.dialog({id:\'add\',iframe:\'?m=search&c=search_type&a=add\', title:\'' . L('add_search_type') . '\', width:\'580\', height:\'240\', lock:true}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);',
                L('add_search_type'),
            ];
    ...
    public function createindex()
    {
        if (isset($_GET['dosubmit'])) {
            ...
            if (!isset($_GET['have_truncate'])) {
                ...
                //删除该站点全文索引
                $this->db->delete(['siteid' => $this->siteid]);

                $types = $this->type_db->select(['siteid' => $this->siteid, 'module' => 'search']);
            ...
            foreach ($types as $_k => $_v) {
                if ($key == $_k) {
                    ...
                    $page++;
                    if ($pages >= $page) {
                        showmessage("正在更新 <span style='color:#ff0000;font-size:14px;text-decoration:underline;' >{$_v['name']}</span> - 总数：{$total} - 当前第 <font color='red'>{$page}</font> 页",
                            "?m=search&c=search_admin&a=createindex&menuid=909&page={$page}&total={$total}&key={$key}&pagesize={$pagesize}&have_truncate=1&dosubmit=1");
                    }
                    $key++;
                    showmessage("开始更新： <span style='color:#ff0000;font-size:14px;text-decoration:underline;' >{$_v['name']}</span> - 总数：{$total}条",
                        "?m=search&c=search_admin&a=createindex&menuid=909&page=1&key={$key}&pagesize={$pagesize}&have_truncate=1&dosubmit=1");
