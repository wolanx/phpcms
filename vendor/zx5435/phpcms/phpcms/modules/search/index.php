...
use phpcms\model\search_keyword_model;
use phpcms\model\search_model;

/**
 * @property search_keyword_model $keyword_db
 */
class index
{
    ...
    public function init()
    {
        ...
        if (isset($_GET['q'])) {
            ...
            $time = empty($_GET['time']) || !in_array($_GET['time'], ['all', 'day', 'month', 'year', 'week']) ? 'all' : trim($_GET['time']);
            ...
            if ($setting['sphinxenable']) {
                ...
                $res = $sphinx->search($q, [$siteid], [$typeid], [$search_time, SYS_TIME], $offset, $pagesize, '@weight desc');
                $totalnums = $res['total'];
                //如果结果不为空
            ...
            if ($setting['relationenble']) {
                ...
                if (strlen($q) < 17 && strlen($q) > 5 && !empty($segment_q)) {
                    $res = $this->keyword_db->get_one(['keyword' => $q]);
                    if ($res) {
                        //关键词搜索数+1
                        ...
                        $this->keyword_db->insert(['keyword' => $q, 'searchnums' => 1, 'data' => $segment_q, 'pinyin' => $pinyin]);
                    }
                }
                //相关搜索
                if (!empty($segment_q)) {
    ...
    public function public_get_suggest_keyword()
    {
        ...
        $trust_url = ['c8430fcf851e85818b546addf5bc4dd3'];
        $urm_md5 = md5($url);
        if (!in_array($urm_md5, $trust_url)) {
            exit;
        }
    ...
    public function public_suggest_search()
    {
        ...
        $this->keyword_db = new search_keyword_model();
