...
namespace phpcms\modules\release;

use pc_base;
use phpcms\libs\classes\ftps;
use phpcms\model\queue_model;
use phpcms\model\release_point_model;
...
class index extends admin
{
    ...
    {
        parent::__construct();
        $this->db = new release_point_model();
    ...
    public function public_sync()
    {
        ...
        if (!empty($ids)) {
            ...
            if (empty($total)) {
                $total = $queue->count(["siteid" => $this->get_siteid(), "status" . ($id + 1) => $statuses]);
            }
            $totalpage = ceil($total / $pagesize);
            $data = $queue->select(["siteid" => $this->get_siteid(), "status" . ($id + 1) => $statuses], 'id, type, path', $pagesize);
        }
        $release_point = $this->db->get_one(['id' => $this->point[$id]]);
        $ftps = new ftps();
        if (is_array($data) && !empty($data)) {
            if ($ftps->connect($release_point['host'], $release_point['username'], $release_point['password'], $release_point['port'], $release_point['pasv'],
                $release_point['ssl'])) {
                ...
                foreach ($data as $v) {
                    ...
                    $queue->update(['status' . ($id + 1) => $status, 'times' => SYS_TIME], ['id' => $v['id']]);
                }
            } else {
                exit('<script type="text/javascript">alert("' . L("release_point_connect_failure", ['name' => $release_point['name']]) . '");</script>');
            }
        }
