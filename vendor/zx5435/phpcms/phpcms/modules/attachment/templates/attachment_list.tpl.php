...
<div class="pad-lr-10">
    <form name="searchform" action="" method="get">
        ...
        <table width="100%" cellspacing="0" class="search-form">
            ...
            <tr>
                <td>
                    <div class="explain-col"><?php echo L('name') ?> <input type="text" value="<?php echo $filename ?>" class="input-text"
                                                                            name="info[filename]"> <?php echo L('uploadtime') ?>  <?php echo form::date('info[start_uploadtime]',
                            $start_uploadtime, '1', '0', 'true', '1') ?><?php echo L('to') ?>   <?php echo form::date('info[end_uploadtime]',
                            $end_uploadtime) ?>  <?php echo L('filetype') ?> <input type="text" value="<?php echo $fileext ?>" class="input-text" name="info[fileext]"> <input
                                type="submit" value="<?php echo L('search') ?>" class="button" name="dosubmit"> <a
                                href="?m=attachment&c=manage&a=dir&menuid=<?php echo $_GET['menuid'] ?>"><?php echo L('dir_schema') ?></a>
                    </div>
                </td>
            </tr>
            </tbody>
    ...
    <div class="table-list">
        <form name="myform" action="?m=admin&c=role&a=listorder" method="post" id="myform">
                    ...
                    <th width="8%"><?php echo L('moudle') ?>
                        <div class="tab-use">
                            <div style="position:relative">
                                ...
                                <ul id="module-div" class="tab-web-panel" onmouseover="this.style.display='block'" onmouseout="hoverUse('module-div');"
                                    style="height:150px; width:100px; text-align:left; overflow-y:scroll;">
                                    <?php foreach ($modules as $module) {
                                        if (in_array($module['module'],
                                            ['pay', 'digg', 'search', 'scan', 'attachment', 'block', 'dbsource', 'template', 'release', 'cnzz', 'comment', 'mood'])) {
                                            continue;
                                        }
                ...
                if (is_array($infos)) {
                    foreach ($infos as $info) {
                        ...
                        <tr>
                            ...
                            <td width="20%"><img src="<?php echo file_icon($info['filename'],
                                    'gif') ?>"/> <?php echo $info['filename'] ?> <?php echo $thumb ? '<img title="' . L('att_thumb_manage') . '" src="statics/images/admin_img/havthumb.png" onclick="showthumb(' . $info['aid'] . ', \'' . new_addslashes($info['filename']) . '\')"/>' : '' ?> <?php echo $info['status'] ? '<img src="statics/images/admin_img/link.png"' : '' ?>
                            </td>
                            ...
                            <td align="center"><a
                                        href="javascript:preview(<?php echo $info['aid'] ?>, '<?php echo $info['filename'] ?>','<?php echo $this->upload_url . $info['filepath'] ?>')"><?php echo L('preview') ?></a>
                                | <a href="javascript:void(0);" onclick="att_delete(this,'<?php echo $info['aid'] ?>')"><?php echo L('delete') ?></a></td>
                        </tr>
                        <?php
                    }
                }
            ...
            <div class="btn"><a href="#" onClick="javascript:$('input[type=checkbox]').attr('checked', true)"><?php echo L('selected_all') ?></a>/<a href="#"
                ...
                <input type="submit" class="button" name="dosubmit" value="<?php echo L('delete') ?>"
                       onClick="document.myform.action='?m=attachment&c=manage&a=public_delete_all';return confirm('<?php echo L('del_confirm') ?>')"/></div>
...
<script type="text/javascript">
    ...
    function showthumb(id, name) {
        window.top.art.dialog({
            ...
            id: 'edit',
            iframe: '?m=attachment&c=manage&a=pullic_showthumbs&aid=' + id,
            width: '500px',
            height: '400px'
        });
    }
