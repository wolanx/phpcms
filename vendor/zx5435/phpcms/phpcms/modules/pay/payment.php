...
namespace phpcms\modules\pay;

use pc_base;
use phpcms\libs\classes\param;
use phpcms\modules\admin\classes\admin;
use phpcms\modules\pay\classes\pay_method;

pc_base::load_app_func('global');

/**
 * @property pay_method $method
 */
class payment extends admin
{
    ...
    function __construct()
    {
        ...
        $this->modules_path = PATH_PHPCMS . 'modules' . DS . 'pay';
    ...
    public function edit()
    {
        if (isset($_POST['dosubmit'])) {
            ...
            $info['website'] = $infos['website'];
            $info['version'] = $infos['version'];
            $infos = $this->db->update($info, ['pay_id' => $info['pay_id']]);
            showmessage(L('edit') . L('succ'), '', '', 'edit');
        } else {
            $pay_id = intval($_GET['id']);
            $infos = $this->db->get_one(['pay_id' => $pay_id]);
            extract($infos);
    ...
    public function pay_list()
    {
        ...
        if ($_GET['dosubmit']) {
            extract($_GET['info']);
            if ($trade_sn) {
                $where = "AND `trade_sn` LIKE '%$trade_sn%' ";
            }
            if ($username) {
    ...
    public function discount()
    {
        if (isset($_POST['dosubmit'])) {
            ...
            $id = intval($_POST['id']);
            $infos = $this->account_db->update(['discount' => $discount], ['id' => $id]);
            showmessage(L('public_discount_succ'), '', '', 'discount');
        } else {
            $show_header = true;
            $show_validator = true;
            $id = intval($_GET['id']);
            $infos = $this->account_db->get_one(['id' => $id]);
            extract($infos);
        ...
        if (isset($_POST['dosubmit'])) {
            ...
            if ($userinfo) {
                ...
                if (intval($_POST['sendemail'])) {
                    ...
                    $msg = L('account_changes_notice_tips', [
                        'username' => $username,
    ...
    {
        $id = intval($_GET['id']);
        $this->account_db->update(['status' => 'cancel'], ['id' => $id]);
    ...
    public function public_check()
    {
        $id = intval($_GET['id']);
        $infos = $this->account_db->get_one(['id' => $id]);
        $userinfo = $this->member_db->get_one(['userid' => $infos['userid']]);
        $amount = $userinfo['amount'] + $infos['money'];
        $this->account_db->update(['status' => 'succ', 'adminnote' => param::get_cookie('admin_username')], ['id' => $id]);
        $this->member_db->update(['amount' => $amount], ['userid' => $infos['userid']]);
    ...
    private function get_useid($username)
    {
        ...
        if ($result = $this->member_db->get_one(['username' => $username])) {
            return $result;
        } else {
            return false;
        }
    ...
    public function public_checkname_ajax()
    {
        ...
        $this->member_db = pc_base::load_model('member_model');
        if ($r = $this->member_db->get_one(['username' => $username])) {
