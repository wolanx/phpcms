...
use phpcms\modules\pay\classes\pay_deposit;
use phpcms\modules\pay\classes\pay_factory;
...
class deposit extends foreground
{
    ...
    public function pay_recharge()
    {
        if (isset($_POST['dosubmit'])) {
            ...
            if (!$pay_id) {
                showmessage(L('illegal_pay_method'));
            }
            ...
            if (preg_match('![^a-zA-Z0-9/+=]!', $trade_sn)) {
                showmessage(L('illegal_creat_sn'));
            }
            ...
            if (!$factory_info) {
                showmessage(L('order_closed_or_finish'));
            }
            ...
            if ($payment['is_online'] === '1') {
                ...
                $this->account_db->update(['status' => 'waitting', 'pay_type' => 'offline'], ['id' => $recordid]);
