...
use phpcms\model\pay_spend_model;
...
class spend
{
     ...
     * @param int|string $op_userid 操作人
    ...
    private static function _add($data)
    {
        ...
        if ($data['type'] == 1) {//金钱方式消费
            $sql = ['amount' => "-=" . $data['value']];
        } elseif ($data['type'] == 2) { //积分方式消费
            $sql = ['point' => '-=' . $data['value']];
        } else {
            self::$msg = 7;
        ...
        //进入数据库操作
        if ($member_db->update($sql, ['userid' => $data['userid'], 'username' => $data['username']]) && self::$db->insert($data)) {
            self::$msg = 0;
    ...
    {
        if ($user = $db->get_one(['userid' => $userid], '`amount`, `point`')) {
            if ($type == 1) { //金钱消费
