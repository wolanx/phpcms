                ...
                <tr>
                    <td>
                        <div class="explain-col">
                            <?php echo form::select(['1' => L('username'), '2' => L('userid')], $user_type, 'name="user_type"') ?>： <input type="text"
                                                                                                                                           value="<?php echo $username ?>"
                                                                                                                                           class="input-text" name="username">
                            <?php echo L('from') ?>  <?php echo form::date('starttime', format::date($starttime)) ?> <?php echo L('to') ?>   <?php echo form::date('endtime',
                                format::date($endtime)) ?>

                            <?php echo form::select(['' => L('op'), '1' => L('username'), '2' => L('userid')], $op_type, 'name="op_type"') ?>：
                            <input type="text" value="<?php echo $op ?>" class="input-text" name="op">
                            <?php echo form::select(['' => L('expenditure_patterns'), '1' => L('money'), '2' => L('point')], $type, 'name="type"') ?>
            ...
            if (is_array($list)) {
                ...
                foreach ($list as $info) {
                    ...
                    <tr>
                        ...
                        <td width="9%" align="center"><?php if (!empty($info['op_userid'])) {
                                echo $info['op_username'];
                            } else {
                                echo L('self');
                            } ?></td>
                        <td width="8%" align="center"><?php if ($info['type'] == 1) {
                                echo L('money');
                            } elseif ($info['type'] == 2) {
                                echo L('point');
                            } ?></td>
