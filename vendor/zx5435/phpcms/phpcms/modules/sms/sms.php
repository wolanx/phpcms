...
namespace phpcms\modules\sms;

use pc_base;
use phpcms\modules\admin\classes\admin;
use phpcms\modules\sms\classes\smsapi;

pc_base::load_app_func('global', 'sms');

/**
 * @property $sms_setting_arr
 */
class sms extends admin
{
    ...
    public function sms_buy_history()
    {
        ...
        //字符转换
        if (CHARSET != 'utf-8') {
            if (is_array($payinfo_arr)) {
                foreach ($payinfo_arr as $k => $v) {
    ...
    public function sms_pay_history()
    {
        ...
        $paylist_arr = !empty($paylist_apiarr['datas']) ? $paylist_apiarr['datas'] : [];
    ...
    public function sms_setting()
    {
        ...
        if (isset($_POST['dosubmit'])) {
            ...
            $this->module_db->update(['setting' => $setting], ['module' => 'sms']);
    ...
    public function sms_sent()
    {
        if (!$this->sms_setting['sms_enable']) {
            showmessage(L('please_open_sms_platform_status'));
        }
