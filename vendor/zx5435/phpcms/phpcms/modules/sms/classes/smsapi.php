...
namespace phpcms\modules\sms\classes;
...
class smsapi
{
    ...
    {
        $this->param = ['op' => 'sms_get_productlist'];
        $res = $this->pc_file_get_contents();

        return !empty($res) ? json_decode($res, 1) : [];
    }
    ...
    {
        $this->param = ['op' => 'sms_get_report', 'page' => $page];
        $res = $this->pc_file_get_contents();
    ...
    {
        $this->param = ['op' => 'sms_help', 'page' => $page];
        $res = $this->pc_file_get_contents();
    ...
    public function send_sms($mobile = '', $content = '', $send_time = '', $charset = 'gbk', $id_code = '', $tplid = '', $return_code = 0)
    {
        ...
        if (isset($this->statuscode)) {
            ...
            $sms_report_db->insert(['mobile' => $mobile, 'posttime' => SYS_TIME, 'send_userid' => $send_userid, 'status' => '-2', 'msg' => $new_content, 'ip' => $ip]);
        }
