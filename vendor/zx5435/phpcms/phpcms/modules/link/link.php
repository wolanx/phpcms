...
namespace phpcms\modules\link;

use pc_base;
use phpcms\model\attachment_model;
use phpcms\model\link_model;
use phpcms\model\type_model;
use phpcms\modules\admin\classes\admin;

/**
 * @property link_model $db
 * @property type_model $db2
 */
class link extends admin
{
    public function __construct()
    {
        ...
        $this->db = new link_model();
        $this->db2 = new type_model();
    }

    public function init()
    {
        if ($_GET['typeid'] != '') {
            $where = ['typeid' => intval($_GET['typeid']), 'siteid' => $this->get_siteid()];
        } else {
            $where = ['siteid' => $this->get_siteid()];
        }
        $page = isset($_GET['page']) && intval($_GET['page']) ? intval($_GET['page']) : 1;
        $infos = $this->db->listinfo($where, $order = 'listorder DESC,linkid DESC', $page, $pages = '9');
        $pages = $this->db->pages;
        $types = $this->db2->listinfo(['module' => ROUTE_M, 'siteid' => $this->get_siteid()], $order = 'typeid DESC');
        ...
        $big_menu = [
            'javascript:window.top.art.dialog({id:\'add\',iframe:\'?m=link&c=link&a=add\', title:\'' . L('link_add') . '\', width:\'700\', height:\'450\'}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);',
            L('link_add'),
        ];
    ...
    {
        $link_title = isset($_GET['link_name']) && trim($_GET['link_name']) ? (pc_base::load_config('system', 'charset') == 'gbk' ? iconv('utf-8', 'gbk',
            trim($_GET['link_name'])) : trim($_GET['link_name'])) : exit('0');

        $linkid = isset($_GET['linkid']) && intval($_GET['linkid']) ? intval($_GET['linkid']) : '';
        $data = [];
        if ($linkid) {

            $data = $this->db->get_one(['linkid' => $linkid], 'name');
            if (!empty($data) && $data['name'] == $link_title) {
                exit('1');
            }
        }
        if ($this->db->get_one(['name' => $link_title], 'linkid')) {
            exit('0');
        } else {
            exit('1');
        }
    ...
    {
        $type_name = isset($_GET['type_name']) && trim($_GET['type_name']) ? (pc_base::load_config('system', 'charset') == 'gbk' ? iconv('utf-8', 'gbk',
            trim($_GET['type_name'])) : trim($_GET['type_name'])) : exit('0');
        $type_name = safe_replace($type_name);
        $typeid = isset($_GET['typeid']) && intval($_GET['typeid']) ? intval($_GET['typeid']) : '';
        $data = [];
        if ($typeid) {
            $data = $this->db2->get_one(['typeid' => $typeid], 'name');
            if (!empty($data) && $data['name'] == $type_name) {
                exit('1');
            }
        }
        if ($this->db2->get_one(['name' => $type_name], 'typeid')) {
            exit('0');
        } else {
            exit('1');
        }
    ...
    {
        $result = $this->db->update(['listorder' => '+=1'], ['linkid' => $_GET['linkid']]);
        if ($result) {
            echo 1;
        } else {
            echo 0;
    ...
    public function listorder()
    {
        if (isset($_POST['dosubmit'])) {
            foreach ($_POST['listorders'] as $linkid => $listorder) {
                ...
                $this->db->update(['listorder' => $listorder], ['linkid' => $linkid]);
            }
        ...
        if ((!isset($_GET['typeid']) || empty($_GET['typeid'])) && (!isset($_POST['typeid']) || empty($_POST['typeid']))) {
            ...
            if (is_array($_POST['typeid'])) {
                foreach ($_POST['typeid'] as $typeid_arr) {
                    $this->db2->delete(['typeid' => $typeid_arr]);
                }
                showmessage(L('operation_success'), HTTP_REFERER);
            } else {
                ...
                $result = $this->db2->delete(['typeid' => $typeid]);
                if ($result) {
                    showmessage(L('operation_success'), HTTP_REFERER);
                } else {
    ...
    public function list_type()
    {
        $this->db2 = new type_model();
        $page = isset($_GET['page']) && intval($_GET['page']) ? intval($_GET['page']) : 1;
        $infos = $this->db2->listinfo(['module' => ROUTE_M, 'siteid' => $this->get_siteid()], $order = 'listorder DESC', $page, $pages = '10');
        $big_menu = [
            'javascript:window.top.art.dialog({id:\'add\',iframe:\'?m=link&c=link&a=add\', title:\'' . L('link_add') . '\', width:\'700\', height:\'450\'}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);',
            L('link_add'),
        ];
    ...
    public function edit()
    {
        if (isset($_POST['dosubmit'])) {
            ...
            if (!is_array($_POST['link']) || empty($_POST['link'])) {
                return false;
            }
            if ((!$_POST['link']['name']) || empty($_POST['link']['name'])) {
                return false;
            }
            $this->db->update($_POST['link'], ['linkid' => $linkid]);
            //更新附件状态
            if (pc_base::load_config('system', 'attachment_stat') & $_POST['link']['logo']) {
            ...
            //解出链接内容
            $info = $this->db->get_one(['linkid' => $_GET['linkid']]);
            if (!$info) {
                showmessage(L('link_exit'));
            }
            extract($info);
    ...
    public function edit_type()
    {
        if (isset($_POST['dosubmit'])) {
            ...
            if ((!$_POST['type']['name']) || empty($_POST['type']['name'])) {
                return false;
            }
            $this->db2->update($_POST['type'], ['typeid' => $typeid]);
            showmessage(L('operation_success'), '?m=link&c=link&a=list_type', '', 'edit');

        } else {
            $show_validator = $show_scroll = $show_header = true;
            //解出分类内容
            $info = $this->db2->get_one(['typeid' => $_GET['typeid']]);
            if (!$info) {
        ...
        if ((!isset($_GET['linkid']) || empty($_GET['linkid'])) && (!isset($_POST['linkid']) || empty($_POST['linkid']))) {
            ...
            if (is_array($_POST['linkid'])) {
                foreach ($_POST['linkid'] as $linkid_arr) {
                    //批量删除友情链接
                    $this->db->delete(['linkid' => $linkid_arr]);
                    //更新附件状态
                ...
                //删除友情链接
                $result = $this->db->delete(['linkid' => $linkid]);
                //更新附件状态
                if (pc_base::load_config('system', 'attachment_stat')) {
                    $this->attachment_db = pc_base::load_model('attachment_model');
    ...
    public function setting()
    {
        ...
        if (isset($_POST['dosubmit'])) {
            ...
            $m_db->update(['setting' => $set], ['module' => ROUTE_M]);
        ...
        if (isset($_POST['dosubmit'])) {
            if ((!isset($_GET['linkid']) || empty($_GET['linkid'])) && (!isset($_POST['linkid']) || empty($_POST['linkid']))) {
                ...
                if (is_array($_POST['linkid'])) {//批量审核
                    foreach ($_POST['linkid'] as $linkid_arr) {
                        $this->db->update(['passed' => 1], ['linkid' => $linkid_arr]);
                    }
                    showmessage(L('operation_success'), '?m=link&c=link');
                } else {//单个审核
                    $linkid = intval($_GET['linkid']);
                    ...
                    $result = $this->db->update(['passed' => 1], ['linkid' => $linkid]);
                    if ($result) {
                        showmessage(L('operation_success'), '?m=link&c=link');
                    } else {
                        showmessage(L("operation_failure"), '?m=link&c=link');
            ...
            $where = ['siteid' => $this->get_siteid(), 'passed' => 0];
            $page = isset($_GET['page']) && intval($_GET['page']) ? intval($_GET['page']) : 1;
