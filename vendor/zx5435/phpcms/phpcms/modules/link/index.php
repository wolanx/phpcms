...
class index
{
    ...
    public function register()
    {
        ...
        if (isset($_POST['dosubmit'])) {
            ...
            if (!in_array($_POST['linktype'], ['0', '1'])) {
                $_POST['linktype'] = '0';
            }
            $link_db = new link_model();
            ...
            $url = trim_script($url);
            if ($_POST['linktype'] == '0') {
                $sql = ['siteid' => $siteid, 'typeid' => intval($_POST['typeid']), 'linktype' => intval($_POST['linktype']), 'name' => $name, 'url' => $url];
            } else {
                $sql = ['siteid' => $siteid, 'typeid' => intval($_POST['typeid']), 'linktype' => intval($_POST['linktype']), 'name' => $name, 'url' => $url, 'logo' => $logo];
            }
            $link_db->insert($sql);
