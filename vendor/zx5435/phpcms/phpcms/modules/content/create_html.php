        ...
        if (isset($_POST['dosubmit'])) {
            ...
            if ($modelid) {
                ...
                if ($pages > $page) {
                    ...
                    $message = L('need_update_items', ['total' => $total, 'creatednum' => $creatednum, 'percent' => $percent]);
                    $forward = $start ? "?m=content&c=create_html&a=update_urls&type=$type&dosubmit=1&first=$first&fromid=$fromid&toid=$toid&fromdate=$fromdate&todate=$todate&pagesize=$pagesize&page=$page&pages=$pages&total=$total&modelid=$modelid" : preg_replace("/&page=([0-9]+)&pages=([0-9]+)&total=([0-9]+)/",
                        "&page=$page&pages=$pages&total=$total", $http_url);
                } else {
                ...
                if ($pages > $page) {
                    ...
                    $creatednum = $offset + count($data);
                    $percent = round($creatednum / $total, 2) * 100;
                    $message = '【' . $this->categorys[$catid]['catname'] . '】 ' . L('have_update_items', ['total' => $total, 'creatednum' => $creatednum, 'percent' => $percent]);
                    $forward = $start ? "?m=content&c=create_html&a=update_urls&type=$type&dosubmit=1&first=$first&fromid=$fromid&toid=$toid&fromdate=$fromdate&todate=$todate&pagesize=$pagesize&page=$page&pages=$pages&total=$total&autoid=$autoid&set_catid=1" : preg_replace("/&page=([0-9]+)&pages=([0-9]+)&total=([0-9]+)/",
                        "&page=$page&pages=$pages&total=$total", $http_url);
                } else {
                    $autoid++;
            ...
            if (!empty($this->categorys)) {
                foreach ($this->categorys as $catid => $r) {
                    if ($this->siteid != $r['siteid'] || ($r['type'] != 0 && $r['child'] == 0)) {
                        continue;
                    }
        ...
        if (isset($_POST['dosubmit'])) {
            ...
            if ($modelid) {
                ...
                if ($pages > $page) {
                    ...
                    $message = L('need_update_items', ['total' => $total, 'creatednum' => $creatednum, 'percent' => $percent]);
                    $forward = $start ? "?m=content&c=create_html&a=show&type=$type&dosubmit=1&first=$first&fromid=$fromid&toid=$toid&fromdate=$fromdate&todate=$todate&pagesize=$pagesize&page=$page&pages=$pages&total=$total&modelid=$modelid" : preg_replace("/&page=([0-9]+)&pages=([0-9]+)&total=([0-9]+)/",
                        "&page=$page&pages=$pages&total=$total", $http_url);
                } else {
                ...
                if ($pages > $page) {
                    ...
                    $percent = round($creatednum / $total, 2) * 100;
                    $message = '【' . $this->categorys[$catid]['catname'] . '】 ' . L('have_update_items', ['total' => $total, 'creatednum' => $creatednum, 'percent' => $percent]);
                    $forward = $start ? "?m=content&c=create_html&a=show&type=$type&dosubmit=1&first=$first&fromid=$fromid&toid=$toid&fromdate=$fromdate&todate=$todate&pagesize=$pagesize&page=$page&pages=$pages&total=$total&autoid=$autoid&set_catid=1" : preg_replace("/&page=([0-9]+)&pages=([0-9]+)&total=([0-9]+)/",
                        "&page=$page&pages=$pages&total=$total", $http_url);
                } else {
                    $autoid++;
    ...
    public function category()
    {
        if (isset($_POST['dosubmit'])) {
            ...
            if ($page <= $total_number) {
                ...
                $message = L('updating') . $this->categorys[$catid]['catname'] . L('start_to_end_id', ['page' => $page, 'endpage' => $endpage]);
