...
class sitemodel extends admin
{
    ...
    public function init()
    {
        ...
        $datas = $this->db->listinfo(['siteid' => $this->siteid, 'type' => 0], '', $_GET['page'], 30);
        //模型文章数array('模型id'=>数量);
        $items = [];
        ...
        $this->public_cache();
        $big_menu = [
            'javascript:window.top.art.dialog({id:\'add\',iframe:\'?m=content&c=sitemodel&a=add\', title:\'' . L('add_model') . '\', width:\'580\', height:\'420\', lock:true}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);',
            L('add_model'),
        ];
    ...
    public function add()
    {
        if (isset($_POST['dosubmit'])) {
            ...
            $this->type_db->insert(['name' => $_POST['info']['name'], 'module' => 'search', 'modelid' => $modelid, 'siteid' => $this->siteid]);
    ...
    public function edit()
    {
        if (isset($_POST['dosubmit'])) {
            ...
            $this->db->update($_POST['info'], ['modelid' => $modelid, 'siteid' => $this->siteid]);
            ...
            $r = $this->db->get_one(['modelid' => $modelid]);
            /** @var $admin_list_template string */
            extract($r);
    ...
    {
        $this->sitemodel_field_db = new sitemodel_field_model();
        $modelid = intval($_GET['modelid']);
        $model_cache = getcache('model', 'commons');
        $model_table = $model_cache[$modelid]['tablename'];
        $this->sitemodel_field_db->delete(['modelid' => $modelid, 'siteid' => $this->siteid]);
        $this->db->drop_table($model_table);
        $this->db->drop_table($model_table . '_data');

        $this->db->delete(['modelid' => $modelid, 'siteid' => $this->siteid]);
        //删除全站搜索接口数据
        $this->type_db = new type_model();
        $this->type_db->delete(['module' => 'search', 'modelid' => $modelid, 'siteid' => $this->siteid]);
        $cache_api = pc_base::load_app_class('cache_api', 'admin');
        $cache_api->cache('type');
        $cache_api->search_type();
        exit('1');
    ...
    {
        $modelid = intval($_GET['modelid']);
        $r = $this->db->get_one(['modelid' => $modelid, 'siteid' => $this->siteid]);

        $status = $r['disabled'] == '1' ? '0' : '1';
        $this->db->update(['disabled' => $status], ['modelid' => $modelid, 'siteid' => $this->siteid]);
    ...
    function export()
    {
        ...
        $modelinfo = $this->sitemodel_field_db->select(['modelid' => $modelid]);
    ...
    function import()
    {
        if (isset($_POST['dosubmit'])) {
                ...
                if (!empty($model_import_data)) {
                    ...
                    foreach ($model_import_data as $v) {
                        ...
                        if (in_array($field, $system_field)) {
                            ...
                            $this->sitemodel_field_db->update($v, ['modelid' => $modelid, 'field' => $field]);
                        } else {
                            ...
                            require MODEL_PATH . $field_type . DS . 'config.inc.php';
            ...
            $big_menu = [
                'javascript:window.top.art.dialog({id:\'add\',iframe:\'?m=content&c=sitemodel&a=add\', title:\'' . L('add_model') . '\', width:\'580\', height:\'400\', lock:true}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);',
                L('add_model'),
            ];
    ...
    public function cache_field($modelid = 0)
    {
        ...
        $field_array = [];
        $fields = $this->field_db->select(['modelid' => $modelid, 'disabled' => $disabled], '*', 100, 'listorder ASC');
