...
/**
 * @property type_model     $db
 * @property category_model $category_db
 * @property array          $model
 * @property array          $catids_string
...
class type_manage extends admin
{
    ...
    {
        parent::__construct();
        $this->db = new type_model();
        $this->siteid = $this->get_siteid();
        $this->model = getcache('model', 'commons');
        $this->category_db = new category_model();
    }

    public function init()
    {
        $datas = [];
        $result_datas = $this->db->listinfo(['siteid' => $this->siteid, 'module' => 'content'], 'listorder ASC,typeid DESC', $_GET['page']);
        $pages = $this->db->pages;
        ...
        $big_menu = [
            'javascript:window.top.art.dialog({id:\'add\',iframe:\'?m=content&c=type_manage&a=add\', title:\'' . L('add_type') . '\', width:\'780\', height:\'500\', lock:true}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);',
            L('add_type'),
        ];
        $this->cache();
    ...
    public function add()
    {
        if (isset($_POST['dosubmit'])) {
            ...
            if (empty($_POST['info']['name'])) {
                showmessage(L("input") . L('type_name'));
            }
            ...
            foreach ($names as $name) {
                ...
                if (!empty($ids)) {
                    foreach ($ids as $catid) {
                        $r = $this->category_db->get_one(['catid' => $catid], 'usable_type');
                        if ($r['usable_type']) {
                            $usable_type = $r['usable_type'] . $typeid . ',';
                        } else {
                            $usable_type = ',' . $typeid . ',';
                        }
                        $this->category_db->update(['usable_type' => $usable_type], ['catid' => $catid, 'siteid' => $this->siteid]);
                    }
                }
            }
    ...
    public function edit()
    {
        if (isset($_POST['dosubmit'])) {
            ...
            $this->db->update($_POST['info'], ['typeid' => $typeid]);
            $ids = $_POST['ids'];
            if (!empty($ids)) {
                foreach ($ids as $catid) {
                    $r = $this->category_db->get_one(['catid' => $catid], 'usable_type');
                    if ($r['usable_type']) {
                        ...
                        foreach ($usable_type_arr as $_usable_type_arr) {
                            if ($_usable_type_arr && $_usable_type_arr != $typeid) {
            ...
            if ($catids_string) {
                ...
                foreach ($catids_string as $catid) {
                    ...
                    $usable_type_arr = explode(',', $r['usable_type']);
                    foreach ($usable_type_arr as $_usable_type_arr) {
                        if (!$_usable_type_arr || (!in_array($catid, $ids) && $typeid == $_usable_type_arr)) {
                            continue;
                        }
                        $usable_type[] = $_usable_type_arr;
                    }
    ...
    public function delete()
    {
        ...
        foreach ($this->catids_string as $catid) {
            ...
            $usable_type_arr = explode(',', $r['usable_type']);
            foreach ($usable_type_arr as $_usable_type_arr) {
                if (!$_usable_type_arr || $typeid == $_usable_type_arr) {
                    continue;
                }
                $usable_type[] = $_usable_type_arr;
            }
        ...
        $this->db->delete(['typeid' => $_GET['typeid']]);
        $this->cache();//更新类别缓存，按站点
        exit('1');
    }
    ...
    public function listorder()
    {
        if (isset($_POST['dosubmit'])) {
            foreach ($_POST['listorders'] as $id => $listorder) {
                $this->db->update(['listorder' => $listorder], ['typeid' => $id]);
            }
            $this->cache();//更新类别缓存，按站点
    ...
    {
        $datas = [];
        $result_datas = $this->db->select(['siteid' => $this->siteid, 'module' => 'content'], '*', 1000, 'listorder ASC,typeid ASC');
    ...
    {
        $categorys = [];
        $this->categorys = $this->category_db->select(['siteid' => $this->siteid, 'module' => 'content'], '*', 10000, 'listorder ASC');
        foreach ($this->categorys as $r) {
            unset($r['module']);
