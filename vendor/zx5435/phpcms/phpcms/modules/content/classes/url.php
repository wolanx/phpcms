...
class url
{
    ...
    public function show($id, $page = 0, $catid = 0, $time = 0, $prefix = '', $data = '', $action = 'edit', $upgrade = 0)
    {
        ...
        if ($upgrade || (isset($_POST['upgrade']) && defined('IN_ADMIN') && $_POST['upgrade'])) {
            ...
            $urls = str_replace(['{$categorydir}', '{$catdir}', '{$year}', '{$month}', '{$day}', '{$catid}', '{$id}', '{$page}'],
                [$categorydir, $catdir, $year, $month, $day, $catid, $id, $page],
                $urlrule);
    ...
    public function category_url($catid, $page = 1)
    {
        ...
        if (!$setting['ishtml']) { //如果不生成静态
            ...
            $url = str_replace(['{$catid}', '{$page}'], [$catid, $page], $urlrule);
            if (strpos($url, '\\') !== false) {
            ...
            $urls = str_replace(['{$categorydir}', '{$catdir}', '{$catid}', '{$page}'], [$category_dir, $category['catdir'], $catid, $page], $urlrule);
            if ($url && $domain_dir) { //如果存在设置二级域名的情况
                if (strpos($urls, $domain_dir) === 0) {
                    $url = str_replace([$domain_dir, '\\'], [$url, '/'], $urls);
                } else {
        ...
        if (in_array(basename($url), ['index.html', 'index.htm', 'index.shtml'])) {
            $url = dirname($url) . '/';
        }
        if (strpos($url, '://') === false) {
            $url = str_replace('//', '/', $url);
    ...
    public function get_list_url($ruleid, $categorydir, $catdir, $catid, $page = 1)
    {
        ...
        $urls = str_replace(['{$categorydir}', '{$catdir}', '{$year}', '{$month}', '{$day}', '{$catid}', '{$page}'], [$categorydir, $catdir, $year, $month, $day, $catid, $page],
            $urlrule);
