...
class contentpage
{
    private $additems = []; //定义需要补全的开头html代码
    private $bottonitems = []; //定义需要补全的结尾HTML代码
    private $html_tag = []; //HTML标记数组
    ...
    {
        //定义HTML数组
        $this->html_tag = ['p', 'div', 'h', 'span', 'strong', 'ul', 'ol', 'li', 'table', 'tr', 'tbody', 'dl', 'dt', 'dd'];
        $this->html_end_tag = ['/p', '/div', '/h', '/span', '/strong', '/ul', '/ol', '/li', '/table', '/tr', '/tbody', '/dl', '/dt', '/dd'];
                ...
                if ($c) {
                    ...
                    if (in_array(strtolower($find), $this->html_tag)) {
                        ...
                        if (is_array($this->bottonitems) && !empty($this->bottonitems)) {
                            array_pop($this->bottonitems);
                        } //当属于是，将开始和结尾的补全数组取消一个
                        if (is_array($this->additems) && !empty($this->additems)) {
                            array_pop($this->additems);
                        }
                        $str = '<' . $c;
