...
class html
{
    ...
    public function show($file, $data = '', $array_merge = 1, $action = 'add', $upgrade = 0)
    {
        ...
        if (empty($previous_page)) {
            $previous_page = [
                'title' => L('first_page', '', 'content'),
                'thumb' => IMG_PATH . 'nopic_small.gif',
                'url'   => 'javascript:alert(\'' . L('first_page', '', 'content') . '\');',
            ];
        }
        if (empty($next_page)) {
            $next_page = [
                'title' => L('last_page', '', 'content'),
                'thumb' => IMG_PATH . 'nopic_small.gif',
                'url'   => 'javascript:alert(\'' . L('last_page', '', 'content') . '\');',
            ];
        }
    ...
    public function category($catid, $page = 0)
    {
        ...
        if ($create_to_html_root) {
            ...
            $file = PATH_INDEX . substr($this->html_root, 1) . $base_file;
            //添加到发布点队列
        ...
        if ($type == 0) {
            ...
            $GLOBALS['URL_ARRAY'] = ['categorydir' => $parentdir, 'catdir' => $catdir, 'catid' => $catid];
        } else {
            //单网页
    ...
    public function page($catid)
    {
        ...
        $data = $this->page_db->get_one(['catid' => $catid]);
     ...
     * @param string $copyjs 是否复制js，跨站调用评论时，需要该js
     * @return int
     */
    private function createhtml($file, $copyjs = '')
    {
        ...
        if ($copyjs && !file_exists($dir . '/js.html')) {
            @copy(PATH_PHPCMS . 'modules/content/templates/js.html', $dir . '/js.html');
        }
