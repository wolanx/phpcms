...
class push_api
{
    ...
    {
        $this->db = new content_model();  //加载数据模型
    }
     ...
     */
    public function category_list($param = [], $arr = [])
    {
        if ($arr['dosubmit']) {
            $id = $_POST['id'];
            if (empty($id)) {
            ...
            foreach ($id_arr as $id) {
                ...
                foreach ($ids as $catid) {
                    ...
                    $this->db->insert(['id' => $newid]);
                    $hitsid = 'c-' . $modelid . '-' . $newid;
                    $this->hits_db->insert(['hitsid' => $hitsid, 'catid' => $catid, 'updatetime' => SYS_TIME]);
                }
            }
            ...
            if ($_SESSION['roleid'] != 1) {
                ...
                $priv_result = $this->priv_db->select(['action' => 'add', 'roleid' => $_SESSION['roleid'], 'siteid' => $siteid, 'is_admin' => 1]);
                $priv_catids = [];
