...
class search
{
    ...
    public function init()
    {
        ...
        if (param::get_cookie('search_cookie') && param::get_cookie('search_cookie') > SYS_TIME - 2) {
            showmessage(L('search_minrefreshtime', ['min' => $minrefreshtime]), 'index.php?m=content&c=search&catid=' . $catid, $minrefreshtime * 1280);
        } else {
        ...
        foreach ($fields as $field => $r) {
            if ($r['issearch']) {
                ...
                } elseif (in_array($r['formtype'], ['text', 'keyword', 'textarea', 'editor', 'title', 'author', 'omnipotent'])) {
