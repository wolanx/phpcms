...
class sitemodel_field extends admin
{
    ...
    public function init()
    {
        ...
        $datas = $this->db->select(['modelid' => $modelid], '*', 100, 'listorder ASC');
        $r = $this->model_db->get_one(['modelid' => $modelid]);
    ...
    public function add()
    {
        if (isset($_POST['dosubmit'])) {
            ...
            require MODEL_PATH . 'fields.inc.php';
            $modelid = $_GET['modelid'];
            $f_datas = $this->db->select(['modelid' => $modelid], 'field,name', 100, 'listorder ASC');
            $m_r = $this->model_db->get_one(['modelid' => $modelid]);
    ...
    {
        $fieldid = intval($_GET['fieldid']);
        $r = $this->db->get_one(['fieldid' => $_GET['fieldid'], 'siteid' => $this->siteid]);
        //必须放在删除字段前、在删除字段部分，重置了 tablename
        $this->db->delete(['fieldid' => $_GET['fieldid'], 'siteid' => $this->siteid]);
    ...
    {
        $field_array = [];
        $fields = $this->db->select(['modelid' => $modelid, 'disabled' => $disabled], '*', 100, 'listorder ASC');
