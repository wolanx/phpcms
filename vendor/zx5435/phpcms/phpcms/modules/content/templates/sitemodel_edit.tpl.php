...
<script>
    $(function () {
        ...
        }).inputValidator({min: 1, onerror: "<?php echo L("input") . L('model_name')?>"});
    })
</script>
<div class="pad-lr-10">
    <form action="?m=content&c=sitemodel&a=edit" method="post" id="myform">
        ...
        <fieldset>
            ...
            <table width="100%" class="table_form">
                <tr>
                    ...
                    <td>
                        <?php echo form::select($style_list, $default_style, 'name="info[default_style]" id="template_list" onchange="load_file_list(this.value)"',
                            L('please_select')) ?>
                    </td>
                </tr>
                <tr>
                    ...
                    <td id="category_template">
                        <?php echo form::select_template($default_style, 'content', $category_template, 'name="setting[category_template]" id="template_category"',
                            'category') ?></td>
                </tr>
                <tr>
                    <th><?php echo L('category_list_tpl'); ?></th>
                    <td id="list_template"><?php echo form::select_template($default_style, 'content', $list_template, 'name="setting[list_template]" id="template_list"',
                            'list') ?></td>
                </tr>
                <tr>
                    <th><?php echo L('content_tpl'); ?></th>
                    <td id="show_template"><?php echo form::select_template($default_style, 'content', $show_template, 'name="setting[show_template]" id="template_show"',
                            'show') ?></td>
                </tr>
            </table>
        </fieldset>
        <div class="bk15"></div>
        <fieldset>
            <legend><?php echo L('other_template_setting') ?> <input type="checkbox"<?php if ($admin_list_template) { ?> checked<?php } ?> id="other" value="1" name="other">
            </legend>
            <table width="100%" id="other_tab" class="table_form" style="display:none;">
                ...
                <tr>
                    ...
                    <td id="member_add_template"><?php echo form::select_template($default_style, 'member', $member_add_template,
                            'name="setting[member_add_template]" id="template_member_add"', 'content_publish') ?>
                    </td>
                </tr>
            </table>
        </fieldset>
...
<script>
    function load_file_list(id) {
        $.getJSON('?m=admin&c=category&a=public_tpl_file_list&style=' + id, function (data) {
