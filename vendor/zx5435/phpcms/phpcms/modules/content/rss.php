...
use phpcms\modules\content\classes\RSSBuilder;
...
class rss
{
    ...
    public function init()
    {
        ...
        if (empty($this->rssid)) {
            ...
            if (empty($MODEL[$modelid]['tablename'])) {
                showmessage(L('missing_part_parameters'), 'blank');
            }
            ...
            foreach ($info as $r) {
                //添加项目
                if (!empty($r['thumb'])) {
                    $img = "<img src=" . thumb($r['thumb'], 150, 150) . " border='0' /><br />";
                } else {
                    $img = '';
                }
                $about = $link = (strpos($r['url'], 'http://') !== false || strpos($r['url'],
                        'https://') !== false) ? "<![CDATA[" . $r['url'] . "]]> " : (($content_html == 1) ? "<![CDATA[" . substr($sitedomain, 0,
                        -1) . $r['url'] . "]]> " : "<![CDATA[" . substr(APP_PATH, 0, -1) . $r['url'] . "]]> ");
                $title = "<![CDATA[" . $r['title'] . "]]> ";
