...
use phpcms\model\page_model;
use phpcms\modules\admin\classes\admin;
use phpcms\modules\content\fields\content_form;
use phpcms\modules\pay\classes\receipts;
...
/**
 * Class content
 * @property $page_db page_model
 * @package phpcms\modules\content
 */
class content extends admin
{
    ...
    public function __construct()
    {
        ...
        //权限判断
        if (isset($_GET['catid']) && $_SESSION['roleid'] != 1 && ROUTE_A != 'pass' && strpos(ROUTE_A,
                'public_') === false) {
            $catid = intval($_GET['catid']);
            $this->priv_db = new category_priv_model();
            $action = $this->categorys[$catid]['type'] == 0 ? ROUTE_A : 'init';
            $priv_datas = $this->priv_db->get_one(['catid' => $catid, 'is_admin' => 1, 'action' => $action]);
            if (!$priv_datas) {
    ...
    public function init()
    {
        ...
        if (isset($_GET['catid']) && $_GET['catid'] && $this->categorys[$_GET['catid']]['siteid'] == $this->siteid) {
            ...
            if (isset($_GET['keyword']) && !empty($_GET['keyword'])) {
                $type_array = ['title', 'description', 'username'];
            ...
            for ($i = 1; $i <= $workflow_steps; $i++) {
                ...
                $r = $this->db->get_one(['catid' => $catid, 'status' => $i]);
    ...
    public function add()
    {
        if (isset($_POST['dosubmit']) || isset($_POST['dosubmit_continue'])) {
            ...
            if ($category['type'] == 0) {
                ...
                if ($_POST['edit']) {
                    $this->page_db->update($_POST['info'], ['catid' => $catid]);
                } else {
    ...
    public function delete()
    {
        if (isset($_GET['dosubmit'])) {
            ...
            //判断视频模块是否安装 
            if (module_exists('video') && file_exists(PATH_PHPCMS . 'model' . DS . 'video_content_model.class.php')) {
            ...
            foreach ($_POST['ids'] as $id) {
                ...
                if ($content_ishtml && !$r['islink']) {
                    ...
                    foreach ($filelist as $delfile) {
                        ...
                        if (!in_array($lasttext, ['.htm', '.html', '.shtml'])) {
                            continue;
                        }
                        @unlink($delfile);
                        //删除发布点队列数据
                ...
                //删除统计表数据
                $this->hits_db->delete(['hitsid' => 'c-' . $modelid . '-' . $id]);
                //删除附件
                $attachment->api_delete('c-' . $catid . '-' . $id);
                //删除审核表数据
                $this->content_check_db->delete(['checkid' => 'c-' . $id . '-' . $modelid]);
                //删除推荐位数据
                $this->position_data_db->delete(['id' => $id, 'catid' => $catid, 'module' => 'content']);
                //删除全站搜索中数据
                $this->search_db->delete_search($typeid, $id);
                //删除视频库与内容对应关系数据
                if ($video_install == 1) {
                    $video_content_db->delete(['contentid' => $id, 'modelid' => $modelid]);
                }
            ...
            if ($status == 99) {
                ...
                if (isset($_POST['ids']) && !empty($_POST['ids'])) {
                    foreach ($_POST['ids'] as $id) {
                        ...
                        $memberinfo = $member_db->get_one(['username' => $content_info['username']],
                            'userid, username');
                        $flag = $catid . '_' . $id;
                        if ($setting['presentpoint'] > 0) {
                            pc_base::load_app_class('receipts', 'pay', 0);
                            receipts::point($setting['presentpoint'], $memberinfo['userid'], $memberinfo['username'],
                                $flag, 'selfincome', L('contribute_add_point'),
                                $memberinfo['username']);
                        } else {
                            pc_base::load_app_class('spend', 'pay', 0);
                            spend::point($setting['presentpoint'], L('contribute_del_point'), $memberinfo['userid'],
                                $memberinfo['username'], '', '', $flag);
                        }
                        if ($setting['content_ishtml'] == '1') {//栏目有静态配置
                            $urls = $this->url->show($id, 0, $content_info['catid'], $content_info['inputtime'], '',
                                $content_info, 'add');
    ...
    public function public_categorys()
    {
        ...
        $from = isset($_GET['from']) && in_array($_GET['from'], ['block']) ? $_GET['from'] : 'content';
        $tree = new tree();
        if ($from == 'content' && $_SESSION['roleid'] != 1) {
            $this->priv_db = pc_base::load_model('category_priv_model');
            $priv_result = $this->priv_db->select([
                'action'   => 'init',
        ...
        if (!empty($this->categorys)) {
            foreach ($this->categorys as $r) {
                if ($r['siteid'] != $this->siteid || ($r['type'] == 2 && $r['child'] == 0)) {
                    continue;
                }
    ...
    public function update_param()
    {
        ...
        if ($this->db->field_exists($field)) {
            $this->db->update([$field => $value], ['id' => $id]);
            exit('200');
        } else {
            $this->db->table_name = $this->db->table_name . '_data';
            if ($this->db->field_exists($field)) {
    ...
    public function public_relationlist()
    {
        ...
        if (!isset($_GET['modelid'])) {
            ...
            if (isset($_GET['keywords'])) {
                ...
                $field = $_GET['field'];
                if (in_array($field, ['id', 'title', 'keywords', 'description'])) {
                    if ($field == 'id') {
    ...
    public function public_getjson_ids()
    {
        ...
        $r = $this->db->get_one(['id' => $id], 'relation');
        ...
        if (isset($_POST['dosubmit'])) {
            ...
            if ($_POST['fromtype'] == 0) {
                ...
                foreach ($ids as $id) {
                    ...
                    $this->content_check_db->update(['catid' => $tocatid], ['checkid' => $checkid]);
                    $hitsid = 'c-' . $modelid . '-' . $id;
                    $this->hits_db->update(['catid' => $tocatid], ['hitsid' => $hitsid]);
                }
                $ids = implode(',', $ids);
                $this->db->set_model($modelid);
                $this->db->update(['catid' => $tocatid], "id IN($ids)");
            } else {
                if (!$_POST['fromid']) {
                    showmessage(L('please_input_move_source'));
                }
                ...
                $this->db->set_model($modelid);
                $this->db->update(['catid' => $tocatid], "catid IN($fromid)");
                $this->hits_db->update(['catid' => $tocatid], "catid IN($fromid)");
            }
            showmessage(L('operation_success'), HTTP_REFERER);
            //ids
        } else {
                        ...
                        if ($t == 'content') {
                            ...
                            foreach ($model as $modelid) {
                                ...
                                if ($r = $db->count()) { //判断模型下是否有数据
                                    $sql_file = CACHE_PATH . 'bakup' . DS . 'default' . DS . $model_arr[$modelid]['tablename'] . '.sql';
                                    $result = $data = $db->select();
                                    $this->create_sql_file($result,
                                        $db->db_tablepre . $model_arr[$modelid]['tablename'], $sql_file);
                                    $db->query('TRUNCATE TABLE `phpcms_' . $model_arr[$modelid]['tablename'] . '`');
                                    //开始清理模型data表数据
                                    $db->table_name = $db->table_name . '_data';
                                    $sql_file = CACHE_PATH . 'bakup' . DS . 'default' . DS . $model_arr[$modelid]['tablename'] . '_data.sql';
                                    $result = $db->select();
                                    $this->create_sql_file($result,
                                        $db->db_tablepre . $model_arr[$modelid]['tablename'] . '_data', $sql_file);
                                    ...
                                    if (is_array($result)) {
                                        $sql_file = CACHE_PATH . 'bakup' . DS . 'default' . DS . 'hits-' . $modelid . '.sql';
                                    ...
                                    if (is_array($result)) {
                                        $sql_file = CACHE_PATH . 'bakup' . DS . 'default' . DS . 'search-' . $modelid . '.sql';
                                    ...
                                    if (is_array($result)) {
                                        $sql_file = CACHE_PATH . 'bakup' . DS . 'default' . DS . 'position_data-' . $modelid . '.sql';
                                        $this->create_sql_file($result, $position_db->db_tablepre . 'position_data',
                                            $sql_file);
                                    }
                                    ...
                                    if (module_exists('video')) {
                                        ...
                                        if (is_array($result)) {
                                            $sql_file = CACHE_PATH . 'bakup' . DS . 'default' . DS . 'video_content-' . $modelid . '.sql';
                                            $this->create_sql_file($result,
                                                $video_content_db->db_tablepre . 'video_content', $sql_file);
                                        }
                            ...
                            $comment_db = new comment_data_model();
                            for ($i = 1; ; $i++) {
                                $comment_db->table_name($i);
                                if ($comment_db->table_exists(str_replace($comment_db->db_tablepre, '',
                                    $comment_db->table_name))) {
                                    if ($r = $comment_db->count()) {
                                        $sql_file = CACHE_PATH . 'bakup' . DS . 'default' . DS . 'comment_data_' . $i . '.sql';
                                        $result = $comment_db->select();
                                        $this->create_sql_file($result, $comment_db->db_tablepre . 'comment_data_' . $i,
                                            $sql_file);
                            ...
                            if ($r = $db->count()) {
                                $result = $db->select();
                                $sql_file = CACHE_PATH . 'bakup' . DS . 'default' . DS . $t . '.sql';
    ...
    /**
     * 备份数据到文件
     * @param array $data 备份的数据数组
     * @param string $db 数据所属数据表
     * @param string $file 备份到的文件
     * @return bool
     */
