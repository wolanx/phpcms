...
class index extends admin
{
    ...
    protected $safe = ['file_type' => 'php|js', 'code' => '', 'func' => 'com|system|exec|eval|escapeshell|cmd|passthru|base64_decode|gzuncompress', 'dir' => '', 'md5_file' => ''];
    ...
    public function init()
    {
        ...
        if (file_exists(CACHE_PATH . 'caches_scan' . DS . 'caches_data')) {
            $md5_file_list = glob(CACHE_PATH . 'caches_scan' . DS . 'caches_data' . DS . 'md5_*.php');
        ...
        foreach ($scan['dir'] as $v) {
            if (is_dir($v)) {
                foreach ($scan['file_type'] as $k) {
                    $list = array_merge($list, scan_file_lists($v . DS, 1, $k, 0, 1, 1));
                }
            } else {
                $list = array_merge($list, [str_replace(PHPCMS_PATH, '', $v) => md5_file($v)]);
            }
        }
