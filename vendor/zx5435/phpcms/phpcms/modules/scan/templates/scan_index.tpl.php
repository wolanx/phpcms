                    ...
                    <td>
                        <ul id="file" style="list-style:none; height:200px;overflow:auto;width:300px;">
                            ...
                            foreach ($list as $v) {
                                $filename = basename($v);
                                if (is_dir($v)) {
                                    $dir .= '<li><input type="checkbox" name="dir[]" value="' . $v . '" ' . (isset($scan['dir']) && is_array($scan['dir']) && !empty($scan['dir']) && in_array($v,
                                            $scan['dir']) ? 'checked' : '') . '><img src="' . IMG_PATH . 'folder.gif"> ' . $filename . '</li>';
                                } elseif (substr(strtolower($v), -3, 3) == 'php') {
                                    $file .= '<li><input type="checkbox" name="dir[]" value="' . $v . '" ' . (isset($scan['dir']) && is_array($scan['dir']) && !empty($scan['dir']) && in_array($v,
                                            $scan['dir']) ? 'checked' : '') . '><img src="' . IMG_PATH . 'file.gif">' . $filename . '</li>';
                                } else {
                                    continue;
                                }
                            }
...
<script type="text/javascript">
    function check_form() {
        var checked = 0;
        $("input[type='checkbox'][name='dir[]']").each(function (i, n) {
