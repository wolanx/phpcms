...
namespace phpcms\modules\video;
...
use phpcms\model\video_store_model;
use phpcms\modules\admin\classes\admin;
use phpcms\modules\content\classes\html;
use phpcms\modules\content\classes\url;
use phpcms\modules\video\classes\ku6api;
use phpcms\modules\video\classes\v;
 ...
 * @property video_store_model $db
 * @property module_model      $module_db
 * @property ku6api            $ku6api
 * @property v                 $v
 */
class video extends admin
{
    ...
    public function add()
    {
        if ($_POST['dosubmit']) {
            ...
            if (!$data['vid']) {
                showmessage(L('failed_you_video_uploading'), 'index.php?m=video&c=video&a=add');
            }
    ...
    public function edit()
    {
        ...
        if (isset($_POST['dosubmit'])) {
            ...
            if ($this->ku6api->vms_edit($data)) {
                ...
                if ($return) {
                    showmessage(L('operation_success'), 'index.php?m=video&c=video&a=init');
                } else {
                    showmessage(L('operation_failure'), 'index.php?m=video&c=video&a=edit&vid=' . $vid . '&menuid=' . $_GET['menuid']);
                }
            } else {
            ...
            $show_validator = true;
            $info = $this->db->get_one(['videoid' => $vid]);
    ...
    {
        $vid = $_GET['vid'];
        $r = $this->db->get_one(['videoid' => $vid], 'vid');
        if (!$r) {
            showmessage(L('video_not_exist_or_deleted'));
        }
        if (!$this->ku6api->delete_v($r['vid'])) {
            showmessage(L('operation_failure'), 'index.php?m=video&c=video&a=init&meunid=' . $_GET['meunid']);
        }
        ...
        if (isset($_GET['dosubmit'])) {
            ...
            if (is_array($ids)) {
                ...
                foreach ($ids as $videoid) {
                    ...
                    $r = $this->db->get_one(['videoid' => $videoid], 'vid');
                    if (!$this->ku6api->delete_v($r['vid'])) {
                        continue;
                    }
                    $this->v->del_video($videoid);
                    $video_content_db->delete(['videoid' => $videoid]);
                }
            }
    ...
    public function public_update_content()
    {
        ...
        $pagesize = 10;
        $result = $video_content_db->select(['videoid' => $videoid], '*', $pagesize);
        ...
        foreach ($result as $rs) {
            ...
            $video_content_db->delete(['videoid' => $videoid, 'contentid' => $contentid, 'modelid' => $modelid]);
            $content_db->set_model($modelid);
            $table_name = $content_db->table_name;
            $r1 = $content_db->get_one(['id' => $contentid]);
            if ($this->ishtml($r1['catid'])) {
                $content_db->table_name = $table_name . '_data';
                $r2 = $content_db->get_one(['id' => $contentid]);
                $r = array_merge($r1, $r2);
                unset($r1, $r2);
                if ($r['upgrade']) {
                    $urls[1] = $r['url'];
    ...
    public function setting()
    {
        if (isset($_POST['dosubmit'])) {
            ...
            $this->module_db->update(['setting' => $setting], ['module' => 'video']);
            if (!$this->ku6api->testapi()) {
    ...
    public function public_get_pos()
    {
        ...
        $info = $category->get_one(['catid' => $catid], 'modelid, arrchildid');
        if (!$info) {
            exit(0);
        }
        ...
        foreach ($position as $_key => $_value) {
            if ($_value['modelid'] && ($_value['modelid'] != $modelid) || ($_value['catid'] && strpos(',' . $info['arrchildid'] . ',', ',' . $catid . ',') === false)) {
                continue;
            }
    ...
    public function subscribe_list()
    {
        if (isset($_GET['dosubmit'])) {
            if (is_array($_GET['sub']) && !empty($_GET['sub'])) {
                $sub = $_GET['sub'];
                if (!$sub['channelid'] || !$sub['catid']) {
                    showmessage(L('please_choose_catid_and_channel'));
                }
                $sub['catid'] = intval($sub['catid']);
                $sub['posid'] = intval($sub['posid']);
                $result = $this->ku6api->subscribe($sub);
                if ($result['check'] == 6) {
                    showmessage(L('subscribe_for_default'));
                }
                if ($result['code'] == 200) {
                    showmessage(L('operation_success'), 'index.php?m=video&c=video&a=subscribe_list');
                } else {
                    showmessage(L('subscribe_set_failed'), 'index.php?m=video&c=video&a=subscribe_list&meunid=' . $_GET['meunid']);
                }
            } else {
    ...
    public function sub_del()
    {
        $id = intval($_GET['id']);
        if (!$id) {
            showmessage(L('illegal_parameters'), 'index.php?m=video&c=video&a=subscribe_list&meunid=' . $_GET['meunid']);
        }
        if ($this->ku6api->sub_del($id)) {
            showmessage(L('operation_success'), 'index.php?m=video&c=video&a=subscribe_list&meunid=' . $_GET['meunid']);
        } else {
            showmessage(L('delete_failed'), 'index.php?m=video&c=video&a=subscribe_list&meunid=' . $_GET['meunid']);
        }
    }
    ...
    public function user_sub_del()
    {
        ...
        if ($this->ku6api->user_sub_del($id)) {
            showmessage(L('operation_success'), 'index.php?m=video&c=video&a=subscribe_list&meunid=' . $_GET['meunid'] . '&type=' . $type);
        } else {
            showmessage(L('delete_failed'), 'index.php?m=video&c=video&a=subscribe_list&meunid=' . $_GET['meunid'] . '&type=' . $type);
        }
    }
    ...
    public function swfupload_json_del()
    {
        ...
        $json_str = json_encode($arr);
        $att_arr_exist = param::get_cookie('att_json');
        $att_arr_exist = str_replace([$json_str, '||||'], ['', '||'], $att_arr_exist);
    ...
    public function import_ku6video()
    {
        ...
        //视频质量
        $srctype_array = ['1' => '超清', '2' => '高清', '3' => '标清', '4' => '流畅'];
        $videotime_array = ['1' => '短视频', '2' => '普通视频', '3' => '中视频', '4' => '长视频'];
    ...
    public function get_category()
    {
        ...
        $result = $sitemodel_field->select(['formtype' => 'video', 'siteid' => $siteid], 'modelid');
        if (is_array($result)) {
            $models = '';
            foreach ($result as $r) {
                $models .= $r['modelid'] . ',';
        ...
        if ($models) {
            ...
            if (is_array($result)) {
                $data = $return_data = $categorys = [];
                $data = $return_data = $categorys = [];
                $tree = new tree();//factory::load_class('tree', 'utils');
                ...
                $str = $str2 = "<option value=\$catid \$html_disabled \$selected>\$spacer \$catname</option>";
                $tree->init($categorys);
    ...
    public function public_view_video()
    {
        $id = intval($_GET['id']);
        if (!$id) {
            showmessage('请选择要浏览的视频！');
        }
        $r = $this->db->get_one(['videoid' => $id], 'vid,channelid');
    ...
    public function public_check_status()
    {
        ...
        $r = $this->db->get_one(['videoid' => $id], 'vid,channelid,status,picpath');
        if (!$r) {
            exit('2');
        }
        ...
        if ($return['ku6status'] != $r['status']) {
            $this->db->update(['status' => $return['ku6status'], 'picpath' => $return['picpath']], ['videoid' => $id]);
            ...
            if ($return['ku6status'] == 21) {
                 ...
                 */
                $video_content_db = pc_base::load_model('video_content_model');
                $result = $video_content_db->select(['videoid' => $id]);
                if (is_array($result) && !empty($result)) {
                    //加载更新html类
                    $html = new html();
                    $content_db = new content_model();
                    $content_check_db = new content_check_model();
                    $url = new url();
                    foreach ($result as $rs) {
                        $modelid = intval($rs['modelid']);
                        $contentid = intval($rs['contentid']);
                        $content_db->set_model($modelid);
                        $c_info = $content_db->get_one(['id' => $contentid], 'thumb');

                        $where = ['status' => 99];
                        if (!$c_info['thumb']) {
                            $where['thumb'] = $return['picpath'];
                        }
                        $content_db->update($where, ['id' => $contentid]);
                        $checkid = 'c-' . $contentid . '-' . $modelid;
                        $content_check_db->delete(['checkid' => $checkid]);
                        $table_name = $content_db->table_name;
                        $r1 = $content_db->get_one(['id' => $contentid]);
                        /**
                ...
                $msg_r = json_encode(['change' => 1, 'status' => 21, 'statusname' => iconv(CHARSET, 'UTF-8', $status_arr[$return['ku6status']])]);
            } else {
                $msg_r = json_encode(['change' => 1, 'status' => $return['ku6status'], 'statusname' => iconv(CHARSET, 'UTF-8', $status_arr[$return['ku6status']])]);
            }
        } else {
            if (!$r['picpath'] && $return['picpath']) {
                $this->db->update(['picpath' => $return['picpath']], ['videoid' => $id]);
                $msg_r = json_encode(['change' => 1, 'status' => $return['ku6status'], 'statusname' => iconv(CHARSET, 'UTF-8', $status_arr[$return['ku6status']])]);
            } else {
                $msg_r = json_encode(['change' => 0]);
            }
        }
        exit($msg_r);
    }
    ...
    public function open()
    {
        ...
        if (empty($this->setting['skey']) || empty($this->setting['sn'])) {
            ...
            if ($return_check == 200 && SITE_URL != 'localhost' && !preg_match("/^(127|192|10)\.([1-2]?)([0-9]?)([0-9])\.([1-2]?)([0-9]?)([0-9])\.([1-2]?)([0-9]?)([0-9])/",
                    SITE_URL)) {//存在同域名记录，进行email验证
    ...
    public function complete_info()
    {
        if (isset($_POST['dosubmit'])) {
            ...
            if (!empty($skey_sn_array['skey']) && !empty($skey_sn_array['sn'])) {
                ...
                $complete_info = is_array($return_info) ? $return_info : [];
    ...
    public function get_skey_sn()
    {
        ...
        $this->module_db->update(['setting' => $setting], ['module' => 'video']);//更新模版数据
        //验证配置
    ...
    public function subscribe_uservideo()
    {
        if (is_array($_GET['sub']) && !empty($_GET['sub'])) {
            ...
            if ($result['code'] == 200) {
                showmessage(L('operation_success'), 'index.php?m=video&c=video&a=subscribe_list&type=2');
            } else {
                showmessage(L('subscribe_set_failed'), 'index.php?m=video&c=video&a=subscribe_list&meunid=' . $_GET['meunid']);
            }
        } else {
