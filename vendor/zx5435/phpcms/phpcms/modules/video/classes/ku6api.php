...
use phpcms\model\video_content_model;
...
class ku6api
{
     ...
     */
    private function get_tag($data = [])
    {
        if (is_array($data) && !empty($data)) {
            if ($data['keywords']) {
                $tag = trim(strip_tags($data['keywords']));
            } else {
                $tag = $data['title'];
            }
    ...
    public function update_video_status_from_vms()
    {
        if (is_array($_GET) && !empty($_GET)) {
            ...
            /* 验证vid */
            if (!$vid) {
                return json_encode(['status' => '101', 'msg' => 'vid not allowed to be empty']);
            }
            /* 验证视频大小 */
            if ($size < 100) {
                return json_encode(['status' => '103', 'msg' => 'size incorrect']);
            }
            /* 验证视频时长 */
            if ($timelen < 1) {
                return json_encode(['status' => '104', 'msg' => 'timelen incorrect']);
            }

            $db = $video_store_db = new video_store_model();
            $r = $db->get_one(['vid' => $vid]);
            if ($r) {
                $db->update(['size' => $size, 'picpath' => $picpath, 'status' => $status], ['vid' => $vid]);
                if ($status == 21) {
                    $r = $video_store_db->get_one(['vid' => $vid], 'videoid'); //取出videoid，以便下面操作
                    $videoid = $r['videoid'];
                    /**
                     * 加载视频内容对应关系数据模型，检索与删除视频相关的内容。
                     * 在对应关系表中找出对应的内容id，并更新内容的静态页
                     */
                    $video_content_db = new video_content_model();
                    $result = $video_content_db->select(['videoid' => $videoid]);
                    if (is_array($result) && !empty($result)) {
                        ...
                        foreach ($result as $rs) {
                            ...
                            $content_db->update(['status' => 99], ['id' => $contentid]);
                ...
                return json_encode(['status' => '107', 'msg' => 'Data is empty!']);
            }
        }
        json_encode(['status' => '107', 'msg' => 'Data is empty!']);
    }
     ...
     */
    public function checkusersubscribe($datas = [])
    {
     ...
     * @return bool|array
     */
    public function usersubscribe($datas = [])
    {
        //构造post数据
    ...
    public function get_album_info($albumid = 0)
    {
        ...
        $arr = ['method' => 'GetOneAlbum', 'id' => $albumid];
     ...
     * @return bool
     */
    public function add_album_subscribe($data = [])
    {
        if (!is_array($data) || empty($data)) {
            return false;
        }
     ...
     * @return bool|array
     */
    public function vms_update_video($data = [])
    {
        if (empty($data)) {
            return false;
        }
     ...
     */
    public function update_vms_member($data = [])
    {
        if (empty($data)) {
            return false;
        }
    ...
    */
    ...
    /**
     * 数据到api，post方法是post数据到api下面的v5，而post_api是post到api下面
     * @param string $url
     * @param array  $data post数据
     * @return bool|mixed
     */
    ...
    {
        if (empty($url) || !preg_match("/^(http:\/\/)?([a-z0-9\.]+)(\/api)(\/[a-z0-9\._]+)/i", $url) || empty($data)) {
            return false;
        }
        $ch = curl_init();
