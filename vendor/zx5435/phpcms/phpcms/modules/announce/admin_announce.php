...
namespace phpcms\modules\announce;
...
class admin_announce extends admin
{
    ...
    public function init()
    {
        ...
        switch ($_GET['s']) {
            case '1':
                $sql .= ' AND `passed`=\'1\' AND (`endtime` >= \'' . date('Y-m-d') . '\' or `endtime`=\'0000-00-00\')';
                break;
            case '2':
                $sql .= ' AND `passed`=\'0\'';
                break;
            case '3':
                $sql .= ' AND `passed`=\'1\' AND `endtime`!=\'0000-00-00\' AND `endtime` <\'' . date('Y-m-d') . '\' ';
                break;
        }
        $page = max(intval($_GET['page']), 1);
        $data = $this->db->listinfo($sql, '`aid` DESC', $page);
        $big_menu = [
            'javascript:window.top.art.dialog({id:\'add\',iframe:\'?m=announce&c=admin_announce&a=add\', title:\'' . L('announce_add') . '\', width:\'700\', height:\'500\', lock:true}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);',
            L('announce_add'),
        ];
    ...
    public function add()
    {
        if (isset($_POST['dosubmit'])) {
            ...
            if ($this->db->insert($_POST['announce'])) {
    ...
    public function edit()
    {
        ...
        if (isset($_POST['dosubmit'])) {
            ...
            if ($this->db->update($_POST['announce'], ['aid' => $_GET['aid']])) {
    ...
    public function public_check_title()
    {
        ...
        $title = $_GET['title'];
        if ($_GET['aid']) {
            $r = $this->db->get_one(['aid' => $_GET['aid']]);
            if ($r['title'] == $title) {
                exit('1');
            }
        }
        $r = $this->db->get_one(['siteid' => $this->get_siteid(), 'title' => $title], 'aid');
        if ($r['aid']) {
            exit('0');
        } else {
            exit('1');
    ...
    public function public_approval($aid = 0)
    {
        if ((!isset($_POST['aid']) || empty($_POST['aid'])) && !$aid) {
            ...
            if (is_array($_POST['aid']) && !$aid) {
                array_map([$this, 'public_approval'], $_POST['aid']);
                showmessage(L('announce_passed'), HTTP_REFERER);
            } elseif ($aid) {
                $aid = intval($aid);
                $this->db->update(['passed' => $_GET['passed']], ['aid' => $aid]);
    ...
    public function delete($aid = 0)
    {
        if ((!isset($_POST['aid']) || empty($_POST['aid'])) && !$aid) {
            ...
            if (is_array($_POST['aid']) && !$aid) {
                array_map([$this, 'delete'], $_POST['aid']);
                showmessage(L('announce_deleted'), HTTP_REFERER);
            } elseif ($aid) {
                $aid = intval($aid);
                $this->db->delete(['aid' => $aid]);
            }
        }
    }
    ...
    {
        if ($data['title'] == '') {
            showmessage(L('title_cannot_empty'));
        }
        if ($data['content'] == '') {
            showmessage(L('announcements_cannot_be_empty'));
        }
        $r = $this->db->get_one(['title' => $data['title']]);
