...
use phpcms\libs\classes\tree;
use phpcms\model\category_model;
use phpcms\model\category_priv_model;
use phpcms\model\content_check_model;
use phpcms\model\content_model;
use phpcms\modules\admin\classes\admin;

/**
 * @property array         $categorys
 * @property content_model $content_db
 */
class category extends admin
{
    ...
    public function init()
    {
        ...
        $types = [0 => L('category_type_system'), 1 => L('category_type_page'), 2 => L('category_type_link')];
        if (!empty($result)) {
            foreach ($result as $r) {
                ...
                $r['str_manage'] .= '<a href="?m=admin&c=category&a=edit&catid=' . $r['catid'] . '&menuid=' . $_GET['menuid'] . '&type=' . $r['type'] . '&pc_hash=' . $_SESSION['pc_hash'] . '">' . L('edit') . '</a> | <a href="javascript:confirmurl(\'?m=admin&c=category&a=delete&catid=' . $r['catid'] . '&menuid=' . $_GET['menuid'] . '\',\'' . L('confirm',
                        ['message' => addslashes($r['catname'])]) . '\')">' . L('delete') . '</a> | <a href="?m=admin&c=category&a=remove&catid=' . $r['catid'] . '&pc_hash=' . $_SESSION['pc_hash'] . '">' . L('remove',
                        '', 'content') . '</a>';
                ...
                if ($r['url']) {
                    if (preg_match('/^(http|https):\/\//', $r['url'])) {
                        ...
                        if ($r['type'] == 0 && $setting['ishtml'] && strpos($r['url'], '?') === false && substr_count($r['url'], '/') < 4) {
                            $r['help'] = '<img src="' . IMG_PATH . 'icon/help.png" title="' . L('tips_domain') . $r['url'] . '&#10;' . L('directory_binding') . '&#10;' . $catdir . '/">';
                        }
                    } else {
    ...
    public function add()
    {
        if (isset($_POST['dosubmit'])) {
            ...
            if (isset($_POST['batch_add']) && empty($_POST['batch_add'])) {
                ...
                $_POST['info']['catname'] = str_replace(['%'], '', $_POST['info']['catname']);
            ...
            if (isset($_GET['parentid'])) {
                ...
                $r = $this->db->get_one(['catid' => $parentid]);
                if ($r) {
                    extract($r, EXTR_SKIP);
                }
    ...
    public function edit()
    {
        if (isset($_POST['dosubmit'])) {
            ...
            //应用权限设置到子栏目
            if ($_POST['priv_child']) {
                $arrchildid = $this->db->get_one(['catid' => $catid], 'arrchildid');
            ...
            if ($_POST['template_child']) {
                $this->categorys = $categorys = $this->db->select(['siteid' => $this->siteid, 'module' => 'content'], '*', '', 'listorder ASC, catid ASC', '', 'catid');
                    ...
                    if (!empty($arr)) {
                        foreach ($arr as $v) {
                            $new_setting = array2string(
                                array_merge(string2array($v['setting']), [
                                        'category_template' => $_POST['setting']['category_template'],
                                        'list_template'     => $_POST['setting']['list_template'],
                                        'show_template'     => $_POST['setting']['show_template'],
                                    ]
                                )
                            );
                            $this->db->update(['setting' => $new_setting], 'catid=' . $v['catid']);
                        }
                    }
                }
            }

            $this->db->update($_POST['info'], ['catid' => $catid, 'siteid' => $this->siteid]);
            $this->update_priv($catid, $_POST['priv_roleid']);
            $this->update_priv($catid, $_POST['priv_groupid'], 0);
            $this->cache();
            //更新附件状态
            ...
            pc_base::load_sys_class('form', '', 0);
            $r = $this->db->get_one(['catid' => $catid]);
            if ($r) {
                extract($r);
            }
            $setting = string2array($setting);

            $this->priv_db = new category_priv_model();
            $this->privs = $this->priv_db->select(['catid' => $catid]);
    ...
    public function listorder()
    {
        if (isset($_POST['dosubmit'])) {
            foreach ($_POST['listorders'] as $id => $listorder) {
                $this->db->update(['listorder' => $listorder], ['catid' => $id]);
            }
            $this->cache();
    ...
    /**
     * 递归删除栏目
     * @param int $catid 要删除的栏目id
     * @param     $modelid
     * @return bool
     */
    private function delete_child($catid, $modelid = 0)
    {
        $catid = intval($catid);
        if (empty($catid)) {
            return false;
        }
        $r = $this->db->get_one(['parentid' => $catid]);
        if ($r) {
            $this->delete_child($r['catid']);
            $this->db->delete(['catid' => $r['catid']]);
            if ($modelid != 0) {
    ...
    private function delete_category_video($catid, $modelid)
    {
        ...
        $result = $content_model->select(['catid' => $catid], 'id');
    ...
    public function cache()
    {
        ...
        $models = getcache('model', 'commons');
        foreach ($models as $modelid => $model) {
            $datas = $this->db->select(['modelid' => $modelid], 'catid,type,items', 10000);
            $array = [];
            foreach ($datas as $r) {
                if ($r['type'] == 0) {
        ...
        $this->categorys = $this->db->select(['siteid' => $this->siteid, 'module' => 'content'], '*', 10000, 'listorder ASC');
        ...
        if (is_array($this->categorys)) {
            foreach ($this->categorys as $catid => $cat) {
                ...
                if ($categorys[$catid]['arrparentid'] != $arrparentid || $categorys[$catid]['arrchildid'] != $arrchildid || $categorys[$catid]['child'] != $child) {
                    $this->db->update(['arrparentid' => $arrparentid, 'arrchildid' => $arrchildid, 'child' => $child], ['catid' => $catid]);
                }
                ...
                if ($cat['url'] != $url) {
                    $this->db->update(['url' => $url], ['catid' => $catid]);
                }
                ...
                if ($categorys[$catid]['parentdir'] != $parentdir || $categorys[$catid]['sethtml'] != $sethtml || $categorys[$catid]['letter'] != $letter || $categorys[$catid]['listorder'] != $listorder) {
                    $this->db->update(['parentdir' => $parentdir, 'sethtml' => $sethtml, 'letter' => $letter, 'listorder' => $listorder], ['catid' => $catid]);
                }
            }
        }
     ...
     * @return bool
     */
    private function get_categorys($categorys = [])
    {
        ...
        if (strpos($url, '://') === false) {
            ...
            if ($setting['create_to_html_root']) {
                if (preg_match('/^((http|https):\/\/)?([^\/]+)/i', $url, $matches)) {
                    $url = $matches[0] . '/';
                    $rs = $this->db->get_one(['url' => $url], '`parentdir`,`catid`');
                    if ($catid == $rs['catid']) {
                        return '';
                    } else {
                        return $rs['parentdir'];
    ...
    public function public_check_catdir($return_method = 1, $catdir = '')
    {
        ...
        $r = $this->db->get_one(['siteid' => $this->siteid, 'module' => 'content', 'catdir' => $catdir, 'parentid' => $parentid]);
    ...
    private function update_priv($catid, $priv_datas, $is_admin = 1)
    {
        ...
        $this->priv_db->delete(['catid' => $catid, 'is_admin' => $is_admin]);
        if (is_array($priv_datas) && !empty($priv_datas)) {
            foreach ($priv_datas as $r) {
                $r = explode(',', $r);
                $action = $r[0];
                $roleid = $r[1];
                $this->priv_db->insert(['catid' => $catid, 'roleid' => $roleid, 'is_admin' => $is_admin, 'action' => $action, 'siteid' => $this->siteid]);
            }
        }
    }
    ...
    public function count_items()
    {
        ...
        foreach ($result as $r) {
            if ($r['type'] == 0) {
                ...
                $number = $this->content_db->count(['catid' => $r['catid']]);
                $this->db->update(['items' => $number], ['catid' => $r['catid']]);
            }
        }
        showmessage(L('operation_success'), HTTP_REFERER);
    }
    ...
    public function public_tpl_file_list()
    {
        ...
        if ($_GET['type'] == 1) {
            $html = [
                'page_template' => form::select_template($style, 'content',
                    (isset($cat['setting']['page_template']) && !empty($cat['setting']['page_template']) ? $cat['setting']['page_template'] : 'category'),
                    'name="setting' . $batch_str . '[page_template]"', 'page'),
            ];
        } else {
            $html = [
                'category_template' => form::select_template($style, 'content',
                    (isset($cat['setting']['category_template']) && !empty($cat['setting']['category_template']) ? $cat['setting']['category_template'] : 'category'),
                    'name="setting' . $batch_str . '[category_template]"', 'category'),
                'list_template'     => form::select_template($style, 'content',
                    (isset($cat['setting']['list_template']) && !empty($cat['setting']['list_template']) ? $cat['setting']['list_template'] : 'list'),
                    'name="setting' . $batch_str . '[list_template]"', 'list'),
                'show_template'     => form::select_template($style, 'content',
                    (isset($cat['setting']['show_template']) && !empty($cat['setting']['show_template']) ? $cat['setting']['show_template'] : 'show'),
    ...
    public function public_ajax_search()
    {
        if ($_GET['catname']) {
            if (preg_match('/([a-z]+)/i', $_GET['catname'])) {
                ...
                if (CHARSET == 'gbk') {
                    $catname = iconv('utf-8', 'gbk', $catname);
                }
            }
    ...
    public function batch_edit()
    {
        ...
        if (isset($_POST['dosubmit'])) {
            ...
            foreach ($infos as $catid => $info) {
                ...
                $this->db->update($info, ['catid' => $catid, 'siteid' => $this->siteid]);
    ...
    public function remove()
    {
        ...
        $this->content_db = new content_model();
        if (isset($_POST['dosubmit'])) {
            $this->content_check_db = new content_check_model();
            if (!$_POST['fromid']) {
                showmessage(L('please_input_move_source', '', 'content'));
            }
            if (!$_POST['tocatid']) {
                showmessage(L('please_select_target_category', '', 'content'));
            }
            $tocatid = intval($_POST['tocatid']);
            $modelid = $this->categorys[$tocatid]['modelid'];
            if (!$modelid) {
                showmessage(L('illegal_operation', '', 'content'));
            }
            $fromid = array_filter($_POST['fromid'], "is_numeric");
            $fromid = implode(',', $fromid);
            $this->content_db->set_model($modelid);
            $this->content_db->update(['catid' => $tocatid], "catid IN($fromid)");
            showmessage(L('operation_success'), HTTP_REFERER);
        } else {
            $show_header = '';
            $catid = intval($_GET['catid']);
            ...
            foreach ($this->categorys as $cid => $r) {
                if ($this->siteid != $r['siteid'] || $r['type']) {
                    continue;
                }
