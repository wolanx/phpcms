...
class site extends admin
{
    ...
    public function init()
    {
        ...
        $show_dialog = true;
        $big_menu = [
            'javascript:window.top.art.dialog({id:\'add\',iframe:\'?m=admin&c=site&a=add\', title:\'' . L('add_site') . '\', width:\'700\', height:\'500\', lock:true}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);',
            L('add_site'),
        ];
    ...
    public function del()
    {
        ...
        if ($this->db->get_one(['siteid' => $siteid])) {
            if ($this->db->delete(['siteid' => $siteid])) {
                $class_site = pc_base::load_app_class('sites');
                $class_site->set_cache();
    ...
    {
        $siteid = isset($_GET['siteid']) && intval($_GET['siteid']) ? intval($_GET['siteid']) : showmessage(L('illegal_parameters'), HTTP_REFERER);
        if ($data = $this->db->get_one(['siteid' => $siteid])) {
            if (isset($_POST['dosubmit'])) {
                $name = isset($_POST['name']) && trim($_POST['name']) ? trim($_POST['name']) : showmessage(L('site_name') . L('empty'));
                ...
                $template = isset($_POST['template']) && !empty($_POST['template']) ? $_POST['template'] : showmessage(L('please_select_a_style'));
                $default_style = isset($_POST['default_style']) && !empty($_POST['default_style']) ? $_POST['default_style'] : showmessage(L('please_choose_the_default_style'));
                if ($data['name'] != $name && $this->db->get_one(['name' => $name], 'siteid')) {
                    showmessage(L('site_name') . L('exists'));
                }
                if ($siteid != 1) {
                    if (!preg_match('/^\\w+$/i', $dirname)) {
                    ...
                    if ($data['dirname'] != $dirname && $this->db->get_one(['dirname' => $dirname], 'siteid')) {
                        showmessage(L('site_dirname') . L('exists'));
                    }
                }
                ...
                if (!empty($domain) && $data['domain'] != $domain && $this->db->get_one(['domain' => $domain], 'siteid')) {
                    showmessage(L('site_domain') . L('exists'));
                }
                if (!empty($release_point) && is_array($release_point)) {
                    if (count($release_point) > 4) {
    ...
    public function public_name()
    {
        ...
        if ($siteid) {
            ...
            $data = $this->db->get_one(['siteid' => $siteid], 'name');
    ...
    {
        $dirname = isset($_GET['dirname']) && trim($_GET['dirname']) ? (pc_base::load_config('system', 'charset') == 'gbk' ? iconv('utf-8', 'gbk',
            trim($_GET['dirname'])) : trim($_GET['dirname'])) : exit('0');
        $siteid = isset($_GET['siteid']) && intval($_GET['siteid']) ? intval($_GET['siteid']) : '';
        $data = [];
        if ($siteid) {
            $data = $this->db->get_one(['siteid' => $siteid], 'dirname');
