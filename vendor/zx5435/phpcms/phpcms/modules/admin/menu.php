...
class menu extends admin
{
    ...
    function init()
    {
        ...
        foreach ($result as $r) {
            $r['cname'] = L($r['name']);
            $r['str_manage'] = '<a href="?m=admin&c=menu&a=add&parentid=' . $r['id'] . '&menuid=' . $_GET['menuid'] . '">' . L('add_submenu') . '</a> | <a href="?m=admin&c=menu&a=edit&id=' . $r['id'] . '&menuid=' . $_GET['menuid'] . '">' . L('modify') . '</a> | <a href="javascript:confirmurl(\'?m=admin&c=menu&a=delete&id=' . $r['id'] . '&menuid=' . $_GET['menuid'] . '\',\'' . L('confirm',
                    ['message' => $r['cname']]) . '\')">' . L('delete') . '</a> ';
            $array[] = $r;
        }
    ...
    function add()
    {
        if (isset($_POST['dosubmit'])) {
            ...
            //开发过程中用于自动创建语言包
            $file = PATH_PHPCMS . 'languages' . DS . 'zh-cn' . DS . 'system_menu.lang.php';
            if (file_exists($file)) {
    ...
    private function update_menu_models($id, $old_data, $new_data)
    {
        ...
        if (is_array($models_config)) {
            foreach ($models_config as $_k => $_m) {
                if (!isset($new_data[$_k])) {
                    $new_data[$_k] = 0;
                }
                if ($old_data[$_k] == $new_data[$_k]) {
                    continue;
                } //数据没有变化时继续执行下一项
                $r = $this->db->get_one(['id' => $id], 'parentid');
                $this->db->update([$_k => $new_data[$_k]], ['id' => $id]);
    ...
    private function update_parent_menu_models($id, $field)
    {
        $id = intval($id);
        $r = $this->db->get_one(['id' => $id], 'parentid');
        $this->db->update([$field => 1], ['id' => $id]); //修改父级的所属模式，然后判断父级是否存在父级
        if ($r['parentid']) {
