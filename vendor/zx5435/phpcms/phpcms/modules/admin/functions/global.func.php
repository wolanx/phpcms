<?php 
	...
	function template_list($siteid = '', $disable = 0) {
		$list = glob(PATH_PHPCMS.'templates'.DS.'*', GLOB_ONLYDIR);
		$arr = $template = array();
		if ($siteid) {
		...
		foreach ($list as $key=>$v) {
			...
			if (file_exists($v.DS.'config.php')) {
				$arr[$key] = include $v.DS.'config.php';
	...
	function set_config($config, $filename="system") {
		$configfile = CACHE_PATH.'configs'.DS.$filename.'.php';
