...
class ipbanned extends admin
{
    ...
    function init()
    {
        ...
        $pages = $this->db->pages;
        $big_menu = [
            'javascript:window.top.art.dialog({id:\'add\',iframe:\'?m=admin&c=ipbanned&a=add\', title:\'' . L('add_ipbanned') . '\', width:\'450\', height:\'300\'}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);',
            L('add_ipbanned'),
        ];
    ...
    public function public_name()
    {
        ...
        //添加判断IP是否重复
        if ($this->db->get_one(['ip' => $ip], 'ipbannedid')) {
            exit('0');
        } else {
            exit('1');
        }
    ...
    function delete()
    {
        if (is_array($_POST['ipbannedid'])) {
            foreach ($_POST['ipbannedid'] as $ipbannedid_arr) {
                $this->db->delete(['ipbannedid' => $ipbannedid_arr]);
            }
            $this->public_cache_file();//更新缓存
            ...
            $result = $this->db->delete(['ipbannedid' => $ipbannedid]);
            $this->public_cache_file();//更新缓存
            if ($result) {
                showmessage(L('operation_success'), '?m=admin&c=ipbanned');
            } else {
