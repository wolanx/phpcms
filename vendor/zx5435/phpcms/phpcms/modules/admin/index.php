...
use phpcms\model\admin_panel_model;
 ...
 * Class index
 * @property model $db
 * @property model $menu_db
 * @property model $times_db
 * @property model $panel_db
 */
class index extends admin
{
    ...
    public function __construct()
    {
        ...
        $this->menu_db = new menu_model();
        $this->panel_db = new admin_panel_model();
    }
    ...
    public function public_ajax_add_panel()
    {
        ...
        $menuarr = $this->menu_db->get_one(['id' => $menuid]);
        $url = '?m=' . $menuarr['m'] . '&c=' . $menuarr['c'] . '&a=' . $menuarr['a'] . '&' . $menuarr['data'];
        $data = ['menuid' => $menuid, 'userid' => $_SESSION['userid'], 'name' => $menuarr['name'], 'url' => $url, 'datetime' => SYS_TIME];
        $this->panel_db->insert($data, '', 1);
        $panelarr = $this->panel_db->listinfo(['userid' => $_SESSION['userid']], "datetime");
    ...
    {
        $menuid = isset($_POST['menuid']) ? $_POST['menuid'] : exit('0');
        $this->panel_db->delete(['menuid' => $menuid, 'userid' => $_SESSION['userid']]);
    ...
    public function public_login_screenlock()
    {
        if (empty($_GET['lock_password'])) {
            showmessage(L('password_can_not_be_empty'));
        }
        //密码错误剩余重试次数
        ...
        //查询帐号
        $r = $this->db->get_one(['userid' => $_SESSION['userid']]);
    ...
    public function public_snda_status()
    {
        ...
        if (!strstr(pc_base::load_config('snda', 'snda_status'), '|')) {
            ...
            $uuid_arr = $this->site_db->get_one(['siteid' => 1], 'uuid');
            $uuid = $uuid_arr['uuid'];
            ...
            if (!isset($snda_res[err]) && !empty($snda_res['appid'])) {
                ...
                set_config(['snda_status' => $appid . '|' . $secretkey], 'snda');
            }
        }
    }
