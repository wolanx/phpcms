...
class plugin extends admin {
	...
	public function init() {
		...
		if($pluginfo = $this->db->select('','*','','disable DESC,listorder DESC')) {
			foreach ($pluginfo as $_k=>$_r) {
				if(file_exists(PATH_PHPCMS.'plugin'.DS.$_r['dir'].DS.$_r['dir'].'.class.php')){
	...
	public function import() {
		if(!isset($_GET['dir'])) {
			...
			$pluginsdir = dir(PATH_PHPCMS.'plugin');
			while (false !== ($entry = $pluginsdir->read())) {
				$config_file = '';
				$plugin_data = array();
				if(!in_array($entry, array('.', '..')) && is_dir(PATH_PHPCMS.'plugin'.DS.$entry) && !in_array($entry, $installsdir) && !$this->db->get_one(array('identification'=>$entry))) {
					$config_file = PATH_PHPCMS.'plugin'.DS.$entry.DS.'plugin_'.$entry.'.cfg.php';
			...
			$dir = trim($_GET['dir']);
			$license = 0;
			$config_file = PATH_PHPCMS.'plugin'.DS.$dir.DS.'plugin_'.$dir.'.cfg.php';
			if(file_exists($config_file)) {
				$plugin_data = @require($config_file);
	...
	public function delete() {
		if(isset($_POST['dosubmit'])) {
			...
			$filename = PATH_PHPCMS.'plugin'.DS.$dir.DS.$plugin_data['plugin']['uninstallfile'];
			...
			if($op_status) {
				...
				if($plugin_data['plugin']['iframe']) {
					...
					if(!dir_delete(PATH_PHPCMS.'plugin'.DS.$dir)) {
	...
	public function install() {
		...
		$filename = PATH_PHPCMS.'plugin'.DS.$dir.DS.$plugin_data['plugin']['installfile'];
	...
	public function design() {
	    ...
	    if(isset($_POST['dosubmit'])) {
			...
			$filepath = PATH_PHPCMS.'plugin'.DS.$data['identification'].DS.'plugin_'.$data['identification'].'.cfg.php';
			pc_base::load_sys_func('dir');
	...
	public function install_online() {
		...
		if(!empty($data['iframe'])) {
			$appdirname = PATH_PHPCMS.'plugin'.DS.$data['appenname'];
...
?>
EOF;
					$uninstallres = @file_put_contents($appdirname.DS.'uninstall.php', $installdata);
					$installres = @file_put_contents($appdirname.DS.'install.php', $installdata);
...
?>				
EOF;
					$cfgres = @file_put_contents($appdirname.DS.'plugin_'.$data['appenname'].'.cfg.php', $cfgdata);
			...
			//保存到本地地址
			$upgradezip_path = CACHE_PATH.'caches_open'.DS.$data['id'].'.zip';
			//解压路径
			$upgradezip_source_path = CACHE_PATH.'caches_open'.DS.$data['id'];
			...
			//拷贝gbk/upload文件夹到根目录
			$copy_from = $upgradezip_source_path.DS.CHARSET;
			//动态程序路径
			$copy_to_pcpath = PATH_PHPCMS.'plugin';
			//静态程序路径
			$copy_to_staticspath = PHPCMS_PATH.'statics'.DS.'plugin';

			//应用文件夹名称
			$appdirname = $data['appenname'];
	
			$this->copyfailnum = 0;
			$this->copydir($copy_from.DS.'phpcms'.DS.'plugin', $copy_to_pcpath, $_GET['cover']);
			$this->copydir($copy_from.DS.'statics'.DS.'plugin', $copy_to_staticspath, $_GET['cover']);
	...
	public function config() {
		if(isset($_POST['dosubmit'])) {
			...
			if(!isset($_GET['module'])) {	
				...
				$plugin_admin_path = PATH_PHPCMS.'plugin'.DS.$identification.DS.'plugin_admin.class.php';
	...
	function set_hook_cache() {
		if($info = $this->db->select(array('disable'=>1),'*','','listorder DESC')) {
			foreach($info as $i) {
				$id = $i['identification'];
				$hook_file = PATH_PHPCMS.'plugin'.DS.$id.DS.'hook.class.php';
