...
use phpcms\modules\admin\classes\admin_op;
use phpcms\modules\admin\classes\card;
...
class admin_manage extends admin
{
    ...
    public function add()
    {
        if (isset($_POST['dosubmit'])) {
            ...
            $roles = $this->role_db->select(['disabled' => '0']);
    ...
    public function edit()
    {
        if (isset($_POST['dosubmit'])) {
            ...
            $info = $this->db->get_one(['userid' => $_GET['userid']]);
            extract($info);
            $roles = $this->role_db->select(['disabled' => '0']);
            $show_header = true;
            $admin_manage_code = $this->get_admin_manage_code();
    ...
    public function lock()
    {
        ...
        $this->db->update(['disabled' => $disabled], ['userid' => $userid]);
        showmessage(L('operation_success'), '?m=admin&c=admin_manage');
    }
    ...
    public function public_edit_pwd()
    {
        $userid = $_SESSION['userid'];
        if (isset($_POST['dosubmit'])) {
            $r = $this->db->get_one(['userid' => $userid], 'password,encrypt');
            ...
            $info = $this->db->get_one(['userid' => $userid]);
            extract($info);
    ...
    public function public_edit_info()
    {
        ...
        if (isset($_POST['dosubmit'])) {
            ...
            $lang_dirs = glob(PATH_PHPCMS . 'languages/*');
            $dir_array = [];
            foreach ($lang_dirs as $dirs) {
                $dir_array[] = str_replace(PATH_PHPCMS . 'languages/', '', $dirs);
            }
    ...
    function public_email_ajx()
    {
        $email = $_GET['email'];
        $userid = $_SESSION['userid'];
        $check = $this->db->get_one(['email' => $email], 'userid');
    ...
    function card()
    {
        ...
        $data = [];
        if ($data = $this->db->get_one(['userid' => $userid], '`card`,`username`')) {
            $pic_url = '';
            if ($data['card']) {
    ...
    function creat_card()
    {
        ...
        if ($data = $this->db->get_one(['userid' => $userid], '`card`,`username`')) {
            if (empty($data['card'])) {
                ...
                if ($this->db->update(['card' => $card], ['userid' => $userid])) {
    ...
    function remove_card()
    {
        ...
        $userid = isset($_GET['userid']) && intval($_GET['userid']) ? intval($_GET['userid']) : showmessage(L('user_id_cannot_be_empty'));
        $data = $result = '';
        if ($data = $this->db->get_one(['userid' => $userid], '`card`,`username`,`userid`')) {
            pc_base::load_app_class('card', 'admin', 0);
            if ($result = card::remove_card($data['card'])) {
                $this->db->update(['card' => ''], ['userid' => $userid]);
