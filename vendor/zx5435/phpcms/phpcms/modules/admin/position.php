...
use phpcms\model\position_data_model;
use phpcms\model\position_model;
...
class position extends admin
{
    ...
    {
        parent::__construct();
        $this->db = new position_model();
        $this->db_data = new position_data_model();
        $this->db_content = new content_model();
    ...
    public function init()
    {
        ...
        $show_dialog = true;
        $big_menu = [
            'javascript:window.top.art.dialog({id:\'add\',iframe:\'?m=admin&c=position&a=add\', title:\'' . L('posid_add') . '\', width:\'500\', height:\'360\', lock:true}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);',
            L('posid_add'),
        ];
    ...
    public function edit()
    {
        if (isset($_POST['dosubmit'])) {
            ...
            $this->db->update($_POST['info'], ['posid' => $_POST['posid']]);
            $this->_set_cache();
            showmessage(L('operation_success'), '', '', 'edit');
        } else {
            $info = $this->db->get_one(['posid' => intval($_GET['posid'])]);
            extract($info);
            pc_base::load_sys_class('form');
            $this->sitemodel_db = pc_base::load_model('sitemodel_model');
            $sitemodel = $sitemodel = [];
    ...
    public function listorder()
    {
        if (isset($_POST['dosubmit'])) {
            foreach ($_POST['listorders'] as $posid => $listorder) {
                $this->db->update(['listorder' => $listorder], ['posid' => $posid]);
            }
            $this->_set_cache();
    ...
    public function public_item()
    {
        if (isset($_POST['dosubmit'])) {
            ...
            $page = $_GET['page'] ? $_GET['page'] : '1';
            $pos_arr = $this->db_data->listinfo(['posid' => $posid, 'siteid' => $siteid], 'listorder DESC', $page, $pagesize = 20);
            ...
            $big_menu = [
                'javascript:window.top.art.dialog({id:\'add\',iframe:\'?m=admin&c=position&a=add\', title:\'' . L('posid_add') . '\', width:\'500\', height:\'300\', lock:true}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);',
                L('posid_add'),
            ];
    ...
    public function public_item_manage()
    {
        if (isset($_POST['dosubmit'])) {
            ...
            $id = intval($_POST['id']);
            $pos_arr = $this->db_data->get_one(['id' => $id, 'posid' => $posid, 'modelid' => $modelid]);
            ...
            $array = ['data' => array2string($array), 'synedit' => intval($_POST['synedit']), 'thumb' => $thumb];
            $this->db_data->update($array, ['id' => $id, 'posid' => $posid, 'modelid' => $modelid]);
            showmessage(L('operation_success'), '', '', 'edit');
        } else {
            $posid = intval($_GET['posid']);
            $modelid = intval($_GET['modelid']);
            $id = intval($_GET['id']);
            if ($posid == 0 || $modelid == 0) {
    ...
    public function public_item_listorder()
    {
        if (isset($_POST['posid'])) {
            foreach ($_POST['listorders'] as $_k => $listorder) {
                $pos = [];
                $pos = explode('-', $_k);
                $this->db_data->update(['listorder' => $listorder], ['id' => $pos[1], 'catid' => $pos[0], 'posid' => $_POST['posid']]);
            }
            showmessage(L('operation_success'), HTTP_REFERER);
    ...
    private function content_pos($id, $modelid)
    {
        ...
        $posids = $this->db_data->get_one(['id' => $id, 'modelid' => $modelid]) ? 1 : 0;

        return $this->db_content->update(['posids' => $posids], ['id' => $id]);
    }
