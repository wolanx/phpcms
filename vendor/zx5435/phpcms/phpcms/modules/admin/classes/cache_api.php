...
use phpcms\model\cache_model;
use phpcms\model\dbsource_model;
use phpcms\model\module_model;
use phpcms\model\sitemodel_field_model;
...
class cache_api
{
    ...
    {
        if (file_exists(PATH_PHPCMS . 'model' . DS . $model . '_model.php')) {
    ...
    {
        $infos = $this->db->select(['keyid' => 0]);
        foreach ($infos as $r) {
            $linkageid = intval($r['linkageid']);
            $r = $this->db->get_one(['linkageid' => $linkageid], 'name,siteid,style');
            $info['title'] = $r['name'];
            $info['style'] = $r['style'];
    ...
    {
        $infos = $this->db->select(['disabled' => '0'], $data = '`roleid`,`rolename`', '', 'roleid ASC');
        foreach ($infos as $info) {
    ...
    public function cache_siteid($role)
    {
        ...
        $sitelist = [];
        foreach ($role as $n => $r) {
            $sitelists = $priv_db->select(['roleid' => $n], 'siteid', '', 'siteid');
    ...
    {
        $modules = [];
        $modules = $this->db->select(['disabled' => 0], '*', '', '', '', 'module');
    ...
    {
        define('MODEL_PATH', PATH_PHPCMS . 'modules' . DS . 'content' . DS . 'fields' . DS);
    ...
    {
        $field_array = [];
        $db = new sitemodel_field_model();
        $fields = $db->select(['modelid' => $modelid, 'disabled' => 0], '*', 100, 'listorder ASC');
    ...
    {
        $datas = [];
        $result_datas = $this->db->select(['siteid' => get_siteid(), 'module' => $param], '*', 1000, 'listorder ASC,typeid ASC');
    ...
    {
        $datas = [];
        $workflow_datas = $this->db->select(['siteid' => get_siteid()], '*', 1000);
    ...
    {
        $db = new dbsource_model();
        $list = $db->select();
        $data = [];
        if ($list) {
            foreach ($list as $val) {
                $data[$val['name']] = [
                    'hostname'    => $val['host'] . ':' . $val['port'],
                    'database'    => $val['dbname'],
                    'db_tablepre' => $val['dbtablepre'],
    ...
    {
        define('MEMBER_MODEL_PATH', PATH_PHPCMS . 'modules' . DS . 'member' . DS . 'fields' . DS);
        //模型缓存路径
        define('MEMBER_CACHE_MODEL_PATH', PATH_CACHES . 'caches_model' . DS . 'caches_data' . DS);
        ...
        foreach ($classtypes as $classtype) {
            ...
            $cache_data = str_replace('}?>', '', $cache_data);
            foreach ($fields as $field => $fieldvalue) {
                if (file_exists(MEMBER_MODEL_PATH . $field . DS . $classtype . '.inc.php')) {
                    $cache_data .= file_get_contents(MEMBER_MODEL_PATH . $field . DS . $classtype . '.inc.php');
                }
            }
            $cache_data .= "\r\n } \r\n?>";
    ...
    public function member_model_field()
    {
        ...
        $this->db = new sitemodel_field_model();
        foreach ($member_model as $modelid => $m) {
            $field_array = [];
            $fields = $this->db->select(['modelid' => $modelid, 'disabled' => 0], '*', 100, 'listorder ASC');
            foreach ($fields as $_value) {
                $setting = string2array($_value['setting']);
                $_value = array_merge($_value, $setting);
                $field_array[$_value['field']] = $_value;
    ...
    {
        $this->db = new module_model();
        $setting = $this->db->get_one(['module' => 'search'], 'setting');
    ...
    public function search_type()
    {
        ...
        foreach ($sitelist as $siteid => $_v) {
            $datas = $search_model = [];
            $result_datas = $result_datas2 = $this->db->select(['siteid' => $siteid, 'module' => 'search'], '*', 1000, 'listorder ASC');
            foreach ($result_datas as $_key => $_value) {
                if (!$_value['modelid']) {
                    continue;
                }
    ...
    public function database()
    {
        ...
        if (is_array($M)) {
            foreach ($M as $key => $m) {
                if (file_exists(PATH_PHPCMS . 'modules' . DS . $key . DS . 'classes' . DS . $key . '_tag.class.php') && !in_array($key,
                        ['message', 'block'])) {
                    $module[$key] = $m['name'];
                }
            }
...
";
        ...
        return $file_size = file_put_contents($filepath . 'modules.php', $module);
    }
    ...
    public function cache2database()
    {
        ...
        $result = $cache->select([], '*', $offset . ', 20', 'filename ASC');
        if (is_array($result) && !empty($result)) {
            foreach ($result as $re) {
                if (!file_exists(CACHE_PATH . $re['path'] . $re['filename'])) {
                    $filesize = pc_base::load_config('system', 'lock_ex') ? file_put_contents(CACHE_PATH . $re['path'] . $re['filename'], $re['data'],
                        LOCK_EX) : file_put_contents(CACHE_PATH . $re['path'] . $re['filename'], $re['data']);
                } else {
                    continue;
                }
            }
    ...
    {
        $path = PATH_CACHES . 'caches_template' . DS;
        $files = glob($path . '*');
        pc_base::load_sys_func('dir');
        if (is_array($files)) {
            foreach ($files as $f) {
                $dir = basename($f);
                if (!in_array($dir, ['block', 'dbsource'])) {
                    dir_delete($path . $dir);
                }
            }
        }
        $path = PATH_CACHES . 'caches_tpl_data' . DS . 'caches_data' . DS;
        $files = glob($path . '*');
        if (is_array($files)) {
            foreach ($files as $f) {
                $dir = basename($f);
