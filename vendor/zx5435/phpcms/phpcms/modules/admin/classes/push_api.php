...
/**
 * 推荐至推荐位接口类
 * @property $db_content content_model
 */
class push_api
{
    ...
    private function position_del($catid, $id, $input_posid)
    {
        ...
        //查找已存在推荐位
        $r = $pos_data->select(['id' => $id, 'catid' => $catid], 'posid');
        if (!$r) {
            return false;
        }
        foreach ($r as $v) {
    ...
    private function content_pos($id, $modelid)
    {
        ...
        if ($id && $modelid) {
            ...
            $posids = $db_data->get_one(['id' => $id, 'modelid' => $modelid]) ? 1 : 0;
            if ($posids == 0) {
                $this->db_content->update(['posids' => $posids], ['id' => $id]);
            }
        }
     ...
     */
    public function position_list($param = [], $arr = [], $expiration = 0, $model = 'content_model')
    {
        if ($arr['dosubmit']) {
            if (!$model) {
                $model = 'content_model';
            ...
            if (is_array($arr['posid']) && !empty($arr['posid']) && is_array($param) && !empty($param)) {
                foreach ($arr['posid'] as $pid) {
                    $ext = $func_char = '';
                    $r = $this->db->get_one(['posid' => $pid], 'extention'); //检查推荐位是否启用了扩展字段
                    ...
                    foreach ($param as $d) {
                        ...
                        if ($r = $pos_data->get_one(['id' => $d['id'], 'posid' => $pid, 'catid' => $info['catid']])) {
                            if ($r['synedit'] == '0') {
                                $pos_data->update($info,
                                    ['id' => $d['id'], 'posid' => $pid, 'catid' => $info['catid']]);
                            }
                        } else {
                            $pos_data->insert($info);
                        }
                        $db->update(['posids' => 1], ['id' => $d['id']]);
                        unset($info);
                    }
                    $maxnum = $position_info[$pid]['maxnum'] + 4;
                    $r = $pos_data->select(['catid' => $catid, 'posid' => $pid], 'id, listorder', $maxnum . ',1',
                    ...
                    if ($r && $position_info[$pid]['maxnum']) {
                        ...
                        foreach ($result as $r) {
                            $pos_data->delete(['id' => $r['id'], 'posid' => $pid, 'catid' => $catid]);
            ...
            if (!empty($positions)) {
                foreach ($positions as $pid => $p) {
                    ...
                    //获取栏目下所有子栏目
                    if ($p['catid']) {
                        $catids = explode(',', $category[$p['catid']]['arrchildid']);
                    }
                    if (($p['siteid'] == 0 || $p['siteid'] == $siteid) && ($p['modelid'] == 0 || $p['modelid'] == $param['modelid']) && ($p['catid'] == 0 || in_array($param['catid'],
                                $catids))) {
                        $info[$pid] = $p['name'];
                    }
                }
