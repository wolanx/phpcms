...
use pc_base;
use phpcms\libs\classes\db_factory;
...
class database extends admin
{
    ...
    public function export()
    {
        ...
        if ($dosubmit) {
            if ($_GET['pdo_select'] == '' && $_POST['pdo_select'] == '') {
                showmessage(L('select_pdo'));
            }
    ...
    public function public_down()
    {
        ...
        file_down(CACHE_PATH . 'bakup' . DS . $datadir . DS . $filename);
    }
    ...
    public function delete()
    {
        ...
        $bakfile_path = CACHE_PATH . 'bakup' . DS . $pdo_name . DS;
        if ($filenames) {
    ...
    private function status($tables, $tablepre)
    {
        ...
        foreach ($tables as $table) {
            ...
            $row = [
                'name'      => $name,
                'rows'      => $table['Rows'],
                'size'      => $table['Data_length'] + $row['Index_length'],
                'engine'    => $table['Engine'],
        ...
        return ['phpcmstables' => $phpcms, 'othertables' => $other];
    }
    ...
    private function export_database($tables, $sqlcompat, $sqlcharset, $sizelimit, $action, $fileid, $random, $tableid, $startfrom, $tabletype)
    {
        ...
        if (trim($tabledump)) {
            ...
            $altid = $fileid;
            $fileid++;
            $bakfile_path = CACHE_PATH . 'bakup' . DS . $this->pdo_name;
            if (!dir_create($bakfile_path)) {
                showmessage(L('dir_not_be_created'));
            }
            $bakfile = $bakfile_path . DS . $filename;
            ...
            showmessage(L('bakup_file') . " $filename " . L('bakup_write_succ'),
                '?m=admin&c=database&a=export&sizelimit=' . $sizelimit . '&sqlcompat=' . $sqlcompat . '&sqlcharset=' . $sqlcharset . '&tableid=' . $tableid . '&fileid=' . $fileid . '&startfrom=' . $startrow . '&random=' . $random . '&dosubmit=1&tabletype=' . $tabletype . '&allow=' . $allow . '&pdo_select=' . $this->pdo_name);
        } else {
            $bakfile_path = CACHE_PATH . 'bakup' . DS . $this->pdo_name . DS;
    ...
    private function import_database($filename)
    {
        if ($filename && fileext($filename) == 'sql') {
            $filepath = CACHE_PATH . 'bakup' . DS . $this->pdo_name . DS . $filename;
            ...
            if (file_exists($filepath)) {
                ...
                $fileid++;
                showmessage(L('bakup_data_file') . " $filename " . L('load_success'),
                    "?m=admin&c=database&a=import&pdoname=" . $this->pdo_name . "&pre=" . $pre . "&fileid=" . $fileid . "&dosubmit=1");
            } else {
