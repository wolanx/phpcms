...
use phpcms\modules\admin\classes\role_cat;
use phpcms\modules\admin\classes\role_op;
...
class role extends admin
{
    ...
    public function edit()
    {
        if (isset($_POST['dosubmit'])) {
            ...
            $this->db->update($_POST['info'], ['roleid' => $_POST['roleid']]);
            $this->_cache();
            showmessage(L('operation_success'), '?m=admin&c=role');
        } else {
            $info = $this->db->get_one(['roleid' => $_GET['roleid']]);
            extract($info);
    ...
    public function delete()
    {
        ...
        $this->db->delete(['roleid' => $roleid]);
        $this->priv_db->delete(['roleid' => $roleid]);
        $this->_cache();
    ...
    public function listorder()
    {
        if (isset($_POST['dosubmit'])) {
            foreach ($_POST['listorders'] as $roleid => $listorder) {
                $this->db->update(['listorder' => $listorder], ['roleid' => $roleid]);
            }
    ...
    public function role_priv()
    {
        ...
        if (isset($_POST['dosubmit'])) {
            if (is_array($_POST['menuid']) && count($_POST['menuid']) > 0) {
                ...
                $this->priv_db->delete(['roleid' => $_POST['roleid'], 'siteid' => $_POST['siteid']]);
    ...
    public function change_status()
    {
        ...
        $this->db->update(['disabled' => $disabled], ['roleid' => $roleid]);
        $this->_cache();
        showmessage(L('operation_success'), '?m=admin&c=role');
    }
    ...
    public function member_manage()
    {
        ...
        $infos = $this->admin_db->select(['roleid' => $roleid]);
    ...
    private function _cache_siteid($role)
    {
        $sitelist = [];
        foreach ($role as $n => $r) {
            $sitelists = $this->priv_db->select(['roleid' => $n], 'siteid', '', 'siteid');
