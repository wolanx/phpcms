...
class googlesitemap extends admin
{
    ...
    function set()
    {
        ...
        if ($siteid == 1) {
            ...
            $dir = PHPCMS_PATH . $html_root . DS . $sitecache[$siteid]['dirname'] . DS;
        }
        //模型缓存
        ...
        if ($dosubmit) {
            ...
            if ($_POST['mark']) {
                ...
                foreach ($catids as $catid) {
                    ...
                    $result = $this->content_db->select(['catid' => $catid, 'status' => 99], '*', $limit = "0,$baidunum", 'id desc');
                    ...
                    foreach ($result as $arr) {
                        ...
                        $content_arr = $this->content_db->get_one(['id' => $id], 'content');
            ...
            foreach ($new_model as $modelid => $m) {//每个模块取出num条数据
                ...
                $result = $this->content_db->select(['status' => 99], '*', $limit = "0,$num", $order = 'inputtime desc');
                foreach ($result as $arr) {
                    ...
                    $hit_r = $hits_db->get_one(['hitsid' => 'c-' . $modelid . '-' . $arr['id']]);
                    if ($hit_r['views'] > 1000) {
