...
class release_point extends admin
{
    ...
    public function init()
    {
        $list = $this->db->select();
        $big_menu = [
            'javascript:window.top.art.dialog({id:\'add\',iframe:\'?m=admin&c=release_point&a=add\', title:\'' . L('release_point_add') . '\', width:\'700\', height:\'500\', lock:true}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);',
            L('release_point_add'),
        ];
    ...
    public function edit()
    {
        ...
        if ($data = $this->db->get_one(['id' => $id])) {
            if (isset($_POST['dosubmit'])) {
                ...
                $pasv = isset($_POST['pasv']) && trim($_POST['pasv']) ? trim($_POST['pasv']) : 0;
                $ssl = isset($_POST['ssl']) && trim($_POST['ssl']) ? trim($_POST['ssl']) : 0;
                if ($data['name'] != $name && $this->db->get_one(["name" => $name])) {
                    showmessage(L('release_point_name') . L('exists'));
                }
                if ($this->db->update([
                    'name'     => $name,
