        ...
        <form method="post" name="myform" id="myform" action="?m=admin&c=database&a=export">
            ...
            <table width="100%" cellspacing="0">
                ...
                <tr>
                    ...
                    <td colspan=3><input type="radio" name="sqlcompat" value="" checked> <?php echo L('default') ?> &nbsp; <input type="radio" name="sqlcompat" value="MYSQL40"> MySQL 3.23/4.0.x &nbsp;
                        <input type="radio" name="sqlcompat" value="MYSQL41"> MySQL 4.1.x/5.x &nbsp;
                    </td>
                </tr>
                <tr>
                    <td class="align_r"><?php echo L('sqlcharset') ?></td>
                    <td colspan=3><input type="radio" name="sqlcharset" value="" checked> <?php echo L('default') ?>&nbsp; <input type="radio" name="sqlcharset" value="latin1"> LATIN1 &nbsp; <input
                                type="radio" name="sqlcharset" value='utf8'> UTF-8</option></td>
                </tr>
                <tr>
            ...
            <table width="100%" cellspacing="0">
                ...
                if (is_array($infos)) {
                    ...
                    <thead>
                    <tr>
                        <th align="center" colspan="8"><strong><?php echo $pdo_name ?><?php echo L('pdo_name') ?></strong></th>
                    </tr>
                    </thead>
                    <thead>
                    <tr>
                        <th width="10%"><input type="checkbox" value="" id="check_box" onclick="selectall('tables[]');"> <a class="cu" href="javascript:void(0);"
                                                                                                                            onclick="reselect()"><?php echo L('reselect') ?></a></th>
                    ...
                    <?php foreach ($infos['phpcmstables'] as $v) { ?>
                        <tr>
                            ...
                            <td width="15%" align="center"><a
                                        href="?m=admin&c=database&a=public_repair&operation=optimize&pdo_name=<?php echo $pdo_name ?>&tables=<?php echo $v['name'] ?>"><?php echo L('database_optimize') ?></a>
                                | <a href="?m=admin&c=database&a=public_repair&operation=repair&pdo_name=<?php echo $pdo_name ?>&tables=<?php echo $v['name'] ?>"><?php echo L('database_repair') ?></a>
                                | <a href="javascript:void(0);" onclick="showcreat('<?php echo $v['name'] ?>','<?php echo $pdo_name ?>')"><?php echo L('database_showcreat') ?></a></td>
                        </tr>
                    <?php } ?>
                    </tbody>
                    <?php
            ...
            if (is_array($infos)) {
                ...
                <div class="btn">
                    ...
                    <input type="submit" class="button" name="dosubmit" onclick="document.myform.action='?m=admin&c=database&a=public_repair&operation=optimize&pdo_name=<?php echo $pdo_name ?>'"
                           value="<?php echo L('batch_optimize') ?>"/>
                    <input type="submit" class="button" name="dosubmit" onclick="document.myform.action='?m=admin&c=database&a=public_repair&operation=repair&pdo_name=<?php echo $pdo_name ?>'"
                           value="<?php echo L('batch_repair') ?>"/>
                </div>
                <?php
            }
            ?>
...
<script type="text/javascript">
    ...
    function showcreat(tblname, pdo_name) {
        window.top.art.dialog({
            title: tblname,
            id: 'show',
            iframe: '?m=admin&c=database&a=public_repair&operation=showcreat&pdo_name=' + pdo_name + '&tables=' + tblname,
            width: '500px',
            height: '350px'
        });
    }
