...
<head>
    ...
    <title><?= L('admin_site_title') ?></title>
    ...
    <link rel="stylesheet" href="<?php echo CSS_PATH ?>style/<?php echo SYS_STYLE; ?>-styles1.css">
    <link rel="stylesheet" href="<?= CDN_PATH ?>admin/css/index_init.css">
    ...
    <script>var pc_hash = '<?php echo $_SESSION['pc_hash']?>';</script>
</head>
...
<div id="dvLockScreen" class="ScreenLock"
     style="display:<?php if (isset($_SESSION['lock_screen']) && $_SESSION['lock_screen'] == 0) {
         echo 'none';
     } ?>">
    <div id="dvLockScreenWin" class="inputpwd">
        <h5><b class="ico ico-info"></b><span id="lock_tips"><?php echo L('lockscreen_status'); ?></span></h5>
        <div class="input">
            <label class="lb"><?php echo L('password') ?>：</label><input type="password" id="lock_password"
                                                                         class="input-text" size="24">
...
<div class="header">
    <div class="logo lf"><a href="<?php echo $currentsite['domain'] ?>" target="_blank"><span
                    class="invisible"><?php echo L('phpcms_title') ?></span></a></div>
    <div class="rt-col">
        <div class="tab_style white cut_line text-r">
            <a href="javascript:void(0);" onclick="lock_screen()"><img
                        src="<?php echo IMG_PATH . 'icon/lockscreen.png' ?>"> <?php echo L('lockscreen') ?></a>
            <span>|</span>
    ...
    <div class="col-auto">
        <div class="log white cut_line"><?php echo L('hello'), $admin_username ?> [<?php echo $rolename ?>
                    ...
                    href="?m=admin&c=index&a=public_logout">[<?php echo L('exit') ?>
                ]</a><span>|</span>
            <a href="<?php echo $currentsite['domain'] ?>" target="_blank"
               id="site_homepage"><?php echo L('site_homepage') ?></a><span>|</span>
        ...
        <ul class="nav white" id="top_menu">
            ...
            ?>
            <li class="tab_web"><a href="javascript:void(0);"><span><?php echo $currentsite['name'] ?></span></a></li>
        </ul>
    </div>
</div>
<div id="content">
    <div class="col-left left_menu">
        ...
        <a href="javascript:void(0);" id="openClose"
           style="outline-style: none; outline-color: invert; outline-width: medium;" hideFocus="hidefocus" class="open"
           title="<?php echo L('spread_or_closed') ?>"><span
                    class="hidden"><?php echo L('expand') ?></span></a>
    </div>
    ...
    <div class="col-auto mr8">
        <div class="crumbs">
            <div class="shortcut cu-span"><a
                        href="?m=content&c=create_html&a=public_index&pc_hash=<?php echo $_SESSION['pc_hash']; ?>"
                        target="right"><span><?php echo L('create_index') ?></span></a><a
                        href="?m=admin&c=cache_all&a=init&pc_hash=<?php echo $_SESSION['pc_hash']; ?>"
                        target="right"><span><?php echo L('update_backup') ?></span></a><a
                        href="javascript:art.dialog({id:'map',iframe:'?m=admin&c=index&a=public_map', title:'<?php echo L('background_map') ?>', width:'700', height:'500', lock:true});void(0);"><span><?php echo L('background_map') ?></span></a><?php echo runhook('admin_top_left_menu') ?>
            </div>
            <?php echo L('current_position') ?><span id="current_pos"></span></div>
        <div class="col-1">
            <div class="content" style="position:relative; overflow:hidden">
                <iframe name="right" id="rightMain" src="?m=admin&c=index&a=public_main" frameborder="false"
                        scrolling="auto" style="border:none; margin-bottom:30px" width="100%"
                        height="auto"
                        allowtransparency="true"></iframe>
                <div class="fav-nav">
                    <div id="panellist">
                        <?php foreach ($adminpanel as $v) { ?>
                            <span>
								<a onclick="paneladdclass(this);" target="right"
                                   href="<?php echo $v['url'] . 'menuid=' . $v['menuid'] . '&pc_hash=' . $_SESSION['pc_hash']; ?>"><?php echo L($v['name']) ?></a>
								<a class="panel-delete"
                                   href="javascript:delete_panel('<?= $v['menuid'] ?>', this);"></a></span>
                        <?php } ?>
                    </div>
                    <div id="paneladd"></div>
...
<div class="tab-web-panel hidden" style="position:absolute; z-index:999; background:#fff">
    <ul>
        <?php foreach ($sitelist as $key => $v): ?>
            <li style="margin:0"><a
                        href="javascript:site_select('<?= $v['siteid'] ?>', '<?php echo new_addslashes($v['name']) ?>', '<?php echo $v['domain'] ?>', '<?php echo $v['siteid'] ?>')"><?php echo $v['name'] ?></a>
            </li>
        <?php endforeach; ?>
    </ul>
</div>
<div class="scroll"><a href="javascript:void(0);" class="per" title="使用鼠标滚轴滚动侧栏" onclick="menuScroll(1);"></a><a
            href="javascript:void(0);" class="next" title="使用鼠标滚轴滚动侧栏"
            onclick="menuScroll(2);"></a></div>
<script type="text/javascript">
    ...
    function _MP(menuid, targetUrl) {
        console.info(location.origin + targetUrl + '&menuid=' + menuid + '&pc_hash=' + pc_hash);
        $("#menuid").val(menuid);
        $("#paneladd").html('<a class="panel-add" href="javascript:add_panel();"><em><?php echo L('add')?></em></a>');
