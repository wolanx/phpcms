...
use phpcms\model\linkage_model;
...
/**
 * @property linkage_model $db
 * @property sites $sites
 * @property array $childnode
 */
class linkage extends admin
{
    ...
    public function init()
    {
        ...
        $big_menu = [
            'javascript:window.top.art.dialog({id:\'add\',iframe:\'?m=admin&c=linkage&a=add\', title:\'' . L('linkage_add') . '\', width:\'500\', height:\'220\', lock:true}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);',
            L('linkage_add'),
        ];
    ...
    public function edit()
    {
        if (isset($_POST['dosubmit'])) {
            ...
            $info['style'] = trim(intval($_POST['info']['style']));
            $info['siteid'] = trim(intval($_POST['info']['siteid']));
            $info['setting'] = array2string(['level' => intval($_POST['info']['level'])]);
            if ($_POST['info']['keyid']) {
            ...
            $this->db->update($info, ['linkageid' => $linkageid]);
            $id = $info['keyid'] ? $info['keyid'] : $linkageid;
            showmessage(L('operation_success'), '', '', 'edit');
        } else {
            $linkageid = intval($_GET['linkageid']);
            $info = $this->db->get_one(['linkageid' => $linkageid]);
            extract($info);
    ...
    public function delete()
    {
        ...
        if (is_array($this->childnode)) {
            foreach ($this->childnode as $linkageid_tmp) {
                $this->db->delete(['linkageid' => $linkageid_tmp]);
            }
        }
        $this->db->delete(['keyid' => $linkageid]);
    ...
    {
        if (!is_array($_POST['listorders'])) {
            return false;
        }
        foreach ($_POST['listorders'] as $linkageid => $value) {
            $value = intval($value);
            $this->db->update(['listorder' => $value], ['linkageid' => $linkageid]);
        }
    ...
    public function public_manage_submenu()
    {
        ...
        $tree->icon = ['&nbsp;&nbsp;&nbsp;│ ', '&nbsp;&nbsp;&nbsp;├─ ', '&nbsp;&nbsp;&nbsp;└─ '];
        $tree->nbsp = '&nbsp;&nbsp;&nbsp;';
        $sum = $this->db->count(['keyid' => $keyid]);
        $sql_parentid = $_GET['parentid'] ? trim($_GET['parentid']) : 0;
        $where = $sum > 40 ? ['keyid' => $keyid, 'parentid' => $sql_parentid] : ['keyid' => $keyid];
        $result = $this->db->select($where, '*', '', 'listorder ,linkageid');

        foreach ($result as $areaid => $area) {
            $areas[$area['linkageid']] = [
                'id'          => $area['linkageid'],
                'parentid'    => $area['parentid'],
                'name'        => $area['name'],
                'listorder'   => $area['listorder'],
                'style'       => $area['style'],
                'mod'         => $mod,
                'file'        => $file,
                'keyid'       => $keyid,
            ...
            $areas[$area['linkageid']]['str_manage'] = ($sum > 40 && $this->_is_last_node($area['keyid'],
                    $area['linkageid'])) ? '<a href="?m=admin&c=linkage&a=public_manage_submenu&keyid=' . $area['keyid'] . '&parentid=' . $area['linkageid'] . '">' . L('linkage_manage_submenu') . '</a> | ' : '';
        ...
        $big_menu = [
            'javascript:window.top.art.dialog({id:\'add\',iframe:\'?m=admin&c=linkage&a=public_sub_add&keyid=' . $keyid . '\', title:\'' . L('linkage_add') . '\', width:\'500\', height:\'430\', lock:true}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);',
            L('linkage_add'),
        ];
    ...
    private function _cache($linkageid)
    {
        ...
        $info = [];
        $r = $this->db->get_one(['linkageid' => $linkageid], 'name,siteid,style,keyid,setting');
        $info['title'] = $r['name'];
        $info['style'] = $r['style'];
    ...
    private function submenulist($keyid = 0)
    {
        ...
        if (is_array($result)) {
            foreach ($result as $r) {
                ...
                $this->db->update(['child' => $child, 'arrchildid' => $arrchildid], ['linkageid' => $r['linkageid']]);
                $datas[$r['linkageid']] = $r;
            }
        }
    ...
    {
        $keyid = intval($keyid);
        $info = $this->db->get_one(['linkageid' => $keyid]);
    ...
    {
        $result = $this->db->count(['keyid' => $keyid, 'parentid' => $linkageid]);
