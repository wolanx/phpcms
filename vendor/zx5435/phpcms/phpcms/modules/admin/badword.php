...
use phpcms\model\badword_model;
use phpcms\modules\admin\classes\admin;

/**
 * @property badword_model $db
 */
class badword extends admin
{
    ...
    function init()
    {
        ...
        $pages = $this->db->pages;
        $level = [1 => L('general'), 2 => L('danger')];
        $big_menu = [
            'javascript:window.top.art.dialog({id:\'add\',iframe:\'?m=admin&c=badword&a=add\', title:\'' . L('badword_add') . '\', width:\'450\', height:\'180\'}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);',
            L('badword_add'),
        ];
    ...
    {
        $badword = isset($_GET['badword']) && trim($_GET['badword']) ? (pc_base::load_config('system', 'charset') == 'gbk' ? iconv('utf-8', 'gbk',
            trim($_GET['badword'])) : trim($_GET['badword'])) : exit('0');
        $badid = isset($_GET['badid']) && intval($_GET['badid']) ? intval($_GET['badid']) : '';
        $data = [];
        if ($badid) {
            $data = $this->db->get_one(['badid' => $badid], 'badword');
            if (!empty($data) && $data['badword'] == $badword) {
                exit('1');
            }
        }
        if ($this->db->get_one(['badword' => $badword], 'badid')) {
            exit('0');
        } else {
            exit('1');
        }
    ...
    function listorder()
    {
        ...
        foreach ($_POST['listorders'] as $badid => $listorder) {
            $this->db->update(['listorder' => $listorder], ['badid' => $badid]);
        }
    ...
    function edit()
    {
        if (isset($_POST['dosubmit'])) {
            ...
            $_POST['info']['replaceword'] = str_replace("　", "", trim($_POST['replaceword']));
            $_POST['info']['badword'] = str_replace("　", "", trim($_POST['badword']));
            $this->db->update($_POST['info'], ['badid' => $badid]);
            $this->public_cache_file();//更新缓存
            showmessage(L('operation_success'), '?m=admin&c=badword&a=edit', '', 'edit');
        } else {
            $show_validator = $show_scroll = $show_header = true;
            $info = [];
            $info = $this->db->get_one(['badid' => $_GET['badid']]);
            if (!$info) {
                showmessage(L('keywords_no_exist'));
            }
            extract($info);
    ...
    function delete()
    {
        if (is_array($_POST['badid'])) {
            foreach ($_POST['badid'] as $badid_arr) {
                $this->db->delete(['badid' => $badid_arr]);
            }
            $this->public_cache_file();//更新缓存
            showmessage(L('operation_success'), '?m=admin&c=badword');
        } else {
            ...
            $result = $this->db->delete(['badid' => $badid]);
            if ($result) {
                $this->public_cache_file();//更新缓存
                showmessage(L('operation_success'), '?m=admin&c=badword');
            } else {
    ...
    function import()
    {
        if (isset($_POST['dosubmit'])) {
            ...
            $s = trim($_POST['info']);
            if (empty($s)) {
                showmessage(L('not_information'), '?m=admin&c=badword&a=import');
            }
            $arr = explode("\n", $s);
            ...
            foreach ($arr as $s) {
                ...
                if (!in_array($sql_str['level'], $level_arr)) {
                    $sql_str['level'] = '1';
                }
                if (empty($sql_str['badword'])) {
                    continue;
                } else {
                    $check_badword = $this->db->get_one(['badword' => $sql_str['badword']], $data = '*', $order = '', $group = '');
                    if ($check_badword) {
                        continue;
                    }
