...
namespace phpcms\modules\mood;

use pc_base;
use phpcms\model\mood_model;
...
class mood_admin extends admin
{
    ...
    public function init()
    {
        ...
        $mood_program = isset($mood_program[$this->get_siteid()]) ? $mood_program[$this->get_siteid()] : [];
        $mood_db = new mood_model();
        ...
        if ($catid) {
            ...
            foreach ($content_data as $k => $v) {
                $content_data[$v['id']] = ['title' => $v['title'], 'url' => $v['url']];
                unset($content_data[$k]);
            }
            $pages = $mood_db->pages;
        }
        $order_list = ['-1' => L('total')];
    ...
    public function setting()
    {
        ...
        if (isset($_POST['dosubmit'])) {
            ...
            $data = [];
            foreach ($name as $k => $v) {
                $data[$k] = ['use' => $use[$k], 'name' => $v, 'pic' => $pic[$k]];
            }
