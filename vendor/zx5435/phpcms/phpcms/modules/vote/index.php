...
use phpcms\model\vote_data_model;
use phpcms\model\vote_option_model;
use phpcms\model\vote_subject_model;
use phpcms\modules\pay\classes\receipts;

/**
 * @property vote_subject_model $vote
 * @property vote_option_model  $vote_option
 * @property vote_data_model    $vote_data
 */
class index
{
    public function __construct()
    {
        ...
        $this->vote = new vote_subject_model();//投票标题
        $this->vote_option = new vote_option_model();//投票选项
        $this->vote_data = new vote_data_model(); //投票统计的数据模型
    ...
    public function show()
    {
        ...
        //获取投票结果信息
        $infos = $this->vote_data->select(['subjectid' => $subjectid], 'data');
        //循环每个会员的投票记录
    ...
    public function post()
    {
        ...
        //添加到数据库
        $this->vote_data->insert(['userid' => $this->userid, 'username' => $this->username, 'subjectid' => $subjectid, 'time' => $time, 'ip' => $this->ip, 'data' => $new_data]);
        //查询投票奖励点数，并更新会员点数
        $vote_arr = $this->vote->get_one(['subjectid' => $subjectid]);
        receipts::point($vote_arr['credit'], $this->userid, $this->username, '', 'selfincome', L('vote_post_point'));
        //更新投票人数
        $this->vote->update(['votenumber' => '+=1'], ['subjectid' => $subjectid]);
     ...
     * @return int (1:可投票  0: 多投,时间段内不可投票  -1:单投,已投票,不可重复投票)
     */
    public function check($subjectid)
    {
        ...
        //是否有投票记录
        $user_info = $this->vote_data->select(['subjectid' => $subjectid, 'ip' => $this->ip, 'username' => $this->username], '*', '1', ' time DESC');
        if (!$user_info) {
            return 1;
        } else {
