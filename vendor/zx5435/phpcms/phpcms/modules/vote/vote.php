...
namespace phpcms\modules\vote;

use pc_base;
use phpcms\model\vote_option_model;
use phpcms\model\vote_subject_model;
use phpcms\modules\admin\classes\admin;

/**
 * @property vote_subject_model $db
 * @property vote_option_model  $db2
 */
class vote extends admin
{
    ...
    function __construct()
    {
        ...
        $this->db = new vote_subject_model();
        $this->db2 = new vote_option_model();
    }

    public function init()
    {
        $page = isset($_GET['page']) && intval($_GET['page']) ? intval($_GET['page']) : 1;
        $infos = $this->db->listinfo(['siteid' => $this->get_siteid()], 'subjectid DESC', $page, '14');
        $pages = $this->db->pages;
        $big_menu = [
            'javascript:window.top.art.dialog({id:\'add\',iframe:\'?m=vote&c=vote&a=add\', title:\'' . L('vote_add') . '\', width:\'700\', height:\'450\'}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);',
            L('vote_add'),
        ];
    ...
    {
        $subject_title = isset($_GET['subject_title']) && trim($_GET['subject_title']) ? (pc_base::load_config('system', 'charset') == 'gbk' ? iconv('utf-8', 'gbk',
            trim($_GET['subject_title'])) : trim($_GET['subject_title'])) : exit('0');
        $subjectid = isset($_GET['subjectid']) && intval($_GET['subjectid']) ? intval($_GET['subjectid']) : '';
        $data = [];
        if ($subjectid) {
            $data = $this->db->get_one(['subjectid' => $subjectid], 'subject');
            if (!empty($data) && $data['subject'] == $subject_title) {
                exit('1');
            }
        }
        if ($this->db->get_one(['subject' => $subject_title], 'subjectid')) {
            exit('0');
        } else {
            exit('1');
        }
    ...
    public function add()
    {
        ...
        if (isset($_POST['dosubmit'])) {
            ...
            if (isset($_POST['from_api']) && $_POST['from_api']) {
                showmessage(L('operation_success'), '?m=vote&c=vote&a=add', '100', '',
                    "window.top.$('#voteid').val('" . $subjectid . "');window.top.art.dialog({id:'addvote'}).close();");
            } else {
    ...
    public function edit()
    {
        ...
        if (isset($_POST['dosubmit'])) {
            ...
            $this->db->update($post_data['subject'], ['subjectid' => $subjectid]);//更新投票选项总数
            ...
            //解出投票内容
            $info = $this->db->get_one(['subjectid' => $_GET['subjectid']]);
            if (!$info) {
        ...
        if ((!isset($_GET['subjectid']) || empty($_GET['subjectid'])) && (!isset($_POST['subjectid']) || empty($_POST['subjectid']))) {
            ...
            if (is_array($_POST['subjectid'])) {
                foreach ($_POST['subjectid'] as $subjectid_arr) {
                    ...
                    $this->db->delete(['subjectid' => $subjectid_arr]);
                }
                ...
                //删除投票
                $this->db->delete(['subjectid' => $subjectid]);
                $result = $this->db->delete(['subjectid' => $subjectid]);
                if ($result) {
                    showmessage(L('operation_success'), '?m=vote&c=vote');
                } else {
    ...
    {
        $infos = $this->db->select(['siteid' => $this->get_siteid()], '*');
        if (is_array($infos)) {
            foreach ($infos as $subjectid_arr) {
    ...
    public function public_call()
    {
        ...
        $r = $this->db->get_one(['subjectid' => $_GET['subjectid']]);
    ...
    {
        $infos = $this->db->listinfo(['siteid' => $this->get_siteid()], 'subjectid DESC', $page, '10');
