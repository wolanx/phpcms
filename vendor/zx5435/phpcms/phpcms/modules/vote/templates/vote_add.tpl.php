...
<script>
    $(function () {
        ...
        $("#subject_title").formValidator({onshow: "<?php echo L("input") . L('vote_title')?>", onfocus: "<?php echo L("input") . L('vote_title')?>"}).inputValidator({
            min: 1,
            ...
            type: "get",
            url: "",
            data: "m=vote&c=vote&a=public_name",
            datatype: "html",
            async: 'false',
        ...
        $("#option1").formValidator({onshow: "<?php echo L("input") . L('vote_option')?>", onfocus: "<?php echo L("input") . L('vote_option')?>"}).inputValidator({
            min: 1,
            onerror: "<?php echo L("input") . L('vote_option')?>"
        }).regexValidator({regexp: "notempty", datatype: "enum", param: 'i', onerror: "<?php echo L('input_not_space')?>"});
        $("#option2").formValidator({onshow: "<?php echo L("input") . L('vote_option')?>", onfocus: "<?php echo L("input") . L('vote_option')?>"}).inputValidator({
            min: 1,
            onerror: "<?php echo L("input") . L('vote_option')?>"
        }).regexValidator({regexp: "notempty", datatype: "enum", param: 'i', onerror: "<?php echo L('input_not_space')?>"});
        $("#fromdate").formValidator({
            onshow: "<?php echo L('select') . L('fromdate')?>", onfocus: "<?php echo L('select') . L('fromdate')?>", oncorrect: "<?php echo L('time_is_ok');
                ?>"
        }).inputValidator();
        $("#todate").formValidator({
        ...
        <table cellpadding="2" cellspacing="1" class="table_form" width="100%">
            ...
            <tr id="SizeFormat" style="display: none;">
                ...
                <td><label><?php echo L('minval') ?></label>&nbsp;&nbsp;<input name="subject[minval]"
                                                                               class="input-text" type="text" size="5"> <?php echo L('item') ?> &nbsp;&nbsp;&nbsp;&nbsp;
                    <label><?php echo L('maxval') ?></label>&nbsp;&nbsp;<input
            ...
            <tr>
                ...
                <td><input type="text" name="subject[interval]" value="<?php echo $interval; ?>" size='5'/> <?php echo L('more_ip') ?>，<font
                            color=red>0</font> <?php echo L('one_ip') ?></td>
            </tr>
...
<script>
    ...
    function load_file_list(id) {
        $.getJSON('?m=admin&c=category&a=public_tpl_file_list&style=' + id + '&module=vote&templates=vote_tp&name=vote_subject&pc_hash=' + pc_hash, function (data) {
            $('#show_template').html(data.vote_tp_template);
        });
    }
</script>