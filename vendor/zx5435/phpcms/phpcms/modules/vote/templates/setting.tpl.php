...
<form method="post" action="?m=vote&c=vote&a=setting">
    <table width="100%" cellpadding="0" cellspacing="1" class="table_form">
        ...
        <tr>
            ...
            <td>
                <input type="text" name="setting[interval]" value="<?php echo $interval; ?>" size='5'/> <?php echo L('more_ip') ?>，<font
                        color=red>0</font> <?php echo L('one_ip') ?>
            </td>
        </tr>
        <tr>
        ...
        <tr>
            <td>&nbsp;</td>
            <td>
                <input type="submit" name="dosubmit" id="dosubmit" value="<?= L('confirm') ?>" class="button">&nbsp
...
<script>
    function load_file_list(id) {
        $.getJSON('?m=admin&c=category&a=public_tpl_file_list&style=' + id + '&module=vote&templates=vote_tp&name=setting&pc_hash=' + pc_hash, function (data) {
