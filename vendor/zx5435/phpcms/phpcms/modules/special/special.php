...
use phpcms\model\special_content_model;
use phpcms\model\special_model;
use phpcms\model\type_model;
use phpcms\model\vote_subject_model;
use phpcms\modules\admin\classes\admin;
use phpcms\modules\admin\classes\sites;
use phpcms\modules\special\classes\html;
...
class special extends admin
{
    ...
    public function init()
    {
        ...
        $infos = $this->db->listinfo(['siteid' => $this->get_siteid()], '`listorder` DESC, `id` DESC', $page, 6);
        ...
        if (isset($_POST['dosubmit']) && !empty($_POST['dosubmit'])) {
            ...
            if ($id) {
                ...
                if ($special['siteid'] > 1) {
                    ...
                    $url = $special['ishtml'] ? APP_PATH . substr(pc_base::load_config('system', 'html_root'),
                            1) . '/special/' . $special['filename'] . '/' : APP_PATH . 'index.php?m=special&c=index&id=' . $id;
                }
                $this->db->update(['url' => $url], ['id' => $id, 'siteid' => $this->get_siteid()]);
                ...
                if (pc_base::load_config('system', 'attachment_stat')) {
                    ...
                    $this->attachment_db->api_update([$special['thumb'], $special['banner']], 'special-' . $id, 1);
                }
                $this->special_cache();
            }
    ...
    public function edit()
    {
        ...
        if (isset($_POST['dosubmit']) && !empty($_POST['dosubmit'])) {
            ...
            $this->db->update($special, ['id' => $_GET['specialid'], 'siteid' => $this->get_siteid()]);
            ...
            if (pc_base::load_config('system', 'attachment_stat')) {
                ...
                $this->attachment_db->api_update([$special['thumb'], $special['banner']], 'special-' . $_GET['specialid'], 1);
            }
            $this->special_cache();
            showmessage(L('edit_special_success'), HTTP_REFERER);
        } else {
            $info = $this->db->get_one(['id' => $_GET['specialid'], 'siteid' => $this->get_siteid()]);
            //获取站点模板信息
            pc_base::load_app_func('global', 'admin');
            ...
            $type_db = new type_model();
            $types = $type_db->select(['module' => 'special', 'parentid' => $_GET['specialid'], 'siteid' => $this->get_siteid()], '`typeid`, `name`, `listorder`, `typedir`', '',
    ...
    public function import()
    {
        if (isset($_POST['dosubmit']) || isset($_GET['dosubmit'])) {
            if (!is_array($_POST['ids']) || empty($_POST['ids']) || !$_GET['modelid']) {
                showmessage(L('illegal_action'), HTTP_REFERER);
            }
            if (!isset($_POST['typeid']) || empty($_POST['typeid'])) {
                showmessage(L('select_type'), HTTP_REFERER);
            }
            ...
            $where = '';
            if ($_GET['catid']) {
                $where .= get_sql_catid('category_content_' . $this->get_siteid(), $_GET['catid']) . " AND `status`=99";
            } else {
                $where .= " `status`=99";
            }
            if ($_GET['start_time']) {
    ...
    public function html()
    {
        if ((!isset($_POST['id']) || empty($_POST['id']))) {
            $result = $this->db->select(['disabled' => 0, 'siteid' => $this->get_siteid()], 'id', '', '', '', 'id');
    ...
    public function create_special_list()
    {
        ...
        $html = new html();
        $c = new special_model();
        $result = $c->get_one(['siteid' => $siteid], 'COUNT(*) AS total');
        $total = $result['total'];
        $pages = ceil($total / 20);
        for ($i = 1; $i <= $pages; $i++) {
            $size = $html->create_list($i);
        }
        showmessage(L('index_create_finish', ['size' => sizecount($size)]));
    }
    ...
    public function listorder()
    {
        if (isset($_POST['dosubmit'])) {
            foreach ($_POST['listorder'] as $id => $order) {
                $id = intval($id);
                $order = intval($order);
                $this->db->update(['listorder' => $order], ['id' => $id]);
            }
            $this->special_cache();
    ...
    {
        $info = $this->db->get_one(['id' => $specialid]);
        if (!$info['ishtml']) {
    ...
    public function public_create_type()
    {
        ...
        if (is_array($types) && !empty($types) || $pages) {
            ...
            if (!$pages) {
                $c = new special_content_model();
                $result = $c->get_one(['typeid' => $typeid], 'COUNT(*) AS total');
                $total = $result['total'];
                $pages = ceil($total / 20);
            }
            if ($_GET['typeid']) {
            ...
            if ($pages <= $maxpage) {
                ...
                showmessage($typename . L('type_from') . ($_GET['page'] ? $_GET['page'] : 1) . L('type_end') . $maxpage . '</font> ' . L('update_success'),
                    '?m=special&c=special&a=public_create_type&typeid=' . $typeid . '&typename=' . $typename . '&page=' . $page . '&pages=' . $pages . '&specialid=' . $specialid);
            }
    ...
    public function public_create_content()
    {
        ...
        $c = new special_content_model();
        if (!$pages) {
            $result = $c->get_one(['specialid' => $specialid, 'isdata' => 1], 'COUNT(*) AS total');
            $total = $result['total'];
            $pages = ceil($total / 10);
        }
        $offset = ($page - 1) * 10;
        $result = $c->select(['specialid' => $specialid, 'isdata' => 1], 'id', $offset . ', 10', 'listorder ASC, id ASC');
        foreach ($result as $r) {
            $html = pc_base::load_app_class('html');
            $urls = $html->_create_content($r['id']);
            $c->update(['url' => $urls[0]], ['id' => $r['id']]);
        }
        if ($page >= $pages) {
            showmessage(L('content_update_success'), '?m=special&c=special&a=public_create_html&specialid=' . $specialid);
        } else {
            $page++;
            showmessage(L('content_from') . ' <font color="red">' . intval($offset + 1) . L('type_end') . intval($offset + 10) . '</font> ' . L('update_success'),
                '?m=special&c=special&a=public_create_content&specialid=' . $specialid . '&page=' . $page . '&pages=' . $pages);
        }
    }
    ...
    public function elite()
    {
        ...
        $this->db->update(['elite' => $_GET['value']], ['id' => $_GET['id'], 'siteid' => get_siteid()]);
        showmessage(L('operation_success'), HTTP_REFERER);
    }
    ...
    public function delete($id = 0)
    {
        ...
        if (is_array($_POST['id']) && !$id) {
            array_map([$this, 'delete'], $_POST['id']);
            $this->special_cache();
            showmessage(L('operation_success'), HTTP_REFERER);
    ...
    {
        $specials = [];
        $result = $this->db->select(['disabled' => 0], '`id`, `siteid`, `title`, `url`, `thumb`, `banner`, `ishtml`', '', '`listorder` DESC, `id` DESC');
        foreach ($result as $r) {
            $specials[$r['id']] = $r;
        }
    ...
    public function public_check_special()
    {
        ...
        if ($_GET['id']) {
            $id = intval($_GET['id']);
            $r = $this->db->get_one(['id' => $id, 'siteid' => $this->get_siteid()]);
            if ($r['title'] == $title) {
                exit('1');
            }
        }
        $r = $this->db->get_one(['siteid' => $this->get_siteid(), 'title' => $title], 'id');
        if ($r['id']) {
            exit('0');
        } else {
            exit('1');
    ...
    public function public_check_dir()
    {
        ...
        $r = $this->db->get_one(['siteid' => $this->get_siteid(), 'filename' => $_GET['filename']], 'id');
        if ($r['id']) {
            exit('0');
        } else {
            exit('1');
    ...
    {
        if (!$data['title']) {
            showmessage(L('title_cannot_empty'), HTTP_REFERER);
        }
        if (!$data['banner']) {
            showmessage(L('banner_no_empty'), HTTP_REFERER);
        }
        if (!$data['thumb']) {
            showmessage(L('thumb_no_empty'), HTTP_REFERER);
        }
        if (is_array($data['catids']) && !empty($data['catids'])) {
            $data['catids'] = ',' . implode(',', $data['catids']) . ',';
        }
        if ($a == 'add') {
            if (!$data['index_template']) {
        ...
        if ($data['voteid']) {
            if (strpos($data['voteid'], '|') === false) {
                $vote_db = new vote_subject_model();
                $r = $vote_db->get_one(['subject' => $data['voteid'], 'siteid' => $this->get_siteid()], 'subjectid, subject', 'addtime DESC');
                if ($r) {
