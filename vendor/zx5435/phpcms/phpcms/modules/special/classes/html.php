...
namespace phpcms\modules\special\classes;
...
class html
{
    ...
    public function _create_content($contentid = 0)
    {
        ...
        $r = $this->c_db->get_one(['id' => $contentid]);
        $_special = $s_info = $this->db->get_one(['id' => $r['specialid']]);
        if ($s_info['ishtml'] == 0) {
            return content_url($contentid, '1', 0, 'php');
        }
        unset($arr_content);
        $arr_content = $this->data_db->get_one(['id' => $contentid]);
        @extract($r);
    ...
    public function _index($specialid = 0, $pagesize = 20, $pages_num = 0)
    {
        ...
        $r = $this->db->get_one(['id' => $specialid, 'siteid' => get_siteid()]);
        ...
        if (!$ispage) {
            ...
            $types = $type_db->select(['module' => 'special', 'parentid' => $specialid], '*', '', '`listorder` ASC, `typeid` ASC', '', 'listorder');
        }
        $css = get_css(unserialize($css));
        $template = $index_template ? $index_template : 'index';
        $SEO = seo($siteid, '', $title, $description);
        if ($ispage) {
            $re = $this->c_db->get_one(['specialid' => $specialid], 'COUNT(`id`) AS num');
            $total = $re['num'];
    ...
    public function create_list($page = 1)
    {
        ...
        $site_info = $this->site->get_by_id($siteid);
        define('URLRULE',
            $site_info['domain'] . substr(pc_base::load_config('system', 'html_root'), 1) . '/special/index.html~' . $site_info['domain'] . substr(pc_base::load_config('system',
                'html_root'), 1) . '/special/index-{$page}.html');
        //分站时计算路径
        if ($siteid > 1) {
            if ($page == 1) {
                $file = pc_base::load_config('system', 'html_root') . '/' . $site_info['dirname'] . '/special/index.html';
            } else {
                $file = pc_base::load_config('system', 'html_root') . '/' . $site_info['dirname'] . '/special/index-' . $page . '.html';
            }
        } else {
            if ($page == 1) {
                $file = pc_base::load_config('system', 'html_root') . '/special/index.html';
            } else {
                $file = pc_base::load_config('system', 'html_root') . '/special/index-' . $page . '.html';
            }
        }
        $this->queue->add_queue('add', $file, $siteid);
        $file = PHPCMS_PATH . $file;
    ...
    public function create_type($typeid = 0, $page = 1)
    {
        ...
        $info = $this->type_db->get_one(['typeid' => $typeid]);
        $s_info = $this->db->get_one(['id' => $info['parentid']]);
        extract($s_info);
        $site_info = $this->site->get_by_id($siteid);
        define('URLRULE', $site_info['domain'] . substr(pc_base::load_config('system', 'html_root'),
                1) . '/special/{$specialdir}/{$typedir}/type-{$typeid}.html~' . $site_info['domain'] . substr(pc_base::load_config('system', 'html_root'),
                1) . '/special/{$specialdir}/{$typedir}/type-{$typeid}-{$page}.html');
        $GLOBALS['URL_ARRAY'] = ['specialdir' => $filename, 'typedir' => $info['typedir'], 'typeid' => $typeid];
        $SEO = seo($siteid, '', $info['typename'], '');
        $template = $list_template ? $list_template : 'list';

        if ($siteid > 1) {
            if ($page == 1) {
                $file = pc_base::load_config('system', 'html_root') . '/' . $site_info['dirname'] . '/special/' . $filename . '/' . $info['typedir'] . '/type-' . $typeid . '.html';
            } else {
                $file = pc_base::load_config('system',
                        'html_root') . '/' . $site_info['dirname'] . '/special/' . $filename . '/' . $info['typedir'] . '/type-' . $typeid . '-' . $page . '.html';
            }
        } else {
            if ($page == 1) {
                $file = pc_base::load_config('system', 'html_root') . '/special/' . $filename . '/' . $info['typedir'] . '/type-' . $typeid . '.html';
            } else {
                $file = pc_base::load_config('system', 'html_root') . '/special/' . $filename . '/' . $info['typedir'] . '/type-' . $typeid . '-' . $page . '.html';
            }
        }
        $this->queue->add_queue('add', $file, $siteid);
        $file = PHPCMS_PATH . $file;
     ...
     * 生成分类静态页
     * @param int $typeid 分类ID
     * @param int $pagesize 每页篇数
     * @param int $pages 最大更新页数
     * @return bool
     */
    public function _list($typeid = 0, $pagesize = 20, $pages = 0)
    {
        ...
        $r = $this->c_db->get_one(['typeid' => $typeid], 'COUNT(`id`) AS num');
        $total = $r['num'];
        $times = ceil($total / $pagesize);
        if ($pages) {
