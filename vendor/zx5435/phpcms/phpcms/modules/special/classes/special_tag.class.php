...
class special_tag
{
    ...
    public function __construct()
    {
        ...
        $this->c = new special_content_model();
    }
    ...
    public function lists($data)
    {
        ...
        $listorder = ['`id` ASC', '`id` DESC', '`listorder` ASC, `id` DESC', '`listorder` DESC, `id` DESC'];
    ...
    public function count($data)
    {
        ...
        if ($data['action'] == 'lists') {
            ...
            if ($data['specialid']) {
                $where .= " AND `specialid`='" . $data['specialid'] . "'";
            }
            if ($data['typeid']) {
    ...
    public function hits($data)
    {
        ...
        $sql = "hitsid LIKE '$hitsid'";
        $listorders = ['views DESC', 'yesterdayviews DESC', 'dayviews DESC', 'weekviews DESC', 'monthviews DESC'];
        $result = $this->hits_db->select($sql, '*', $data['limit'], $listorders[$data['listorder']]);
        foreach ($result as $key => $r) {
            $ids = explode('-', $r['hitsid']);
            $id = $ids[3];
            $re = $this->c->get_one(['id' => $id]);
    ...
    public function content_list($data)
    {
        ...
        $listorder = ['`id` ASC', '`id` DESC', '`listorder` ASC', '`listorder` DESC'];
    ...
    public function get_type($specialid = 0, $value = '', $id = '')
    {
        ...
        $data = $arr = [];
        $data = $type_db->select(['module' => 'special', 'parentid' => $specialid]);
    ...
    public function pc_tag()
    {
        ...
        return [
            'action'       => ['lists' => L('special_list', '', 'special'), 'content_list' => L('content_list', '', 'special'), 'hits' => L('hits_order', '', 'special')],
            'lists'        => [
                'siteid'    => ['name' => L('site_id', '', 'comment'), 'htmltype' => 'input_select', 'data' => $sitelist],
                'elite'     => ['name' => L('iselite', '', 'special'), 'htmltype' => 'radio', 'defaultvalue' => '0', 'data' => [L('no'), L('yes')]],
                'thumb'     => ['name' => L('get_thumb', '', 'special'), 'htmltype' => 'radio', 'defaultvalue' => '0', 'data' => [L('no'), L('yes')]],
                'listorder' => [
                    'name'         => L('order_type', '', 'special'),
                    'htmltype'     => 'select',
                    'defaultvalue' => '3',
                    'data'         => [L('id_asc', '', 'special'), L('id_desc', '', 'special'), L('order_asc', '', 'special'), L('order_desc', '', 'special')],
                ],
            ],
            'content_list' => [
                'specialid' => [
                    'name'     => L('special_id', '', 'special'),
                    'htmltype' => 'input_select',
                    'data'     => $specials,
                    'ajax'     => ['name' => L('for_type', '', 'special'), 'action' => 'get_type', 'id' => 'typeid'],
                ],
                'thumb'     => ['name' => L('content_thumb', '', 'special'), 'htmltype' => 'radio', 'defaultvalue' => '0', 'data' => [L('no'), L('yes')]],
                'listorder' => [
                    'name'         => L('order_type', '', 'special'),
                    'htmltype'     => 'select',
                    'defaultvalue' => '3',
            ...
            'hits'         => [
                'specialid' => ['name' => L('special_id', '', 'special'), 'htmltype' => 'input_select', 'data' => $specials],
                'listorder' => [
                    'name'     => L('order_type', '', 'special'),
                    'htmltype' => 'select',
                    'data'     => [L('total', '', 'special'), L('yesterday', '', 'special'), L('today', '', 'special'), L('week', '', 'special'), L('month', '', 'special')],
                ],
            ],
        ];
    }
