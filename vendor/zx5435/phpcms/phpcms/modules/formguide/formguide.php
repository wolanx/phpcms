...
/**
 * @property sitemodel_field_model $m_db
 */
class formguide extends admin
{
    ...
    public function init()
    {
        ...
        $data = $this->db->listinfo(['type' => 3, 'siteid' => $this->get_siteid()], '`modelid` DESC', $page);
        $big_menu = [
            'javascript:window.top.art.dialog({id:\'add\',iframe:\'?m=formguide&c=formguide&a=add\', title:\'' . L('formguide_add') . '\', width:\'700\', height:\'500\', lock:true}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);',
            L('formguide_add'),
        ];
    ...
    public function add()
    {
        if (isset($_POST['dosubmit'])) {
            ...
            $this->m_db = new sitemodel_field_model();
    ...
    public function edit()
    {
        ...
        if (isset($_POST['dosubmit'])) {
            ...
            $_POST['info']['js_template'] = $_POST['info']['show_js_template'];
            unset($_POST['info']['show_js_template']);
            $this->db->update($_POST['info'], ['modelid' => $formid]);
            ...
            $data = $this->db->get_one(['modelid' => $formid]);
    ...
    public function disabled()
    {
        ...
        $this->db->update(['disabled' => $val], ['modelid' => $formid, 'siteid' => $this->get_siteid()]);
    ...
    public function public_preview()
    {
        ...
        $formid = intval($_GET['formid']);
        $f_info = $this->db->get_one(['modelid' => $formid, 'siteid' => $this->get_siteid()], 'name');
    ...
    public function public_checktable()
    {
        ...
        $r = $this->db->get_one(['tablename' => $_GET['tablename']], 'tablename, modelid');
        if (!$r['modelid']) {
            exit('1');
     ...
     * @return array
     */
    private function check_info($data = [], $formid = 0)
    {
        ...
        $r = $this->db->get_one(['tablename' => $data['tablename']], 'tablename, modelid');
    ...
    public function delete()
    {
        ...
        if (isset($_GET['formid']) && !empty($_GET['formid'])) {
            ...
            $m_db->delete(['modelid' => $formid, 'siteid' => $siteid]);
            $m_info = $this->db->get_one(['modelid' => $formid], 'tablename');
            $tablename = $m_db->db_tablepre . 'form_' . $m_info['tablename'];
            $m_db->query("DROP TABLE `$tablename`");
            $this->db->delete(['modelid' => $formid, 'siteid' => $siteid]);
            showmessage(L('operation_success'), HTTP_REFERER);
            ...
            if (is_array($_POST['formid'])) {
                foreach ($_POST['formid'] as $fid) {
                    $m_info = $this->db->get_one(['modelid' => $fid], 'tablename');
                    $tablename = $m_db->db_tablepre . 'form_' . $m_info['tablename'];
                    $m_db->query("DROP TABLE `$tablename`");
                    $this->db->delete(['modelid' => $fid, 'siteid' => $siteid]);
                }
            }
            showmessage(L('operation_success'), HTTP_REFERER);
        } else {
    ...
    public function stat()
    {
        ...
        $f_info = $this->db->get_one(['modelid' => $formid, 'siteid' => $this->get_siteid()], 'tablename');
        $tablename = 'form_' . $f_info['tablename'];
        $m_db = pc_base::load_model('sitemodel_field_model');
        $result = $m_db->select(['modelid' => $formid, 'formtype' => 'box'], 'field, setting');
