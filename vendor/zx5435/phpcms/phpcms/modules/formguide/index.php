...
class index
{
    ...
    public function index()
    {
        ...
        $r = $this->db->get_one(['siteid' => $this->siteid, 'type' => 3, 'disabled' => 0], 'COUNT(`modelid`) AS sum');
        $total = $r['sum'];
        $pages = pages($total, $page, 20);
        $offset = ($page - 1) * 20;
        $datas = $this->db->select(['siteid' => $this->siteid, 'type' => 3, 'disabled' => 0], 'modelid, name, addtime', $offset . ',20', '`modelid` DESC');
    ...
    public function show()
    {
        ...
        $r = $this->db->get_one(['modelid' => $formid, 'siteid' => $siteid, 'disabled' => 0], 'tablename, setting');
        if (!$r) {
            $_GET['action'] ? exit : showmessage(L('form_no_exist'), HTTP_REFERER);
        }
        ...
        if (isset($_POST['dosubmit'])) {
            ...
            if ($dataid) {
                ...
                $this->db->update(['items' => '+=1'], ['modelid' => $formid, 'siteid' => $this->siteid]);
            }
            ...
            $f_info = $this->db->get_one(['modelid' => $formid, 'siteid' => $this->siteid]);
            extract($f_info);
            $tablename = 'form_' . $r['tablename'];
            $this->m_db->change_table($tablename);
            $ip = ip();
