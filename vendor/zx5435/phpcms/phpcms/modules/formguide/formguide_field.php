...
namespace phpcms\modules\formguide;
...
/**
 * @property int                   $siteid
 * @property sitemodel_field_model $db
 * @property sitemodel_model       $model_db
 */
class formguide_field extends admin
{
    public function __construct()
    {
        ...
        $this->model_db = new sitemodel_model();
    ...
    public function init()
    {
        if (isset($_GET['formid']) && !empty($_GET['formid'])) {
            ...
            $datas = $this->db->select(['modelid' => $formid], '*', 100, 'listorder ASC');
            $r = $this->model_db->get_one(['modelid' => $formid]);
        } else {
            $data = $datas = [];
    ...
    public function add()
    {
        if (isset($_POST['dosubmit'])) {
            ...
            if (isset($_POST['info']['modelid']) && !empty($_POST['info']['modelid'])) {
                ...
                $forminfo = $this->model_db->get_one(['modelid' => $formid, 'siteid' => $this->siteid], 'tablename');
                ...
                if (array_key_exists($_POST['info']['field'], $form_public_field_array)) {
                    ...
                    $form_public_field_array[$_POST['info']['field']] = ['info' => $_POST['info'], 'sql' => $sql];
            ...
            require MODEL_PATH . 'fields.inc.php';
            $formid = intval($_GET['formid']);
            $f_datas = $this->db->select(['modelid' => $formid], 'field,name', 100, 'listorder ASC');
            $m_r = $this->model_db->get_one(['modelid' => $formid]);
    ...
    public function edit()
    {
        if (isset($_POST['dosubmit'])) {
            ...
            if (isset($_POST['info']['modelid']) && !empty($_POST['info']['modelid'])) {
                ...
                $this->db->update($_POST['info'], ['fieldid' => $fieldid, 'siteid' => $this->siteid]);
            } else {
                $unrunsql = 1;
                $tablename = 'formguide_table';
                require MODEL_PATH . 'add.sql.php';
            ...
            if (isset($_GET['formid']) && !empty($_GET['formid'])) {
                ...
                $m_r = $this->model_db->get_one(['modelid' => $formid]);
                $r = $this->db->get_one(['fieldid' => $fieldid]);
                extract($r);
                require MODEL_PATH . $formtype . DS . 'config.inc.php';
            } else {
            ...
            ob_start();
            include MODEL_PATH . $formtype . DS . 'field_edit_form.inc.php';
    ...
    public function disabled()
    {
        ...
        $this->db->update(['disabled' => $disabled], ['fieldid' => $fieldid, 'siteid' => $this->siteid]);
    ...
    public function delete()
    {
        if (isset($_GET['formid']) && !empty($_GET['formid']) && isset($_GET['fieldid']) && !empty($_GET['fieldid'])) {
            $formid = intval($_GET['formid']);
            $fieldid = intval($_GET['fieldid']);
            $r = $this->model_db->get_one(['modelid' => $formid], 'tablename');
            $rs = $this->db->get_one(['fieldid' => $fieldid, 'siteid' => $this->siteid], 'field');
            $this->db->delete(['fieldid' => $fieldid, 'siteid' => $this->siteid]);
            if ($r) {
                $field = $rs['field'];
        ...
        if (isset($_POST['dosubmit'])) {
            if (isset($_GET['formid']) && !empty($_GET['formid'])) {
                foreach ($_POST['listorders'] as $id => $listorder) {
                    $this->db->update(['listorder' => $listorder], ['fieldid' => $id]);
                }
            } else {
                $form_public_field_array = getcache('form_public_field_array', 'model');
    ...
    public function public_checkfield()
    {
        ...
        if (in_array($field, ['dataid', 'userid', 'username', 'datetime', 'ip'])) {
            exit('0');
        }

        if ($modelid) {
            $forminfo = $this->model_db->get_one(['modelid' => $modelid], 'tablename');
    ...
    public function public_field_setting()
    {
        ...
        require MODEL_PATH . $fieldtype . DS . 'config.inc.php';
        ob_start();
        include MODEL_PATH . $fieldtype . DS . 'field_add_form.inc.php';
    ...
    /**
     * 更新指定表单向导的字段缓存
     * @param int $formid 表单向导id
     * @param int $disabled 字段状态
     * @return bool
     */
    public function cache_field($formid = 0, $disabled = 0)
    {
        $field_array = [];
        $fields = $this->db->select(['modelid' => $formid, 'disabled' => $disabled], '*', 100, 'listorder ASC');
