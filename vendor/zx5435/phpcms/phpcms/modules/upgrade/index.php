...
namespace phpcms\modules\upgrade;
...
class index extends admin
{
    ...
    private $_filearr = ['api', 'phpcms', 'statics', ''];
    //md5验证地址
    ...
    public function init()
    {
        ...
        if (!empty($_GET['s'])) {
            ...
            foreach ($pathlist as $k => $v) {
                ...
                //保存到本地地址
                $upgradezip_path = CACHE_PATH . 'caches_upgrade' . DS . $v;
                //解压路径
                $upgradezip_source_path = CACHE_PATH . 'caches_upgrade' . DS . basename($v, ".zip");
                ...
                //拷贝gbk/upload文件夹到根目录
                $copy_from = $upgradezip_source_path . DS . $patch_charset . DS . 'upload' . DS;
                $copy_to = PHPCMS_PATH;
                ...
                if ($this->copyfailnum > 0) {
                    //如果失败，恢复当前版本
                    @file_put_contents(CACHE_PATH . 'configs' . DS . 'version.php', '<?php return ' . var_export($current_version, true) . ';?>');
                ...
                //执行sql
                //sql目录地址
                $sql_path = CACHE_PATH . 'caches_upgrade' . DS . basename($v, ".zip") . DS . $patch_charset . DS . 'upgrade' . DS . 'ext' . DS;
                $file_list = glob($sql_path . '*');
                if (!empty($file_list)) {
                    foreach ($file_list as $fk => $fv) {
                        if (in_array(strtolower(substr($fv, -3, 3)), ['php', 'sql'])) {
                            if (strtolower(substr($file_list[$fk], -3, 3)) == 'sql' && $data = file_get_contents($file_list[$fk])) {
                                ...
                                if (strtolower(basename($file_list[$fk])) == 'system_menu.lang.php' && file_exists($file_list[$fk])) {
                                    ...
                                    $menu_lan_file = PATH_PHPCMS . 'languages' . DS . $lang . DS . 'system_menu.lang.php';
                ...
                //配置文件地址
                $configpath = CACHE_PATH . 'caches_upgrade' . DS . basename($v, ".zip") . DS . $patch_charset . DS . 'upgrade' . DS . 'config.php';
                if (file_exists($configpath)) {
                    $config_arr = include $configpath;
                    $version_arr = ['pc_version' => $config_arr['to_version'], 'pc_release' => $config_arr['to_release']];
                    //版本文件地址
                    $version_filepath = CACHE_PATH . 'configs' . DS . 'version.php';
