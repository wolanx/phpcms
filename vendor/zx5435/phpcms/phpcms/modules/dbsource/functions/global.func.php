...
{
    $filepath = PATH_PHPCMS . 'modules' . DS . $module . DS . 'classes' . DS . $module . '_tag.class.php';
...
/**
 * 返回模板地址。
 * @param int $id 数据源调用ID
 */
function template_url($id)
{
    $filepath = CACHE_PATH . 'caches_template' . DS . 'dbsource' . DS . $id . '.php';
    if (!file_exists($filepath)) {
        $datacall = pc_base::load_model('datacall_model');
        $str = $datacall->get_one(['id' => $id], 'template');
