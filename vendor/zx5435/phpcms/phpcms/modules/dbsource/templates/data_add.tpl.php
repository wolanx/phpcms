...
<script type="text/javascript">
    $(function () {
        ...
        $("#name").formValidator({onshow: "<?php echo L('input') . L('name')?>", onfocus: "<?php echo L('input') . L('name')?>"}).inputValidator({
            min: 1,
            onerror: "<?php echo L('input') . L('name')?>"
        }).ajaxValidator({
            type: "get", url: "", data: "m=dbsource&c=data&a=public_name", datatype: "html", async: 'false', success: function (data) {
                if (data == "1") {
                    return true;
                } else {
                    return false;
                }
            }, buttons: $("#dosubmit"), onerror: "<?php echo L('name') . L('exists')?>", onwait: "<?php echo L('connecting')?>"
        });
        ...
        $("#cache").formValidator({
            ...
            empty: true
        }).regexValidator({regexp: "num1", datatype: 'enum', param: 'i', onerror: "<?php echo L("cache_time_can_only_be_positive")?>"});
        $("#num").formValidator({onshow: "<?php echo L('input') . L("num")?>", onfocus: "<?php echo L('input') . L("num")?>", empty: true}).regexValidator({
            regexp: "num1",
            datatype: 'enum',
            param: 'i',
            ...
            <fieldset>
                ...
                <table width="100%" class="table_form">
                    <tr>
                        ...
                        <td class="y-bg"><?php echo form::radio(['0' => L('model_configuration'), '1' => L('custom_sql')], $type ? $type : 0,
                                'name="type" onclick="location.href=\'' . get_url() . '&type=\'+this.value"') ?></td>
                    </tr>
                    <?php if ($type == 0) : ?>
                        <tr>
                            <th><?php echo L('select_model') ?>：</th>
                            <td class="y-bg"><?php echo form::select($modules, $module,
                                    'name="module" id="module" onchange="location.href=\'' . get_url() . '&module=\'+this.value"') ?>
                                <script type="text/javascript">$(function () {
                                        $("#module").formValidator({
                                            onshow: "<?php echo L('please_select_model')?>",
                                            onfocus: "<?php echo L('please_select_model')?>"
                                        }).inputValidator({min: 1, onerror: '<?php echo L('please_select_model')?>'});
                                    });</script>
                            </td>
                        </tr>
                        <?php if ($module): ?>
                            <tr>
                                ...
                                <td class="y-bg"><?php echo form::radio($html['action'], $action,
                                        'name="action" onclick="location.href=\'' . get_url() . '&action=\'+this.value"') ?></td>
                            </tr>
                        <?php endif; ?>
                        <?php if (isset($html[$action]) && is_array($html[$action]) && $action) {
                            foreach ($html[$action] as $k => $v): ?>
                                <tr>
                    ...
                    <?php else : ?>
                        <tr>
                            ...
                            <td class="y-bg"><textarea name="data" id="data" style="width:386px;height:178px;"></textarea>
                                <script type="text/javascript">$(function () {
                                        $("#data").formValidator({
                                            onshow: "<?php echo L('please_enter_a_sql')?>",
                                            onfocus: "<?php echo L('please_enter_a_sql')?>"
                                        }).inputValidator({min: 1, onerror: '<?php echo L('please_enter_a_sql')?>'});
                                    });</script>
                            </td>
                        </tr>
                    <?php endif; ?>
            ...
            <fieldset>
                ...
                <table width="100%" class="table_form">
                    ...
                    <tr>
                        ...
                        <td class="y-bg"><?php echo form::radio(['1' => 'json', '2' => 'xml', '3' => 'js'], '1', 'name="dis_type" onclick="showcode(this.value)"') ?></td>
                    </tr>
