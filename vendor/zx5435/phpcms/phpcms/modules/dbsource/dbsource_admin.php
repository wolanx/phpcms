...
namespace phpcms\modules\dbsource;
...
class dbsource_admin extends admin
{
    ...
    public function init()
    {
        ...
        $list = $this->db->listinfo(['siteid' => $this->get_siteid()], '', $page, 20);
        $pages = $this->db->pages;
        $big_menu = [
            'javascript:window.top.art.dialog({id:\'add\',iframe:\'?m=dbsource&c=dbsource_admin&a=add\', title:\'' . L('added_external_data_source') . '\', width:\'700\', height:\'500\', lock:true}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);',
            L('added_external_data_source'),
        ];
    ...
    public function add()
    {
        if (isset($_POST['dosubmit'])) {
            ...
            $charset = isset($_POST['charset']) && in_array(trim($_POST['charset']),
                ['gbk', 'utf8', 'gb2312', 'latin1']) ? trim($_POST['charset']) : showmessage(L('charset') . L('illegal_parameters'));
            $siteid = $this->get_siteid();
            if (!preg_match('/^\\w+$/i', $name)) {
                showmessage(L('data_source_of_the_letters_and_figures'));
            }
            //检察数据源名是否已经存在
            if ($this->db->get_one(['siteid' => $siteid, 'name' => $name], 'id')) {
