...
use pc_base;
use phpcms\model\datacall_model;
...
class data extends admin
{
    ...
    {
        $big_menu = [
            'javascript:window.top.art.dialog({id:\'add\',iframe:\'?m=dbsource&c=data&a=add\', title:\'' . L('adding_data_source_call') . '\', width:\'700\', height:\'500\', lock:true}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);',
            L('adding_data_source_call'),
        ];
    ...
    public function add()
    {
        ...
        if (isset($_POST['dosubmit'])) {
            ...
            if ($type == '1') { //自定义SQL
                ...
                $sql = ['data' => array2string($data), 'module' => $module, 'action' => $action];
            }
            ...
            if ($id = $this->db->insert($sql, true)) {
                ...
                if ($dis_type == 3) {
                    ...
                    $str = $this->db->get_one(['id' => $id], 'template');
                    $str = $tpl->template_parse($str['template']);
                    $filepath = CACHE_PATH . 'caches_template' . DS . 'dbsource' . DS;
                    if (!is_dir($filepath)) {
            ...
            $modules = array_merge(['' => L('please_select')], pc_base::load_config('modules'));
    ...
    public function edit()
    {
        ...
        if (!$edit_data = $this->db->get_one(['id' => $id])) {
