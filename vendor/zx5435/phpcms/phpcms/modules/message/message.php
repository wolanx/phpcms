...
namespace phpcms\modules\message;

use pc_base;
use phpcms\libs\classes\param;
use phpcms\model\message_group_model;
use phpcms\model\message_model;
use phpcms\modules\admin\classes\admin;

/**
 * @property message_model $db
 * @property message_group_model $group_db
...
class message extends admin
{
    ...
    {
        parent::__construct();
        $this->db = new message_model();
        $this->group_db = new message_group_model();
    ...
    public function init()
    {
        ...
        $pages = $this->db->pages;
        $big_menu = [
            'javascript:window.top.art.dialog({id:\'add\',iframe:\'?m=message&c=message&a=message_send\', title:\'' . L('all_send_message') . '\', width:\'550\', height:\'300\'}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);',
            L('all_send_message'),
        ];
    ...
    */
    ...
    {
        $tousername = isset($_GET['tousername']) && trim($_GET['tousername']) ? (pc_base::load_config('system', 'charset') == 'gbk' ? iconv('utf-8', 'gbk',
            trim($_GET['tousername'])) : trim($_GET['tousername'])) : exit('0');
        //不能发给自己
        ...
        if ((!isset($_GET['messageid']) || empty($_GET['messageid'])) && (!isset($_POST['messageid']) || empty($_POST['messageid']))) {
            ...
            if (is_array($_POST['messageid'])) {
                foreach ($_POST['messageid'] as $messageid_arr) {
                    //批量删除友情链接
                    $this->db->delete(['messageid' => $messageid_arr]);
                }
                ...
                //删除短消息
                $result = $this->db->delete(['messageid' => $messageid]);
                if ($result) {
                    showmessage(L('operation_success'), '?m=message&c=message');
                } else {
        ...
        if ((!isset($_GET['message_group_id']) || empty($_GET['message_group_id'])) && (!isset($_POST['message_group_id']) || empty($_POST['message_group_id']))) {
            ...
            if (is_array($_POST['message_group_id'])) {
                foreach ($_POST['message_group_id'] as $messageid_arr) {
                    //批量删除系统消息
                    $this->group_db->delete(['id' => $messageid_arr]);
                }
                ...
                //删除短消息
                $result = $this->group_db->delete(['id' => $group_id]);
                if ($result) {
                    showmessage(L('operation_success'), '?m=message&c=message&a=message_group_manage');
                } else {
    ...
    public function message_send()
    {
        if (isset($_POST['dosubmit'])) {
            //插入群发表
            $group_message = [];
            if (empty($_POST['info']['subject']) || empty($_POST['info']['content'])) {
                return false;
            }
    ...
    {
        $where = ['send_to_id' => $this->_username, 'folder' => 'inbox'];
        $page = isset($_GET['page']) && intval($_GET['page']) ? intval($_GET['page']) : 1;
        $infos = $this->db->listinfo($where, $order = 'messageid DESC', $page, $pages = '12');
        $pages = $this->db->pages;
        $big_menu = [
    ...
    public function delete_inbox()
    {
        if ((!isset($_GET['messageid']) || empty($_GET['messageid'])) && (!isset($_POST['messageid']) || empty($_POST['messageid']))) {
            ...
            if (is_array($_POST['messageid'])) {
                foreach ($_POST['messageid'] as $messageid_arr) {
                    //批量删除短消息
                    $this->db->update(['folder' => 'outbox'], ['messageid' => $messageid_arr, 'send_to_id' => $this->_username]);
                }
                showmessage(L('operation_success'), HTTP_REFERER);
            } else {
                $messageid = intval($_GET['messageid']);
                ...
                //删除单个短消息
                $result = $this->db->update(['folder' => 'outbox'], ['messageid' => $messageid, 'send_to_id' => $this->_username]);
                showmessage(L('operation_success'), HTTP_REFERER);
            }
    ...
    public function my_outbox()
    {
        ...
        $where = ['send_from_id' => $this->_username, 'del_type' => '0'];
        $page = isset($_GET['page']) && intval($_GET['page']) ? intval($_GET['page']) : 1;
        $infos = $this->db->listinfo($where, $order = 'messageid DESC', $page, $pages = '12');
        $pages = $this->db->pages;
        $big_menu = [
            'javascript:window.top.art.dialog({id:\'add\',iframe:\'?m=message&c=message&a=message_send\', title:\'' . L('all_send_message') . '\', width:\'550\', height:\'350\'}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);',
            L('all_send_message'),
        ];
        $trade_status = $this->trade_status;
    ...
    public function delete_outbox()
    {
        if ((!isset($_GET['messageid']) || empty($_GET['messageid'])) && (!isset($_POST['messageid']) || empty($_POST['messageid']))) {
            ...
            if (is_array($_POST['messageid'])) {
                foreach ($_POST['messageid'] as $messageid_arr) {
                    //批量删除短消息
                    $this->db->update(['del_type' => '1'], ['messageid' => $messageid_arr, 'send_from_id' => $this->_username]);
                }
                showmessage(L('operation_success'), HTTP_REFERER);
            } else {
                $messageid = intval($_GET['messageid']);
                ...
                //删除单个短消息
                $result = $this->db->update(['del_type' => '1'], ['messageid' => $messageid, 'send_from_id' => $this->_username]);
                showmessage(L('operation_success'), HTTP_REFERER);
            }
    ...
    public function search_message()
    {
        ...
        $pages = $this->db->pages;
        $big_menu = [
            'javascript:window.top.art.dialog({id:\'add\',iframe:\'?m=message&c=message&a=message_send\', title:\'' . L('all_send_message') . '\', width:\'700\', height:\'450\'}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);',
            L('all_send_message'),
        ];
        $trade_status = $this->trade_status;
