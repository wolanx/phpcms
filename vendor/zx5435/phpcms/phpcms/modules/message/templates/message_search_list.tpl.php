...
<div class="pad-lr-10">
    <form name="searchform" action="?m=message&c=message&a=search_message&menuid=<?php echo $_GET['menuid']; ?>" method="post">
        <table width="100%" cellspacing="0" class="search-form">
            ...
            <tr>
                <td>
                    <div class="explain-col"><?php echo L('query_type') ?>:<?php echo form::select($trade_status, $status, 'name="search[status]"',
                            L('all')) ?>      <?php echo L('username') ?>: <input type="text" value="<?php echo $username; ?>" class="input-text"
                                                                                  name="search[username]"> <?php echo L('time') ?>: <?php echo form::date('search[start_time]',
                            $start_time, '') ?> <?php echo L('to') ?>   <?php echo form::date('search[end_time]', $end_time, '') ?> <input type="submit"
                                                                                                                                           value="<?php echo L('search') ?>"
                    ...
                    foreach ($infos as $info) {
                        ...
                        <tr>
                            ...
                            <td align="center" width="15%"><a
                                        ...
                                        onClick="return confirm('<?php echo L('confirm', ['message' => $info['subject']]) ?>')"><?php echo L('delete') ?></a>
                            </td>
                        </tr>
                        <?php
                    }
...
<script>
    function see_all(id, name) {
        ...
        window.top.art.dialog({
            title: '<?php echo L('details');//echo L('edit')?> ' + name + ' ',
            id: 'edit',
            iframe: '?m=message&c=message&a=see_all&messageid=' + id,
            width: '700',
            height: '450'
        }, function () {
            var d = window.top.art.dialog({id: 'see_all'}).data.iframe;
            var form = d.document.getElementById('dosubmit');
            form.click();
            return false;
        }, function () {
            window.top.art.dialog({id: 'see_all'}).close()
        });
    }
