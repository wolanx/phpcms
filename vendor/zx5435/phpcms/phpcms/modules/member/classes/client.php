...
class client
{
    ...
    public function ps_member_register($username, $password, $email, $regip = '', $random = '')
    {
        ...
        return $this->_ps_send('register', ['username' => $username, 'password' => $password, 'email' => $email, 'regip' => $regip, 'random' => $random]);
    }
    ...
    public function ps_member_login($username, $password, $isemail = 0)
    {
        if ($isemail) {
            ...
            $return = $this->_ps_send('login', ['email' => $username, 'password' => $password]);
        } else {
            $return = $this->_ps_send('login', ['username' => $username, 'password' => $password]);
        }
    ...
    public function ps_member_synlogin($uid)
    {
        ...
        return $this->_ps_send('synlogin', ['uid' => $uid]);
    }
    ...
    {
        return $this->_ps_send('synlogout', []);
    }
    ...
    public function ps_member_edit($username, $email, $password = '', $newpassword = '', $uid = '', $random = '')
    {
        ...
        return $this->_ps_send('edit', ['username' => $username, 'password' => $password, 'newpassword' => $newpassword, 'email' => $email, 'uid' => $uid, 'random' => $random]);
    }
    ...
    {
        return $this->_ps_send('deleteavatar', ['uid' => $uid]);
    }
    ...
    {
        if ($type == 1) {
            $userinfo = $this->_ps_send('getuserinfo', ['uid' => $mix]);
        } elseif ($type == 2) {
            $userinfo = $this->_ps_send('getuserinfo', ['username' => $mix]);
        } elseif ($type == 3) {
            if (!$this->_is_email($mix)) {
                return -4;
            }
            $userinfo = $this->_ps_send('getuserinfo', ['email' => $mix]);
        }
        if ($userinfo) {
            return $userinfo;
        } else {
    ...
    {
        if ($type == 1) {
            $res = $this->_ps_send('delete', ['uid' => $mix]);
        } elseif ($type == 2) {
            $res = $this->_ps_send('delete', ['username' => $mix]);
        } elseif ($type == 3) {
            if (!$this->_is_email($mix)) {
                return -4;
            }
            $res = $this->_ps_send('delete', ['email' => $mix]);
        }
    ...
    {
        return $this->_ps_send('checkname', ['username' => $username]);
    }
    ...
    {
        return $this->_ps_send('checkemail', ['email' => $email]);
    }
    ...
    {
        return $this->_ps_send('getapplist', []);
    }
    ...
    {
        return $this->_ps_send('getcredit', []);
    }
    ...
    {
        return $this->_ps_send('changecredit', ['uid' => $uid, 'from' => $from, 'toappid' => $toappid, 'to' => $to, 'credit' => $credit]);
    }
    ...
    public function ps_getavatar($uid)
    {
        ...
        $avatar = ['180' => $url . '180x180.jpg', '90' => $url . '90x90.jpg', '45' => $url . '45x45.jpg', '30' => $url . '30x30.jpg'];
    ...
    {
        $auth_data = $this->auth_data(['uid' => $uid, 'ps_auth_key' => $this->ps_auth_key], '', $this->ps_auth_key);
