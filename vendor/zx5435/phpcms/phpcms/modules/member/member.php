...
class member extends admin
{
    ...
    function init()
    {
        ...
        $memberinfo['vipnum'] = $this->db->count(['vip' => 1]);
        $memberinfo['verifynum'] = $this->verify_db->count(['status' => 0]);
    ...
    function search()
    {
        ...
        $pages = $this->db->pages;
        $big_menu = ['?m=member&c=member&a=manage&menuid=72', L('member_research')];
    ...
    function manage()
    {
        ...
        $big_menu = [
            'javascript:window.top.art.dialog({id:\'add\',iframe:\'?m=member&c=member&a=add\', title:\'' . L('member_add') . '\', width:\'700\', height:\'500\', lock:true}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);',
            L('member_add'),
        ];
        include $this->admin_tpl('member_list');
    }
    ...
    function edit()
    {
        if (isset($_POST['dosubmit'])) {
            ...
            if ($status >= 0) {
                ...
                $this->db->update($info, ['userid' => $userid]);
                ...
                if ($userinfo) {
                    $this->db->update($modelinfo, ['userid' => $userid]);
                } else {
            ...
            $membermodelinfo = $this->db->get_one(['userid' => $userid]);
    ...
    function move()
    {
        if (isset($_POST['dosubmit'])) {
            ...
            $where = to_sqls($uidarr, '', 'userid');
            $this->db->update(['groupid' => $groupid], $where);
    ...
    function memberinfo()
    {
        ...
        if (!empty($userid)) {
            $memberinfo = $this->db->get_one(['userid' => $userid]);
        } elseif (!empty($username)) {
            $memberinfo = $this->db->get_one(['username' => $username]);
        } else {
        ...
        $this->db->set_model($modelid);
        $member_modelinfo = $this->db->get_one(['userid' => $userid]);
        //模型字段名称
