...
class content extends foreground
{
    ...
    public function publish()
    {
        ...
        $priv_db = new category_priv_model(); //加载栏目权限表数据模型
        ...
        if (isset($_POST['dosubmit'])) {
            ...
            if (!$priv_db->get_one(['catid' => $catid, 'roleid' => $memberinfo['groupid'], 'is_admin' => 0, 'action' => 'add'])) {
            ...
            $_POST['linkurl'] = str_replace(['"', '(', ')', ",", ' ', '%'], '', new_html_special_chars(strip_tags($_POST['linkurl'])));
            ...
            if ($setting['presentpoint'] < 0 && $memberinfo['point'] < abs($setting['presentpoint'])) {
                showmessage(L('points_less_than', ['point' => $memberinfo['point'], 'need_point' => abs($setting['presentpoint'])]),
            ...
            if ($info['status'] == 99) {
                ...
                if ($setting['presentpoint'] > 0) {
                    ...
                    receipts::point($setting['presentpoint'], $memberinfo['userid'], $memberinfo['username'], $flag, 'selfincome', L('contribute_add_point'),
                        $memberinfo['username']);
                } else {
            ...
            foreach ($model_cache as $modelid => $model) {
                if ($model['siteid'] == $siteid) {
                    $datas = [];
                    $this->content_db->set_model($modelid);
                    $datas = $this->content_db->select(['username' => $memberinfo['username'], 'sysadd' => 0], 'id,catid,title,url,username,sysadd,inputtime,status', 100,
                        'id DESC');
                    if ($datas) {
            ...
            foreach ($CATEGORYS as $catid => $cat) {
                if ($cat['siteid'] == $siteid && $cat['child'] == 0 && $cat['type'] == 0 && $priv_db->get_one([
                        'catid'    => $catid,
    ...
    public function published()
    {
        ...
        $infos = $this->content_check_db->listinfo(['username' => $_username, 'siteid' => $siteid], 'inputtime DESC', $page);
        $datas = [];
        foreach ($infos as $_v) {
        ...
        if (isset($_POST['dosubmit'])) {
            ...
            if (isset($_GET['catid']) && $_GET['catid']) {
                ...
                if ($category['type'] == 0) {
                    ...
                    $r = $this->content_db->get_one(['id' => $id, 'username' => $_username, 'sysadd' => 0]);
                    ...
                    $r2 = $this->content_db->get_one(['id' => $id]);
    ...
    public function delete()
    {
        ...
        if (!$array) {
            ...
            $check_pushed_db->delete(['checkid' => $checkid]);//删除对应投稿表
    ...
    function info_top()
    {
        ...
        foreach ($toptype_posid as $_k => $_v) {
            if ($pos_data->get_one(['id' => $id, 'catid' => $catid, 'posid' => $_v])) {
                $exist_posids[$_k] = 1;
            }
        }
    ...
    public function upload_video()
    {
        ...
        if (isset($_POST['dosubmit'])) {
            ...
            $_POST['info'][$field] = 1;
            $_POST[$field . '_video'] = [1 => ['title' => $_POST['info']['title'], 'vid' => $_POST['vid'], 'listorder' => 1]];
            unset($_POST['vid']);
            $this->publish();
        } else {
            ...
            if (is_array($categorys) && !empty($categorys)) {
                ...
                foreach ($categorys as $cid => $c) {
                    if ($c['child'] == 0 && $c['type'] == 0 && !$priv_db->get_one(['catid' => $cid, 'roleid' => $memberinfo['groupid'], 'is_admin' => 0, 'action' => 'add'])) {
                        unset($categorys[$cid]);
                    }
                }
                if (empty($categorys)) {
