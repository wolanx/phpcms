...
use phpcms\model\member_verify_model;
use phpcms\modules\member\classes\foreground;
use phpcms\modules\member\fields\member_form;

/**
 * 会员前台管理中心、账号管理、收藏操作类
 * @property member_verify_model $verify_db
 */
class index extends foreground
{
    ...
    public function register()
    {
        ...
        if (isset($_POST['dosubmit'])) {
            ...
            if ($member_setting['enablemailcheck']) {    //是否需要邮件验证
                ...
                $this->verify_db = new member_verify_model();
            ...
            if (pc_base::load_config('system', 'phpsso')) {
                ...
                if ($status > 0) {
                    ...
                    if ($member_setting['enablemailcheck']) {
                        ...
                        $message = str_replace(['{click}', '{url}', '{username}', '{email}', '{password}'],
                            ['<a href="' . $url . '">' . L('please_click') . '</a>', $url, $userinfo['username'], $userinfo['email'], $password], $message);
            ...
            if (!empty($_GET['verify'])) {
                ...
                $this->db->update(['groupid' => $this->_get_usergroup_bypoint()], ['userid' => $userid]);
    ...
    public function send_newmail()
    {
        ...
        $r = $this->db->get_one(['userid' => intval($_userid)]);
        ...
        $message = $member_setting['registerverifymessage'];
        $message = str_replace(['{click}', '{url}', '{username}', '{email}', '{password}'],
            ['<a href="' . $url . '">' . L('please_click') . '</a>', $url, $_username, $newemail, $password], $message);

        if (sendmail($newemail, L('reg_verify_email'), $message)) {
            //更新新的邮箱，用来验证
            $this->db->update(['email' => $newemail], ['userid' => $_userid]);
    ...
    public function account_manage()
    {
        ...
        $member_modelinfo_arr = $this->db->get_one(['userid' => $this->memberinfo['userid']]);
    ...
    public function account_manage_avatar()
    {
        ...
        $auth_data = $this->client->auth_data(['uid' => $this->memberinfo['phpssouid'], 'sys_auth_time' => microtime(true)], '', $ps_auth_key);
    ...
    public function account_manage_info()
    {
        if (isset($_POST['dosubmit'])) {
            ...
            if ($nickname) {
                $this->db->update(['nickname' => $nickname], ['userid' => $this->memberinfo['userid']]);
                if (!isset($cookietime)) {
                    $get_cookietime = param::get_cookie('cookietime');
                }
                $_cookietime = $cookietime ? intval($cookietime) : ($get_cookietime ? $get_cookietime : 0);
            ...
            $this->db->set_model($this->memberinfo['modelid']);
            $membermodelinfo = $this->db->get_one(['userid' => $this->memberinfo['userid']]);
            if (!empty($membermodelinfo)) {
                $this->db->update($modelinfo, ['userid' => $this->memberinfo['userid']]);
            } else {
    ...
    public function account_manage_password()
    {
        if (isset($_POST['dosubmit'])) {
            ...
            $this->db->update($updateinfo, ['userid' => $this->memberinfo['userid']]);
            if (pc_base::load_config('system', 'phpsso')) {
                //初始化phpsso
                $this->_init_phpsso();
                $res = $this->client->ps_member_edit('', $email, $_POST['info']['password'], $_POST['info']['newpassword'], $this->memberinfo['phpssouid'],
                    $this->memberinfo['encrypt']);
                $message_error = [
                    '-1' => L('user_not_exist'),
                    '-2' => L('old_password_incorrect'),
                    '-3' => L('email_already_exist'),
                    '-4' => L('email_error'),
                    '-5' => L('param_error'),
                ];
                if ($res < 0) {
                    showmessage($message_error[$res]);
                }
            }
        ...
        if (isset($_POST['dosubmit'])) {
            ...
            if ($mobile) {
                ...
                if ($r && $r['id_code'] == $mobile_verify) {
                    $sms_report_db->update(['id_code' => ''], $where);
                    $this->db->update(['mobile' => $mobile], ['userid' => $memberinfo['userid']]);
    ...
    public function account_manage_upgrade()
    {
        ...
        if (isset($_POST['dosubmit'])) {
            ...
            //消费类型，包年、包月、包日，价格
            $typearr = [$grouplist[$groupid]['price_y'], $grouplist[$groupid]['price_m'], $grouplist[$groupid]['price_d']];
            //消费类型，包年、包月、包日，时间
            ...
            if ($memberinfo['amount'] >= $cost) {
                $this->db->update(['groupid' => $groupid, 'overduedate' => $overduedate, 'vip' => 1], ['userid' => $memberinfo['userid']]);
                //消费记录
        ...
        if (isset($_POST['dosubmit'])) {
            ...
            if (pc_base::load_config('system', 'phpsso')) {
                ...
                if (isset($memberinfo['uid'])) {
                    //查询帐号
                    $r = $this->db->get_one(['phpssouid' => $memberinfo['uid']]);
                    if (!$r) {
                        //插入会员详细信息，会员不存在 插入会员
                        $info = [
                ...
                $rtime = $this->times_db->get_one(['username' => $username]);
                if ($rtime['times'] > 4) {
                ...
                if ($r['password'] != $password) {
                    ...
                    if ($rtime && $rtime['times'] < 5) {
                        ...
                        $this->times_db->update(['ip' => $ip, 'times' => '+=1'], ['username' => $username]);
                    } else {
                        $this->times_db->insert(['username' => $username, 'ip' => $ip, 'logintime' => SYS_TIME, 'times' => 1]);
                        $times = 5;
                    }
                    showmessage(L('password_error', ['times' => $times]), 'index.php?m=member&c=index&a=login', 3000);
                }
            ...
            $updatearr = ['lastip' => ip(), 'lastdate' => SYS_TIME];
            //vip过期，更新vip和会员组
            ...
            if ($r['point'] >= 0 && !in_array($r['groupid'], ['1', '7', '8']) && empty($r[vip])) {
                $grouplist = getcache('grouplist');
            ...
            $this->db->update($updatearr, ['userid' => $userid]);
    ...
    public function favorite()
    {
        ...
        if (isset($_GET['id']) && trim($_GET['id'])) {
            $this->favorite_db->delete(['userid' => $memberinfo['userid'], 'id' => intval($_GET['id'])]);
            showmessage(L('operation_success'), HTTP_REFERER);
        } else {
            $page = isset($_GET['page']) && trim($_GET['page']) ? intval($_GET['page']) : 1;
            $favoritelist = $this->favorite_db->listinfo(['userid' => $memberinfo['userid']], 'id DESC', $page, 10);
    ...
    public function friend()
    {
        ...
        if (isset($_GET['friendid'])) {
            $this->friend_db->delete(['userid' => $memberinfo['userid'], 'friendid' => intval($_GET['friendid'])]);
            showmessage(L('operation_success'), HTTP_REFERER);
        } else {
            //初始化phpsso
            $phpsso_api_url = $this->_init_phpsso();
            ...
            $friendids = $this->friend_db->listinfo(['userid' => $memberinfo['userid']], '', $page, 10);
    ...
    public function change_credit()
    {
        ...
        if (isset($_POST['dosubmit'])) {
            ...
            if ($status == 1) {
                if ($from == 1) {
                    $this->db->update(['point' => "-=$fromvalue"], ['userid' => $memberinfo['userid']]);
                } elseif ($from == 2) {
                    $this->db->update(['amount' => "-=$fromvalue"], ['userid' => $memberinfo['userid']]);
                }
                showmessage(L('operation_success'), HTTP_REFERER);
            } else {
                showmessage(L('operation_failure'), HTTP_REFERER);
            ...
            $this->db->update(['point' => "+=$point"], ['userid' => $memberinfo['userid']]);
            //加入消费记录，同时扣除金钱
            pc_base::load_app_class('spend', 'pay', 0);
    ...
    protected function _checkname($username)
    {
        ...
        if ($this->db->get_one(['username' => $username])) {
            return false;
        }
    ...
    public function public_checkname_ajax()
    {
        ...
        if ($this->verify_db->get_one(['username' => $username])) {
            exit('0');
        }
    ...
    public function public_checknickname_ajax()
    {
        ...
        //首先判断会员审核表
        $this->verify_db = pc_base::load_model('member_verify_model');
        if ($this->verify_db->get_one(['nickname' => $nickname])) {
            exit('0');
        }
    ...
    public function public_sina_login()
    {
        ...
        if (isset($_GET['callback']) && trim($_GET['callback'])) {
            ...
            if (!empty($me['id'])) {
                ...
                $where = ['connectid' => $me['id'], 'from' => 'sina'];
                ...
                if (!empty($r)) {
                    ...
                    $username = $r['username'];
                    $nickname = empty($r['nickname']) ? $username : $r['nickname'];
                    $this->db->update(['lastip' => ip(), 'lastdate' => SYS_TIME, 'nickname' => $me['name']], ['userid' => $userid]);
    ...
    public function public_snda_login()
    {
        ...
        if (isset($_GET['callback']) && trim($_GET['callback'])) {
            ...
            if (!empty($userid)) {
                ...
                //检查connect会员是否绑定，已绑定直接登录，未绑定提示注册/绑定页面
                $where = ['connectid' => $userid, 'from' => 'snda'];
                $r = $this->db->get_one($where);
    ...
    public function public_qq_loginnew()
    {
        ...
        if (!isset($_GET['code'])) {
            ...
            if (!empty($openid)) {
                $r = $this->db->get_one(['connectid' => $openid, 'from' => 'qq']);
    ...
    public function public_qq_login()
    {
        ...
        if (isset($_GET['callback']) && trim($_GET['callback'])) {
            ...
            if (!empty($_SESSION['last_key']['name'])) {
                //检查connect会员是否绑定，已绑定直接登录，未绑定提示注册/绑定页面
                $where = ['connectid' => $_REQUEST['openid'], 'from' => 'qq'];
                $r = $this->db->get_one($where);
            ...
            $keys = $o->getRequestToken(['callback' => $oauth_callback]);
            $_SESSION['keys'] = $keys;
    ...
    public function public_forget_password()
    {
        ...
        if (isset($_POST['dosubmit'])) {
            ...
            $memberinfo = $this->db->get_one(['email' => $_POST['email']]);
            ...
            $message = str_replace(['{click}', '{url}'], ['<a href="' . $url . '">' . L('please_click') . '</a>', $url], $message);
            //获取站点名称
            ...
            if (is_array($code) && is_numeric($code[0]) && date('y-m-d h', SYS_TIME) == date('y-m-d h', $code[1])) {
                $memberinfo = $this->db->get_one(['userid' => $code[0]]);
                ...
                $this->db->update($updateinfo, ['userid' => $code[0]]);
                if (pc_base::load_config('system', 'phpsso')) {
                    //初始化phpsso
                    $this->_init_phpsso();
    ...
    public function public_changepwd_bymobile()
    {
        ...
        if (!empty($this->sms_setting_arr[$siteid])) {
            ...
            $this->sms_setting = ['userid' => '', 'productid' => '', 'sms_key' => ''];
        }
        ...
        $array = $this->db->get_one(['mobile' => $phone, 'username' => $username]);
        if (empty($array)) {
            echo 1;
        } else {
            $result = $this->db->update(['password' => $newpwd], ['mobile' => $phone, 'username' => $username]);
            if ($result) {
                //修改成功，发送短信给用户回执
                //检查短信余额
    ...
    public function public_forget_password_mobile()
    {
        ...
        if (isset($_POST['dosubmit']) && $step == 2) {
            ...
            $r = $this->db->get_one(['username' => $username], 'userid,mobile');
            if ($r['mobile'] == '') {
                $_SESSION['mobile'] = '';
                $_SESSION['userid'] = '';
                $_SESSION['code'] = '';
            ...
            if ($mobile) {
                ...
                if ($r && $r['id_code'] == $mobile_verify) {
                    ...
                    $this->db->update($updateinfo, ['userid' => $userid]);
                    $rs = $this->db->get_one(['userid' => $userid], 'phpssouid');
                    if (pc_base::load_config('system', 'phpsso')) {
                        //初始化phpsso
                        $this->_init_phpsso();
    ...
    public function public_forget_password_username()
    {
        ...
        if (isset($_POST['dosubmit']) && $step == 2) {
            ...
            $r = $this->db->get_one(['username' => $username], 'userid,email');
            if ($r['email'] == '') {
                $_SESSION['userid'] = '';
                $_SESSION['code'] = '';
