...
use phpcms\model\member_menu_model;
use phpcms\modules\admin\classes\admin;

/**
 * 管理员后台会员中心菜单管理类
 * @property member_menu_model $db
 */
class member_menu extends admin
{
    ...
    public function manage()
    {
        ...
        foreach ($result as $r) {
            ...
            $r['str_manage'] = '<a href="?m=member&c=member_menu&a=edit&id=' . $r['id'] . '&menuid=' . $_GET['menuid'] . '">' . L('edit') . '</a> | <a href="javascript:confirmurl(\'?m=member&c=member_menu&a=delete&id=' . $r['id'] . '&menuid=' . $_GET['menuid'] . '\',\'' . L('confirm',
    ...
    function add()
    {
        if (isset($_POST['dosubmit'])) {
            ...
            //开发过程中用于自动创建语言包
            $file = PATH_PHPCMS . 'languages' . DS . 'zh-cn' . DS . 'member_menu.lang.php';
            if (file_exists($file)) {
    ...
    function delete()
    {
        ...
        $menu = $this->db->get_one(["id" => $_GET['id']]);
        if (!$menu) {
            showmessage('菜单不存在！请返回！', HTTP_REFERER);
        }
