...
class member_model extends admin
{
    ...
    function manage()
    {
        ...
        $member_model_list = $this->db->listinfo(['type' => 2, 'siteid' => $this->get_siteid()], 'sort', $page, 10);
        $pages = $this->db->pages;
        $big_menu = [
            'javascript:window.top.art.dialog({id:\'add\',iframe:\'?m=member&c=member_model&a=add\', title:\'' . L('add_model') . '\', width:\'700\', height:\'500\', lock:true}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);',
            L('member_model_add'),
        ];
    ...
    function edit()
    {
        if (isset($_POST['dosubmit'])) {
            ...
            $this->db->update($info, ['modelid' => $modelid]);
            ...
            $show_header = $show_scroll = true;
            $modelinfo = $this->db->get_one(['modelid' => $_GET['modelid']]);
    ...
    function delete()
    {
        ...
        if ($this->db->delete($where)) {
            ...
            $this->member_db->update(['modelid' => 10], $where);
    ...
    function move()
    {
        if (isset($_POST['dosubmit'])) {
            ...
            $show_header = $show_scroll = true;
            $modelarr = $this->db->select(['type' => 2]);
    ...
    function sort()
    {
        if (isset($_POST['sort'])) {
            foreach ($_POST['sort'] as $k => $v) {
                $this->db->update(['sort' => $v], ['modelid' => $k]);
            }
    ...
    public function public_checkmodelname_ajax()
    {
        ...
        $status = $this->db->get_one(['name' => $modelname]);
        if ($status) {
            exit('0');
        } else {
            exit('1');
