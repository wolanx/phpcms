...
use phpcms\modules\member\classes\member_cache;
...
class member_modelfield extends admin
{
    ...
    public function manage()
    {
        ...
        $modelinfo = $this->model_db->get_one(['modelid' => $modelid]);
        $big_menu = [
            'javascript:window.top.art.dialog({id:\'add\',iframe:\'?m=member&c=member_modelfield&a=add&modelid=' . $modelinfo['modelid'] . '\', title:\'' . L('member_modelfield_add') . ' ' . L('model_name') . '：' . $modelinfo['name'] . '\', width:\'700\', height:\'500\', lock:true}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);',
            L('member_modelfield_add'),
        ];
    ...
    public function edit()
    {
        if (isset($_POST['dosubmit'])) {
            ...
            $_POST['info']['unsetgroupids'] = isset($_POST['unsetgroupids']) ? implode(',', $_POST['unsetgroupids']) : '';
            $_POST['info']['unsetroleids'] = isset($_POST['unsetroleids']) ? implode(',', $_POST['unsetroleids']) : '';
            $this->db->update($_POST['info'], ['fieldid' => $fieldid]);
            $this->cache_field($modelid);
    ...
    public function delete()
    {
        ...
        //删除模型字段
        $this->db->delete(['fieldid' => $fieldid]);
    ...
    public function disable()
    {
        ...
        $this->db->update(['disabled' => $disabled], ['fieldid' => $fieldid]);
        showmessage(L('operation_success'), HTTP_REFERER);
    }
    ...
    {
        $field_array = [];
        $fields = $this->db->select(['modelid' => $modelid], '*', 100, 'listorder ASC');
