...
class member_verify extends admin
{
    ...
    {
        parent::__construct();
        $this->db = new member_verify_model();
        $this->_init_phpsso();
    }
    ...
    function modelinfo()
    {
        ...
        //模型字段名称
        $this->member_field_db = new sitemodel_field_model();
        $model_fieldinfo = $this->member_field_db->select(['modelid' => $modelid], "*", 100);
        //用户模型字段信息
    ...
    function reject()
    {
        if (isset($_POST['userid'])) {
            $uidarr = isset($_POST['userid']) ? $_POST['userid'] : showmessage(L('illegal_parameters'), HTTP_REFERER);
            $where = to_sqls($uidarr, '', 'userid');
            $res = $this->db->update(['status' => 4, 'message' => $_POST['message']], $where);
            //发送 email通知
            if ($res) {
                if ($_POST['sendemail']) {
                    $memberinfo = $this->db->select($where);
    ...
    function ignore()
    {
        if (isset($_POST['userid'])) {
            $uidarr = isset($_POST['userid']) ? $_POST['userid'] : showmessage(L('illegal_parameters'), HTTP_REFERER);
            $where = to_sqls($uidarr, '', 'userid');
            $res = $this->db->update(['status' => 2, 'message' => $_POST['message']], $where);
            //发送 email通知
            if ($res) {
                if ($_POST['sendemail']) {
                    $memberinfo = $this->db->select($where);
