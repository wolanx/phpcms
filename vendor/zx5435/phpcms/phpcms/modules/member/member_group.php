...
use phpcms\model\member_group_model;
...
class member_group extends admin
{
    ...
    {
        parent::__construct();
        $this->db = new member_group_model();
    }
    ...
    function manage()
    {
        ...
        $this->member_db = new member_model();
        //TODO 此处循环中执行sql，会严重影响效率，稍后考虑在memebr_group表中加入会员数字段和统计会员总数功能解决。
        foreach ($member_group_list as $k => $v) {
            $membernum = $this->member_db->count(['groupid' => $v['groupid']]);
            $member_group_list[$k]['membernum'] = $membernum;
        }
        $pages = $this->db->pages;

        $big_menu = [
            'javascript:window.top.art.dialog({id:\'add\',iframe:\'?m=member&c=member_group&a=add\', title:\'' . L('member_group_add') . '\', width:\'700\', height:\'500\', lock:true}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);',
            L('member_group_add'),
        ];
    ...
    function edit()
    {
        if (isset($_POST['dosubmit'])) {
            ...
            $this->db->update($info, ['groupid' => $info['groupid']]);
            ...
            $groupinfo = $this->db->get_one(['groupid' => $groupid]);
    ...
    function sort()
    {
        if (isset($_POST['sort'])) {
            foreach ($_POST['sort'] as $k => $v) {
                $this->db->update(['sort' => $v], ['groupid' => $k]);
            }
    ...
    private function _checkname($name = null)
    {
        ...
        if ($this->db->get_one(['name' => $name], 'groupid')) {
            return false;
        }
