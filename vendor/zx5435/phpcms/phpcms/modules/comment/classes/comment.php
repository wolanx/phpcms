...
namespace phpcms\modules\comment\classes;
...
class comment
{
    ...
    public function add($commentid, $siteid, $data, $id = '', $title = '', $url = '')
    {
        ...
        if (!$comment = $this->comment_db->get_one(['commentid' => $commentid, 'siteid' => $siteid], 'tableid, commentid')) { //评论不存在
            //取得当前可以使用的内容数据表
            ...
            //新建评论到评论总表中。
            $comment_data = ['commentid' => $commentid, 'siteid' => $siteid, 'tableid' => $tableid, 'display_type' => ($data['direction'] > 0 ? 1 : 0)];
            if (!empty($title)) {
        ...
        if ($id) {
            $r = $this->comment_data_db->get_one(['id' => $id]);
            if ($r) {
                pc_base::load_sys_class('format', '', 0);
                if ($r['reply']) {
                    $data['content'] = '<div class="content">' . str_replace('<span></span>',
                            '<span class="blue f12">' . $r['username'] . ' ' . L('chez') . ' ' . format::date($r['creat_at'], 1) . L('release') . '</span>',
                            $r['content']) . '</div><span></span>' . $data['content'];
                } else {
                    $data['content'] = '<div class="content"><span class="blue f12">' . $r['username'] . ' ' . L('chez') . ' ' . format::date($r['creat_at'],
                            1) . L('release') . '</span><pre>' . $r['content'] . '</pre></div><span></span>' . $data['content'];
                }
                $data['reply'] = 1;
            }
        }
        ...
        if ($comment_data_id = $this->comment_data_db->insert($data, true)) {
            //需要审核，插入到审核表
            if ($data['status'] == 0) {
                $this->comment_check_db->insert(['comment_data_id' => $comment_data_id, 'siteid' => $siteid, 'tableid' => $tableid]);
            ...
            $this->comment_db->update($sql, ['commentid' => $commentid]);
            if ($site['check']) {
                $this->msg_code = 7;
            } else {
                $this->msg_code = 0;
    ...
    {
        if ($data = $this->comment_db->get_one(['commentid' => $commentid], 'tableid')) {
            $this->comment_data_db->table_name($data['tableid']);
            if ($this->comment_data_db->update(['support' => '+=1'], ['id' => $id])) {
                $this->msg_code = 0;
    ...
    {
        if (!$comment = $this->comment_db->get_one(['commentid' => $commentid], 'tableid, commentid')) {
            $this->msg_code = 6;
        ...
        if (!$comment_data = $this->comment_data_db->get_one(['id' => $id, 'commentid' => $commentid])) {
            $this->msg_code = 6;
        ...
        //读取评论的站点配置信息
        $site = $this->comment_setting_db->get_one(['siteid' => $comment_data['siteid']]);
        ...
        if ($status == 1) {//通过的时候
            ...
            $this->comment_db->update($sql, ['commentid' => $comment['commentid']]);
            //更新评论内容状态
            $this->comment_data_db->update(['status' => $status], ['id' => $id, 'commentid' => $commentid]);
            ...
            //删除存储表的数据
            $this->comment_data_db->delete(['id' => $id, 'commentid' => $commentid]);
        ...
        //删除审核表中的数据
        $this->comment_check_db->delete(['comment_data_id' => $id]);
    ...
    public function del($commentid, $siteid, $id, $catid)
    {
        ...
        for ($i = 1; ; $i++) {
            ...
            if ($this->comment_data_db->table_exists($table)) { //检查构建的表名是否存在，如果存在执行删除操作
                ...
                $this->comment_data_db->delete(['commentid' => $commentid]);
            } else { //不存在，则退出循环
                break;
            }
        }
        $this->comment_db->delete(['commentid' => $commentid]); //删除评论主表的内容
        return true;
    }
