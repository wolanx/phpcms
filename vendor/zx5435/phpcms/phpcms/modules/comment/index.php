...
class index
{
    ...
    public function init()
    {
        ...
        $setting = $comment_setting_db->get_one(['siteid' => $this->siteid]);
        //SEO
    ...
    public function post()
    {
        ...
        $data = ['userid' => $userid, 'username' => $username, 'content' => $content, 'direction' => $direction];
        $comment->add($this->commentid, $this->siteid, $data, $id, $title, $url);
        $this->_show_msg($comment->get_error() . "<iframe width='0' id='top_src' height='0' src='$domain/js.html?200'></iframe>", (in_array($comment->msg_code, [0, 7]) ? HTTP_REFERER : ''),
            (in_array($comment->msg_code, [0, 7]) ? 1 : 0));
    }
    ...
    public function ajax()
    {
        ...
        $comment = [];
        if ($comment = $pc_tag->get_comment(['commentid' => $commentid])) {
            $page = isset($_GET['page']) && intval($_GET['page']) ? intval($_GET['page']) : 1;
            $offset = ($page - 1) * $num;
            $data = ['commentid' => $commentid, 'site' => $siteid, 'limit' => $offset . ',' . $num, 'direction' => $direction];
    ...
    protected function _show_msg($msg, $url = '', $status = 0)
    {
        ...
        switch ($this->format) {
            ...
            case 'jsonp':
                $msg = pc_base::load_config('system', 'charset') == 'gbk' ? iconv('gbk', 'utf-8', $msg) : $msg;
                echo strip_tags($this->callback) . '(' . json_encode(['msg' => $msg, 'status' => $status]) . ')';
                exit;
                break;
