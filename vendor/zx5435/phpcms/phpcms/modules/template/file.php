...
namespace phpcms\modules\template;
...
class file extends admin
{
    ...
    {
        $this->style = isset($_GET['style']) && trim($_GET['style']) ? str_replace(['..\\', '../', './', '.\\', '/', '\\'], '', trim($_GET['style'])) : showmessage(L('illegal_operation'),
            HTTP_REFERER);
        if (empty($this->style)) {
            showmessage(L('illegal_operation'), HTTP_REFERER);
        }
        $this->filepath = PATH_PHPCMS . 'templates' . DS . $this->style . DS;
    ...
    public function init()
    {
        ...
        $local = str_replace([PATH_PHPCMS, DS . DS], ['', DS], $filepath);
        if (substr($local, -1, 1) == '.') {
            $local = substr($local, 0, (strlen($local) - 1));
        }
        $encode_local = str_replace(['/', '\\'], '|', $local);
    ...
    public function edit_file()
    {
        ...
        $dir = isset($_GET['dir']) && trim($_GET['dir']) ? str_replace(['..\\', '../', './', '.\\'], '', urldecode(trim($_GET['dir']))) : '';
        ...
        if ($file) {
            ...
            $file_t_v = [
                'header'   => [
                    '{$SEO[\'title\']}'       => L('seo_title'),
                    '{$SEO[\'site_title\']}'  => L('site_title'),
                    '{$SEO[\'keyword\']}'     => L('seo_keyword'),
                    '{$SEO[\'description\']}' => L('seo_des'),
                ],
                'category' => [
                    '{$catid}'      => L('cat_id'),
                    '{$catname}'    => L('cat_name'),
                    '{$url}'        => L('cat_url'),
                    '{$r[catname]}' => L('cat_name'),
                    '{$r[url]}'     => 'URL',
                    '{$CATEGORYS}'  => L('cats'),
                ],
                'list'     => ['{$catid}' => L('cat_id'), '{$catname}' => L('cat_name'), '{$url}' => L('cat_url'), '{$CATEGORYS}' => L('cats')],
                'show'     => [
                    '{$title}'                => L('title'),
                    '{$inputtime}'            => L('inputtime'),
                    '{$copyfrom}'             => L('comeform'),
                    '{$content}'              => L('content'),
                    '{$previous_page[url]}'   => L('pre_url'),
                    '{$previous_page[title]}' => L('pre_title'),
                    '{$next_page[url]}'       => L('next_url'),
                    '{$next_page[title]}'     => L('next_title'),
                ],
                'page'     => ['{$CATEGORYS}' => L('cats'), '{$content}' => L('content')],
            ];
        }
        if (substr($file, -4, 4) != 'html') {
            showmessage(L("can_edit_html_files"));
        }
        $filepath = $this->filepath . $dir . DS . $file;
        $is_write = 0;
    ...
    public function public_name()
    {
        ...
        $filepath = $this->filepath . $dir . DS . $name . '.html';
        if (file_exists($filepath)) {
            exit('0');
        } else {
            exit('1');
    ...
    {
        $dir = isset($_GET['dir']) && trim($_GET['dir']) ? str_replace(['..\\', '../', './', '.\\'], '', urldecode(trim($_GET['dir']))) : showmessage(L('illegal_operation'), HTTP_REFERER);
    ...
    public function edit_pc_tag()
    {
        ...
        $dir = isset($_GET['dir']) && trim($_GET['dir']) ? str_replace(['..\\', '../', './', '.\\'], '', urldecode(trim($_GET['dir']))) : showmessage(L('illegal_operation'));
        ...
        switch ($op) {
            ...
            case 'json':
                if ($_POST['dosubmit']) {
                    ...
                    $pc_tag = creat_pc_tag($op, ['url' => $url, 'cache' => $cache, 'return' => $return]);
            ...
            case 'get':
                if ($_POST['dosubmit']) {
                    ...
                    $return = isset($_POST['return']) && trim($_POST['return']) ? trim($_POST['return']) : '';
                    $tag_md5_list = tag_md5($filepath);
                    $pc_tag = creat_pc_tag($op, ['sql' => $sql, 'dbsource' => $dbsource, 'cache' => $cache, 'return' => $return]);
                    if (in_array($tag_md5, $tag_md5_list[0])) {
                        $old_pc_tag = $tag_md5_list[1][$tag_md5];
                    }
                    if (replace_pc_tag($filepath, $old_pc_tag, $pc_tag, $this->style, $dir)) {
            ...
            default:
                if (!file_exists(PATH_PHPCMS . DS . 'modules' . DS . $op . DS . 'classes' . DS . $op . '_tag.class.php')) {
