...
class style extends admin
{
    ...
    {
        $this->filepath = PATH_PHPCMS . 'templates' . DS;
        parent::__construct();
    }
    ...
    public function init()
    {
        ...
        $big_menu = [
            'javascript:window.top.art.dialog({id:\'import\',iframe:\'?m=template&c=style&a=import\', title:\'' . L('import_style') . '\', width:\'500\', height:\'250\', lock:true}, function(){var d = window.top.art.dialog({id:\'import\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'import\'}).close()});void(0);',
            L('import_style'),
        ];
    ...
    public function disable()
    {
        ...
        $filepath = $this->filepath . $style . DS . 'config.php';
        if (file_exists($filepath)) {
            ...
            $arr = ['name' => $style, 'disable' => 1, 'dirname' => $style];
            file_put_contents($filepath, '<?php return ' . var_export($arr, true) . ';?>');
        }
    ...
    public function import()
    {
        if (isset($_POST['dosubmit'])) {
            ...
            //echo $this->filepath.$code['dirname'].DS.'config.php';
            if (!file_exists($this->filepath . $code['dirname'] . DS . 'config.php')) {
                @mkdir($this->filepath . $code['dirname'] . DS, 0755, true);
                if (@is_writable($this->filepath . $code['dirname'] . DS)) {
                    @file_put_contents($this->filepath . $code['dirname'] . DS . 'config.php', '<?php return ' . var_export($code, true) . ';?>');
    ...
    public function updatename()
    {
        ...
        if (is_array($name)) {
            foreach ($name as $key => $val) {
                $filepath = $this->filepath . $key . DS . 'config.php';
                if (file_exists($filepath)) {
                    $arr = include $filepath;
                    $arr['name'] = $val;
                } else {
                    $arr = ['name' => $val, 'disable' => 0, 'dirname' => $key];
                }
                @file_put_contents($filepath, '<?php return ' . var_export($arr, true) . ';?>');
            }
            showmessage(L('operation_success'), HTTP_REFERER);
