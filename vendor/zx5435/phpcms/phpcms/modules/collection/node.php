...
namespace phpcms\modules\collection;

use pc_base;
use phpcms\model\attachment_index_model;
use phpcms\model\attachment_model;
use phpcms\model\collection_content_model;
use phpcms\model\collection_history_model;
use phpcms\model\collection_program_model;
...
class node extends admin
{
    ...
    function __construct()
    {
        ...
        $this->url_list_type = ['1' => L('sequence'), '2' => L('multiple_pages'), '3' => L('single_page'), '4' => 'RSS'];
    ...
    public function manage()
    {
        ...
        $nodelist = $this->db->listinfo(['siteid' => $this->get_siteid()], 'nodeid DESC', $page, 15);
        $pages = $this->db->pages;
    ...
    public function add()
    {
        ...
        if (isset($_POST['dosubmit'])) {
            ...
            if ($this->db->get_one(['name' => $data['name']])) {
    ...
    public function edit()
    {
        ...
        $data = $this->db->get_one(['nodeid' => $nodeid]);
        if (isset($_POST['dosubmit'])) {
            $datas = $data;
            unset($data);
            $data = isset($_POST['data']) ? $_POST['data'] : showmessage(L('illegal_parameters'), HTTP_REFERER);
            ...
            $data['customize_config'] = array2string($data['customize_config']);
            if ($this->db->update($data, ['nodeid' => $nodeid])) {
                showmessage(L('operation_success'), '?m=collection&c=node&a=manage');
            } else {
                showmessage(L('operation_failure'), HTTP_REFERER);
            }
    ...
    public function public_name()
    {
        ...
        $data = [];
        if ($nodeid) {
            $data = $this->db->get_one(['nodeid' => $nodeid], 'name');
            ...
            if ($total_page > 0) {
                ...
                if (is_array($url) && !empty($url)) {
                    foreach ($url as $v) {
                        ...
                        $md5 = md5($v['url']);
                        if (!$history_db->get_one(['md5' => $md5, 'siteid' => $this->get_siteid()])) {
                            $history_db->insert(['md5' => $md5, 'siteid' => $this->get_siteid()]);
                            $content_db->insert(['nodeid' => $nodeid, 'status' => 0, 'url' => $v['url'], 'title' => $v['title'], 'siteid' => $this->get_siteid()]);
                        } else {
                            $re++;
                        }
                    }
                ...
                $show_header = $show_dialog = true;
                if ($total_page <= $page) {
                    $this->db->update(['lastdate' => SYS_TIME], ['nodeid' => $nodeid]);
                }
    ...
    public function col_content()
    {
        ...
        if ($data = $this->db->get_one(['nodeid' => $nodeid])) {
            ...
            if (empty($total)) {
                $total = $content_db->count(['nodeid' => $nodeid, 'siteid' => $this->get_siteid(), 'status' => 0]);
            }
            $total_page = ceil($total / 2);
            $list = $content_db->select(['nodeid' => $nodeid, 'siteid' => $this->get_siteid(), 'status' => 0], 'id,url', '2', 'id desc');
            $i = 0;
            if (!empty($list) && is_array($list)) {
                foreach ($list as $v) {
                    ...
                    $content_db->update(['status' => 1, 'data' => array2string($html)], ['id' => $v['id']]);
                    $i++;
                }
            } else {
            ...
            if ($total_page > $page) {
                showmessage(L('collectioning') . ($i + ($page - 1) * 2) . '/' . $total . '<script type="text/javascript">location.href="?m=collection&c=node&a=col_content&page=' . ($page + 1) . '&nodeid=' . $nodeid . '&total=' . $total . '&pc_hash=' . $_SESSION['pc_hash'] . '"</script>',
                    '?m=collection&c=node&a=col_content&page=' . ($page + 1) . '&nodeid=' . $nodeid . '&total=' . $total);
            } else {
                $this->db->update(['lastdate' => SYS_TIME], ['nodeid' => $nodeid]);
    ...
    {
        $nodeid = isset($_GET['nodeid']) ? intval($_GET['nodeid']) : showmessage(L('illegal_parameters'), HTTP_REFERER);
        $node = $this->db->get_one(['nodeid' => $nodeid], 'name');
        $content_db = pc_base::load_model('collection_content_model');
        $status = isset($_GET['status']) ? intval($_GET['status']) : '';
        $page = isset($_GET['page']) ? intval($_GET['page']) : 1;
        $sql = ['nodeid' => $nodeid, 'siteid' => $this->get_siteid()];
        if ($status) {
            $sql['status'] = $status - 1;
        }
    ...
    public function import()
    {
        ...
        $program_db = new collection_program_model();
        $program_list = $program_db->select(['nodeid' => $nodeid, 'siteid' => $this->get_siteid()], 'id, catid');
    ...
    public function content_del()
    {
        ...
        if (is_array($id)) {
            $collection_content_db = new collection_content_model();
            $history_db = new collection_history_model();
            $del_array = $id;
            $ids = implode('\',\'', $id);
            if ($history) {
    ...
    public function import_program_add()
    {
        ...
        include dirname(__FILE__) . DS . 'spider_funs' . DS . 'config.php';

        //读取栏目缓存
        $catlist = getcache('category_content_' . $this->siteid, 'commons');
        $cat = $catlist[$catid];
        $cat['setting'] = string2array($cat['setting']);
        if ($cat['siteid'] != $this->get_siteid() || $cat['type'] != 0) {
            showmessage(L('illegal_section_parameter'), HTTP_REFERER);
        }
        ...
        if (isset($_POST['dosubmit'])) {
            ...
            $data = ['config' => array2string($config), 'siteid' => $this->get_siteid(), 'nodeid' => $nodeid, 'modelid' => $cat['modelid'], 'catid' => $catid];
            $program_db = pc_base::load_model('collection_program_model');
            if ($id = $program_db->insert($data, true)) {
                showmessage(L('program_add_operation_success'),
                    '?m=collection&c=node&a=import_content&programid=' . $id . '&nodeid=' . $nodeid . '&ids=' . $ids . '&type=' . $type);
            } else {
        ...
        if (empty($model)) {
            showmessage(L('model_does_not_exist_please_update_the_cache_model'));
        }
        $node_data = $this->db->get_one(['nodeid' => $nodeid], "customize_config");
        $node_data['customize_config'] = string2array($node_data['customize_config']);
        $node_field = ['' => L('please_choose'), 'title' => L('title'), 'author' => L('author'), 'comeform' => L('comeform'), 'time' => L('time'), 'content' => L('content')];
    ...
    public function import_content()
    {
        ...
        $ids = isset($_GET['ids']) ? $_GET['ids'] : '';
        $type = isset($_GET['type']) ? trim($_GET['type']) : '';
        if (!$node = $this->db->get_one(['nodeid' => $nodeid], 'coll_order,content_page')) {
        ...
        $attach_status = false;
        if (pc_base::load_config('system', 'attachment_stat')) {
            $attachment_db = new attachment_model();
        ...
        if ($type == 'all') {
            ...
            if (empty($total)) {
                $total = $collection_content_db->count(['siteid' => $this->get_siteid(), 'nodeid' => $nodeid, 'status' => 1]);
            }
            $total_page = ceil($total / 20);
            $page = isset($_GET['page']) && intval($_GET['page']) ? intval($_GET['page']) : 1;
            $total_page = ceil($total / 20);
            $data = $collection_content_db->select(['siteid' => $this->get_siteid(), 'nodeid' => $nodeid, 'status' => 1], 'id, data', '20', $order);
        ...
        $program = $program_db->get_one(['id' => $programid]);
        ...
        //加载所有的处理函数
        $funcs_file_list = glob(dirname(__FILE__) . DS . 'spider_funs' . DS . '*.php');
        foreach ($funcs_file_list as $v) {
            include $v;
        }
        foreach ($data as $k => $v) {
            $sql = ['catid' => $program['catid'], 'status' => $program['config']['content_status']];
            ...
            if ($contentid) {
                ...
                //更新附件状态,将采集关联重置到内容关联
                if ($attach_status) {
                    $datas = $att_index_db->select(['keyid' => 'cj-' . $v['id']], '*', 100, '', '', 'aid');
                    if (!empty($datas)) {
                        $datas = array_keys($datas);
                        $datas = implode(',', $datas);
                        $att_index_db->update(['keyid' => 'c-' . $program['catid'] . '-' . $contentid], ['keyid' => 'cj-' . $v['id']]);
                        $attachment_db->update(['module' => 'content'], " aid IN ($datas)");
                    }
                }
            } else {
                $collection_content_db->delete(['id' => $v['id']]);
            }
        }
        $sql_id = implode('\',\'', $coll_contentid);
        $collection_content_db->update(['status' => 2], " id IN ('$sql_id')");
