...
<script>
    function test_spider(id) {
        ...
        window.top.art.dialog({
            ...
            id: 'test',
            iframe: '?m=collection&c=node&a=public_test&nodeid=' + id,
            width: '700',
            height: '500'
        }, '', function () {
            window.top.art.dialog({id: 'test'}).close()
    ...
    function copy_spider(id) {
        window.top.art.dialog({id: 'test'}).close();
        window.top.art.dialog({title: '<?php echo L('copy_node')?>', id: 'test', iframe: '?m=collection&c=node&a=copy&nodeid=' + id, width: '420', height: '120'}, function () {
            var d = window.top.art.dialog({id: 'test'}).data.iframe;
    ...
    function import_spider() {
        ...
        window.top.art.dialog({
            ...
            id: 'test',
            iframe: '?m=collection&c=node&a=node_import',
            width: '420',
            height: '200'
        }, function () {
            var d = window.top.art.dialog({id: 'test'}).data.iframe;
