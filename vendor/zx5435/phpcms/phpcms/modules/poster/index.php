...
class index
{
    ...
    public function poster_click()
    {
        ...
        if ($id) {
            ...
            $this->s_db->insert([
                ...
                'area'      => $area,
                'ip'        => $ip,
                'referer'   => safe_replace(HTTP_REFERER),
                'clicktime' => SYS_TIME,
                'type'      => 1,
            ]);
        }
        $this->db->update(['clicks' => '+=1'], ['id' => $id]);
    ...
    public function show_poster()
    {
        ...
        $now = SYS_TIME;
        $siteid = get_siteid();
        $r = $sdb->get_one(['siteid' => $siteid, 'spaceid' => $id]);
        if (!$r) {
            exit();
        }
        if ($r['setting']) {
    ...
    protected function show_stat($siteid = 0, $spaceid = 0, $id = 0)
    {
        ...
        if (isset($M[$siteid]['enablehits']) && $M[$siteid]['enablehits'] == 0) {
            return true;
        }
        ...
        if (!$siteid || !$spaceid) {
            $r = $this->db->get_one(['id' => $id], 'siteid, spaceid');
            $siteid = $r['id'];
            $spaceid = $r['spaceid'];
        }
        $ip = ip();
