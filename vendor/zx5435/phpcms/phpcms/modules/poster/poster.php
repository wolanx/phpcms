...
class poster extends admin
{
    ...
    public function init()
    {
        ...
        $infos = $this->db->listinfo(['spaceid' => $spaceid, 'siteid' => $this->get_siteid()], '`listorder` ASC, `id` DESC', $page);
        pc_base::load_sys_class('format', '', 0);
        $types = ['images' => L('photo'), 'flash' => L('flash'), 'text' => L('title')];
    ...
    public function add()
    {
        if (isset($_POST['dosubmit'])) {
            ...
            if ($id) {
                $this->s_db->update(['items' => '+=1'], ['spaceid' => $poster['spaceid'], 'siteid' => $this->get_siteid()]);
            ...
            $sinfo = $this->s_db->get_one(['spaceid' => $spaceid, 'siteid' => $this->get_siteid()], 'name, type');
    ...
    public function edit()
    {
        ...
        if (isset($_POST['dosubmit'])) {
            ...
            $setting = $this->check_setting($_POST['setting'], $poster['type']);
            $poster['setting'] = array2string($setting);
            $this->db->update($poster, ['id' => $_GET['id'], 'siteid' => $this->get_siteid()]);
            ...
            $info = $this->db->get_one(['id' => $_GET['id'], 'siteid' => $this->get_siteid()]);
            $sinfo = $this->s_db->get_one(['spaceid' => $info['spaceid'], 'siteid' => $this->get_siteid()], 'name, type');
            $setting = $this->get_setting($sinfo['type']);
            $TYPES = get_types();
    ...
    public function listorder()
    {
        if (isset($_POST['listorder']) && is_array($_POST['listorder'])) {
            ...
            foreach ($listorder as $k => $v) {
                ...
                $this->db->update(['listorder' => $v], ['id' => $k]);
            }
        }
    ...
    private function create_js($id = 0)
    {
        ...
        if (!$html->create_js($id)) {
            showmessge($html->msg, HTTP_REFERER);
        }
    ...
    public function public_approval()
    {
        if (!isset($_POST['id']) || !is_array($_POST['id'])) {
            ...
            array_map([$this, _approval], $_POST['id']);
        }
        showmessage(L('operation_success'), HTTP_REFERER);
    }
    ...
    private function _approval($id = 0)
    {
        ...
        $this->db->update(['disabled' => $_GET['passed']], ['id' => $id, 'siteid' => $this->get_siteid()]);
    ...
    private function _del($id = 0)
    {
        ...
        $r = $this->db->get_one(['id' => $id, 'siteid' => $this->get_siteid()], 'spaceid');
        $this->s_db->update(['items' => '-=1'], ['spaceid' => $r['spaceid'], 'siteid' => $this->get_siteid()]);
        $this->db->delete(['id' => $id, 'siteid' => $this->get_siteid()]);
        if (pc_base::load_config('system', 'attachment_stat')) {
            $this->attachment_db = pc_base::load_model('attachment_model');
            $keyid = 'poster-' . $id;
    ...
    public function stat()
    {
        ...
        $_GET['id'] = intval($_GET['id']);
        $info = $this->db->get_one(['id' => $_GET['id']], 'spaceid');
        if (!$_GET['id']) {
    ...
    private function check($data)
    {
        if (!isset($data['name']) || empty($data['name'])) {
            showmessage(L('adsname_no_empty'), HTTP_REFERER);
        }
        if (!isset($data['type']) || empty($data['type'])) {
            showmessage(L('no_ads_type'), HTTP_REFERER);
        }
        ...
        if ($data['startdate'] >= $data['enddate']) {
            $data['enddate'] = strtotime('next month', $data['startdate']);
        }
    ...
    public function public_check_poster()
    {
        ...
        $r = $this->db->get_one(['siteid' => $this->get_siteid(), 'name' => $_GET['name']], 'id');
        if ($r['id']) {
            exit('0');
        } else {
            exit('1');
