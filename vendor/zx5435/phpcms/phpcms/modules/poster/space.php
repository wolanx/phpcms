...
class space extends admin
{
    ...
    function __construct()
    {
        ...
        $this->db = new poster_space_model();
    }
    ...
    public function init()
    {
        ...
        $infos = $this->db->listinfo(['siteid' => $this->get_siteid()], '`spaceid`', $page);
        $pages = $this->db->pages;
        $big_menu = [
            'javascript:window.top.art.dialog({id:\'add\',iframe:\'?m=poster&c=space&a=add\', title:\'' . L('add_space') . '\', width:\'540\', height:\'320\'}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);',
            L('add_space'),
        ];
    ...
    public function add()
    {
        if (isset($_POST['dosubmit'])) {
            ...
            if ($spaceid) {
                ...
                $this->db->update(['path' => $path], ['siteid' => $this->get_siteid(), 'spaceid' => $spaceid]);
    ...
    public function edit()
    {
        ...
        if (isset($_POST['dosubmit'])) {
            ...
            if (isset($_POST['old_type']) && $_POST['old_type'] != $space['type']) {
                ...
                $poster_db->delete(['spaceid' => $_GET['spaceid']]);
                $space['items'] = 0;
            }
            if ($this->db->update($space, ['spaceid' => $_GET['spaceid']])) {
                showmessage(L('edited_successful'), '?m=poster&c=space', '', 'testIframe' . $_GET['spaceid']);
            }
        } else {
            $info = $this->db->get_one(['spaceid' => $_GET['spaceid']]);
    ...
    public function public_call()
    {
        ...
        $r = $this->db->get_one(['spaceid' => $_GET['sid'], 'siteid' => $this->get_siteid()]);
    ...
    public function public_preview()
    {
        if (is_numeric($_GET['spaceid'])) {
            $_GET['spaceid'] = intval($_GET['spaceid']);
            $r = $this->db->get_one(['spaceid' => $_GET['spaceid'], 'siteid' => $this->get_siteid()]);
            $scheme = $_SERVER['SERVER_PORT'] == '443' ? 'https://' : 'http://';
            if ($r['type'] == 'code') {
                $db = pc_base::load_model('poster_model');
                $rs = $db->get_one(['spaceid' => $r['spaceid'], 'siteid' => $this->get_siteid()], 'setting', '`id` ASC');
                if ($rs['setting']) {
    ...
    /**
     * 删除广告版位
     * @param int $sid 广告版位的ID，当批量删除时系统会递归删除
     */
    public function delete()
    {
        if ((!isset($_GET['spaceid']) || empty($_GET['spaceid'])) && (!isset($_POST['spaceid']) || empty($_POST['spaceid']))) {
            ...
            if (is_array($_POST['spaceid'])) {
                array_map([$this, _del], $_POST['spaceid']); //如果是批量操作，则递归数组
            } elseif ($_GET['spaceid']) {
                $_GET['spaceid'] = intval($_GET['spaceid']);
                $db = pc_base::load_model('poster_model');
                $db->delete(['siteid' => $this->get_siteid(), 'spaceid' => $_GET['spaceid']]);
                $this->db->delete(['siteid' => $this->get_siteid(), 'spaceid' => $_GET['spaceid']]);
            }
    ...
    private function _del($spaceid = 0)
    {
        ...
        $db = pc_base::load_model('poster_model');
        $db->delete(['siteid' => $this->get_siteid(), 'spaceid' => $spaceid]);
        $this->db->delete(['siteid' => $this->get_siteid(), 'spaceid' => $spaceid]);
    ...
    public function setting()
    {
        if (isset($_POST['dosubmit'])) {
            ...
            $m_db->update(['setting' => $setting], ['module' => ROUTE_M]); //将配置信息存入数据表中
    ...
    public function poster_template()
    {
        ...
        $templatedir = PATH_PHPCMS . $tpl_root . pc_base::load_config('system', 'tpl_name') . DS . 'poster' . DS;
    ...
    public function public_tempate_setting()
    {
        ...
        if (isset($_POST['dosubmit'])) {
            if (is_array($_POST['info']['type']) && !empty($_POST['info']['type'])) {
                $type2name = ['images' => L('photo'), 'flash' => L('flash'), 'text' => L('title')];
                $type = [];
                foreach ($_POST['info']['type'] as $t) {
                    if (in_array($t, ['images', 'flash', 'text'])) {
                        $type[$t] = $type2name[$t];
                    } else {
                        continue;
                    }
    ...
    public function create_js($page = 0)
    {
        $page = max(intval($_GET['page']), 1);
        if ($page == 1) {
            $result = $this->db->get_one(['disabled' => 0, 'siteid' => get_siteid()], 'COUNT(*) AS num');
            if ($result['num']) {
                $total = $result['num'];
                $pages = ceil($total / 20);
            }
        ...
        $offset = ($page - 1) * 20;
        $data = $this->db->listinfo(['disabled' => 0, 'siteid' => get_siteid()], 'spaceid ASC', $page);
        $html = new html();
        foreach ($data as $d) {
            if ($d['type'] != 'code') {
    ...
    public function public_check_space()
    {
        ...
        if ($_GET['spaceid']) {
            ...
            $r = $this->db->get_one(['spaceid' => $spaceid, 'siteid' => $this->get_siteid()]);
            if ($r['name'] == $name) {
                exit('1');
            }
        }
        $r = $this->db->get_one(['siteid' => $this->get_siteid(), 'name' => $name], 'spaceid');
        if ($r['spaceid']) {
            exit('0');
        } else {
            exit('1');
    ...
    private function check($data = [])
    {
        ...
        $info = $this->db->get_one(['name' => $data['name'], 'siteid' => $this->get_siteid()], 'spaceid');
        if (($info['spaceid'] && $info['spaceid'] != $_GET['spaceid']) || ($info['spaceid'] && !isset($_GET['spaceid']))) {
            showmessage(L('space_exist'), HTTP_REFERER);
        }
        if ((!isset($data['width']) || $data['width'] == 0) && in_array($data['type'], ['banner', 'fixure', 'float', 'couplet', 'imagechange', 'imagelist'])) {
            showmessage(L('plate_width_not_empty'), HTTP_REFERER);
        } else {
            $data['width'] = intval($data['width']);
        }
        if ((!isset($data['height']) || $data['height'] == 0) && in_array($data['type'], ['banner', 'fixure', 'float', 'couplet', 'imagechange', 'imagelist'])) {
