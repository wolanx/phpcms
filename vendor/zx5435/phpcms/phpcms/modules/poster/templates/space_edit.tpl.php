...
<form method="post" action="?m=poster&c=space&a=edit&spaceid=<?php echo $_GET['spaceid'] ?>" name="myform" id="myform">
    <table class="table_form" width="100%" cellspacing="0">
        ...
        <tr>
            ...
            <td><?php echo form::select($TYPES, $info['type'], 'name="space[type]" id="type" onchange="AdsType(this.value)"') ?>&nbsp;&nbsp;<span id="ScrollSpan"
                                                                                                                                                  style="padding-left:30px;display:none;"><label><input
                                type="checkbox" id="ScrollBox" name="setting[scroll]"<?php if ($setting['scroll']) { ?> checked<?php } ?>
                                value='1'/> <?php echo L('rolling') ?></label></span>
                <span id="AlignSpan" style="padding-left:30px;display:none;"><label><input type="checkbox" <?php if ($setting['align']) { ?> checked<?php } ?> id="AlignBox"
                                                                                           name="setting[align]" value='1'/> <?php echo L('lightbox') ?></label></span></td>
        </tr>
        <tr id="trPosition" style="display:none;">
            <th align="right" valign="top"><strong><?php echo L('position') ?>：</strong></th>
            <td valign="top" colspan="2">
                <?php echo L('left_margin') ?>：<input name='setting[paddleft]' id='PaddingLeft' type='text' size='5' value='<?php echo $setting['paddleft'] ?>' class="input-text">
                px&nbsp;&nbsp;
                <?php echo L('top_margin') ?>：<input name='setting[paddtop]' id='PaddingTop' type='text' size='5' value='<?php echo $setting['paddtop'] ?>' class="input-text"/>
                px</div>
            </td>
        </tr>
        <tr id="SizeFormat">
            <th><strong><?php echo L('size_format') ?>：</strong></th>
            <td><label><?php echo L('plate_width') ?></label><input name="space[width]" id="s_width" class="input-text" type="text" value="<?php echo $info['width'] ?>" size="10">
                px &nbsp;&nbsp;&nbsp;&nbsp; <label><?php echo L('plate_height') ?></label><input name="space[height]" id="h_height" type="text" class="input-text"
                                                                                                 value="<?php echo $info['height'] ?>" size="10"> px
                <div id="w_hTip"></div>
            </td>
        </tr>
        <tr>
...
<script language="javascript" type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        $("#name").formValidator({
            onshow: "<?php echo L('please_input_space_name')?>",
            onfocus: "<?php echo L('spacename_three_length')?>",
            oncorrect: "<?php echo L('correct')?>"
        }).inputValidator({min: 6, onerror: "<?php echo L('spacename_illegality')?>"}).ajaxValidator({
            type: "get",
            url: "",
            data: "m=poster&c=space&a=public_check_space&spaceid=<?php echo $_GET['spaceid']?>",
            datatype: "html",
            cached: false,
            async: 'true',
        ...
        $('#type').formValidator({
            onshow: "<?php echo L('choose_space_type')?>",
            onfocus: "<?php echo L('choose_space_type')?>",
            oncorrect: "<?php echo L('correct')?>"
        }).inputValidator();
        $('#s_width').formValidator({
            tipid: "w_hTip",
            onshow: "<?php echo L('input_width_height')?>",
            onfocus: "<?php echo L('three_numeric')?>",
            oncorrect: "<?php echo L('correct')?>"
        }).inputValidator().defaultPassed();
    })
</script>