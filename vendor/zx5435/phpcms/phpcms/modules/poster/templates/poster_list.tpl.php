    ...
    <form name="myform" action="?m=poster&c=poster&a=listorder" method="post">
        <div class="table-list">
            <table width="100%" cellspacing="0" class="contentWrap">
                ...
                if (is_array($infos)) {
                    foreach ($infos as $info) {
                        $space = $this->s_db->get_one(['spaceid' => $info['spaceid']], 'name');
                        ?>
                        <tr>
                            <td align="center">
                            ...
                            <td align="center"><?php if ($info['disabled']) {
                                    echo L('stop');
                                } elseif ((strtotime($info['enddate']) < SYS_TIME) && (strtotime($info['enddate']) > 0)) {
                                    echo L('past');
                                } else {
                                    echo L('start');
                                } ?></td>
                            ...
                            <td align="center"><a
                                        href="index.php?m=poster&c=poster&a=edit&id=<?php echo $info['id']; ?>&pc_hash=<?php echo $_SESSION['pc_hash']; ?>&menuid=<?php echo $_GET['menuid'] ?>"><?php echo L('edit') ?></a>|<a
                                        href="?m=poster&c=poster&a=stat&id=<?php echo $info['id'] ?>&spaceid=<?php echo $_GET['spaceid']; ?>"><?php echo L('stat') ?></a></td>
                        </tr>
                        <?php
                    }
                }
            ...
            <div class="btn"><label for="check_box"><?php echo L('selected_all') ?>/<?php echo L('cancel') ?></label>
                ...
                <input name="submit" type="submit" class="button" value="<?php echo L('delete') ?>"
                       onClick="document.myform.action='?m=poster&c=poster&a=delete';return confirm('<?php echo L('confirm', ['message' => L('selected')]) ?>')">&nbsp;&nbsp;
            </div>
        </div>
...
<script type="text/javascript">
    function edit(id, name) {
        ...
        window.top.art.dialog({
            ...
            id: 'edit',
            iframe: '?m=poster&c=poster&a=edit&id=' + id,
            width: '600px',
            height: '430px'
        }, function () {
