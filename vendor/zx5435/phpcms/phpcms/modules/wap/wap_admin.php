...
namespace phpcms\modules\wap;

use phpcms\libs\classes\form;
use phpcms\libs\classes\tree;
use phpcms\model\wap_model;
use phpcms\model\wap_type_model;
...
class wap_admin extends admin
{
    ...
    public function __construct()
    {
        ...
        $this->db = new wap_model();
        $this->type_db = new wap_type_model();
    }
    ...
    function init()
    {
        ...
        $show_dialog = true;
        $big_menu = [
            'javascript:window.top.art.dialog({id:\'add\',iframe:\'?m=wap&c=wap_admin&a=add\', title:\'' . L('add_site') . '\', width:\'400\', height:\'550\', lock:true}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);',
            L('wap_add_site'),
        ];
    ...
    function edit()
    {
        if ($_POST['dosubmit']) {
            ...
            $this->db->update(['sitename' => $sitename, 'logo' => $logo, 'domain' => $domain, 'setting' => $setting], ['siteid' => $siteid]);
            $this->wap_site_cache();
            ...
            $info = $this->db->get_one(['siteid' => $siteid]);
            if ($info) {
                /** @var $setting */
                extract($info);
    ...
    function add()
    {
        if ($_POST['dosubmit']) {
            $siteid = intval($_POST['siteid']) ? intval($_POST['siteid']) : showmessage(L('parameter_error'), HTTP_REFERER);
            if ($this->db->get_one(['siteid' => $siteid])) {
            ...
            $domain = trim($_POST['domain']);
            $setting = array2string($_POST['setting']);
            $return_id = $this->db->insert(['siteid' => $siteid, 'sitename' => $sitename, 'logo' => $logo, 'domain' => $domain, 'setting' => $setting], '1');
            $this->wap_site_cache();
    ...
    function delete()
    {
        $siteid = intval($_GET['siteid']) ? intval($_GET['siteid']) : showmessage(L('parameter_error'), HTTP_REFERER);
        if ($siteid == 1) {
            showmessage(L('wap_permission_denied_del'), HTTP_REFERER);
        }
        $this->db->delete(['siteid' => $siteid]);
        $this->type_db->delete(['siteid' => $siteid]);
        $this->wap_site_cache();
    ...
    function public_status()
    {
        ...
        $siteid = intval($_GET['siteid']) ? intval($_GET['siteid']) : showmessage(L('parameter_error'), HTTP_REFERER);
        $this->db->update(['status' => $status], ['siteid' => $siteid]);
        $this->wap_site_cache();
    ...
    function type_manage()
    {
        ...
        if ($_POST['dosubmit']) {
            ...
            $info['siteid'] = $siteid;
            if ($this->type_db->get_one(['cat' => $info['cat'], 'siteid' => $siteid])) {
            ...
            $result = $this->type_db->select(['siteid' => $siteid], '*', '', 'listorder ASC,typeid DESC');
            foreach ($result as $r) {
        ...
        if ($_POST['dosubmit']) {
            if (is_array($_POST['ids']) && !empty($_POST['ids'])) {
                foreach ($_POST['ids'] as $id) {
                    if ($this->type_db->get_one(['parentid' => $id])) {
                        showmessage(L('wap_type_del_error'), HTTP_REFERER);
                    } else {
                        $this->type_db->delete(['typeid' => $id]);
                    }
                }
            }
            showmessage(L('operation_success'), HTTP_REFERER);
