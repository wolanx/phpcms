...
namespace phpcms\modules\tag;

use pc_base;
use phpcms\model\dbsource_model;
use phpcms\model\tag_model;
...
class tag extends admin
{
    ...
    {
        $this->db = new tag_model();
        $this->dbsource = new dbsource_model();
        parent::__construct();
    }
    ...
    {
        $big_menu = [
            'javascript:window.top.art.dialog({id:\'add\',iframe:\'?m=tag&c=tag&a=add\', title:\'' . L('add_tag') . '\', width:\'700\', height:\'500\', lock:true}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);',
            L('add_tag'),
        ];
    ...
    public function add()
    {
        ...
        if (isset($_POST['dosubmit'])) {
            ...
            $siteid = $this->get_siteid();
            $dbsource_data = $dbsource = [];
            $dbsource[] = L('please_select');
            $dbsource_data = $this->dbsource->select(['siteid' => $siteid], 'name');
