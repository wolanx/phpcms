...
class plugin_admin
{
    ...
    public function oauth()
    {
        ...
        if (!isset($_REQUEST['oauth_verifier']) || $_REQUEST['oauth_verifier'] == '') {
            if ($this->auth_db->get_one(['source' => 'sina'])) {
            ...
            $this->auth_db->insert(['uid' => $me['id'], 'token' => $last_key['oauth_token'], 'tsecret' => $last_key['oauth_token_secret'], 'source' => 'sina']);
    ...
    {
        if ($this->auth_db->delete(['source' => 'sina'])) {
            showmessage('ok');
        } else {
            showmessage('fail');
        }
