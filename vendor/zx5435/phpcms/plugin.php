 ...
 */
define('PHPCMS_PATH', dirname(__FILE__) . DS);
...
class plugin_base
{
    ...
    private static function plugin_init()
    {
        ...
        if (method_exists($controller, PLUGIN_ACTION)) {
            if (preg_match('/^[_]/i', PLUGIN_ACTION)) {
                ...
                call_user_func([$controller, PLUGIN_ACTION]);
            }
        } else {
     ...
     * @return mixed
     */
    private static function load_controller($filename = '', $m = '')
    {
        if (empty($filename)) {
        ...
        $filepath = PATH_PHPCMS . 'plugin' . DS . $m . DS . $filename . '.class.php';
    ...
    final public static function plugin_tpl($file, $identification = '')
    {
        ...
        return PATH_PHPCMS . 'plugin' . DS . $identification . DS . 'templates' . DS . $file . '.tpl.php';
    }
